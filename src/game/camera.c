 ...
 */
Vec3f sPlayer2FocusOffset[2];
/**
 * The pitch used for the credits easter egg.
 */
s16 sCreditsPlayer2Pitch[2];
/**
 * The yaw used for the credits easter egg.
 */
s16 sCreditsPlayer2Yaw[2];
/**
 * Used to decide when to zoom out in the pause menu.
 */
u8 sFramesPaused;

extern struct CameraFOVStatus sFOVState[2];
extern struct TransitionInfo sModeTransition[2];
extern struct PlayerGeometry sMarioGeometry[2];
extern s16 sAvoidYawVel[2];
extern s16 sCameraYawAfterDoorCutscene[2];
extern struct HandheldShakePoint sHandheldShakeSpline[4][2];
extern s16 sHandheldShakeMag[2];
extern f32 sHandheldShakeTimer[2];
extern f32 sHandheldShakeInc[2];
extern s16 sHandheldShakePitch[2];
extern s16 sHandheldShakeYaw[2];
extern s16 sHandheldShakeRoll[2];
extern s16 sSelectionFlags[2];
extern s16 s2ndRotateFlags[2];
extern s16 sCameraSoundFlags[2];
extern u16 sCButtonsPressed[2];
extern s16 sCutsceneDialogID[2];
extern struct LakituState gLakituState[2];
extern s16 sAreaYaw[2];
extern s16 sAreaYawChange[2];
extern s16 sLakituDist[2];
extern s16 sLakituPitch[2];
extern f32 sZoomAmount[2];
extern s16 sCSideButtonYaw[2];
extern s16 sBehindMarioSoundTimer[2];
extern f32 sZeroZoomDist[2];
extern s16 sCUpCameraPitch[2];
extern s16 sModeOffsetYaw[2];
extern s16 sSpiralStairsYawOffset[2];
extern s16 s8DirModeBaseYaw[2];
extern s16 s8DirModeYawOffset[2];
extern f32 sPanDistance[2];
extern f32 sCannonYOffset[2];
extern struct ModeTransitionInfo sModeInfo[2];
extern Vec3f sCastleEntranceOffset[2];
extern u32 sParTrackIndex[2];
extern struct ParallelTrackingPoint *sParTrackPath[2];
extern struct CameraStoredInfo sParTrackTransOff[2];
extern struct CameraStoredInfo sCameraStoreCUp[2];
extern struct CameraStoredInfo sCameraStoreCutscene[2];
extern s16 gCameraMovementFlags[2];
extern s16 sStatusFlags[2];
extern struct CutsceneSplinePoint sCurCreditsSplinePos[2][32];
extern struct CutsceneSplinePoint sCurCreditsSplineFocus[2][32];
extern s16 sCutsceneSplineSegment[2];
extern f32 sCutsceneSplineSegmentProgress[2];
extern s16 sCutsceneShot[2];
extern s16 gCutsceneTimer[2];
extern struct CutsceneVariable sCutsceneVars[10][2];
extern s32 gObjCutsceneDone[2];
extern u32 gCutsceneObjSpawn[2];
extern struct Camera *gCamera; // dont do this

/**
 * Lakitu's position and focus.
 * @see LakituState
 */
struct LakituState gLakituState[2];
struct CameraFOVStatus sFOVState[2];
struct TransitionInfo sModeTransition[2];
struct PlayerGeometry sMarioGeometry[2];
struct Camera *gCamera; // dont do this
 ...
 */
struct CutsceneSplinePoint sCurCreditsSplinePos[2][32];

/**
 * The current spline that controls the camera's focus during the credits.
 */
struct CutsceneSplinePoint sCurCreditsSplineFocus[2][32];
 ...
 * When it becomes >= 1, 1.0 is subtracted from it and sCutsceneSplineSegment[c->cameraID] is increased.
 */
f32 sCutsceneSplineSegmentProgress[2];

/**
 * The current segment of the CutsceneSplinePoint[] being used.
 */
s16 sCutsceneSplineSegment[2];
//s16 unused8033B6E8;

// Shaky Hand-held Camera effect variables
struct HandheldShakePoint sHandheldShakeSpline[4][2];
s16 sHandheldShakeMag[2];
f32 sHandheldShakeTimer[2];
f32 sHandheldShakeInc[2];
s16 sHandheldShakePitch[2];
s16 sHandheldShakeYaw[2];
s16 sHandheldShakeRoll[2];

/**
 * Controls which object to spawn in the intro and ending cutscenes.
 */
u32 gCutsceneObjSpawn[2];
/**
 * Controls when an object-based cutscene should end. It's only used in the star spawn cutscenes, but
 * Yoshi also toggles this.
 */
s32 gObjCutsceneDone[2];
...
/**
 * How much sAreaYaw[c->cameraID] changed when Mario moved.
 */
s16 sAreaYawChange[2];
 ...
 */
s16 sBehindMarioSoundTimer[2];

/**
 * Virtually unused aside being set to 0 and compared with gCameraZoomDist[c->cameraID] (which is never < 0)
 */
f32 sZeroZoomDist[2];
 ...
 */
struct CutsceneVariable sCutsceneVars[10][2];
struct ModeTransitionInfo sModeInfo[2];
/**
 * Offset added to sFixedModeBasePosition[c->cameraID] when Mario is inside, near the castle lobby entrance
 */
Vec3f sCastleEntranceOffset[2];

/**
 * The index into the current parallel tracking path
 */
u32 sParTrackIndex[2];

/**
 * The current list of ParallelTrackingPoints used in update_parallel_tracking_camera()
 */
struct ParallelTrackingPoint *sParTrackPath[2];
 ...
 */
u8 gRecentCutscene[2] = { 0, 0 }; // KEEP GOING HERE

/**
 * A timer that increments for 8 frames when a cutscene ends.
 * When it reaches 8, it sets gRecentCutscene[c->cameraID] to 0.
 */
u8 sFramesSinceCutsceneEnded[2] = { 0, 0 };
/**
 ...
 */
u8 sCutsceneDialogResponse[2] = { 0, 0 };
...
Vec3f sFixedModeBasePosition[2] = { { 646.0f, 143.0f, -1513.0f }, { 646.0f, 143.0f, -1513.0f } };
int areaWarped = 0;
 ...
 */
void set_camera_shake_from_hit(s16 shake, struct Camera *c) {
    switch (shake) {
        // Makes the camera stop for a bit
        case SHAKE_ATTACK:
            gLakituState[c->cameraID].focHSpeed = 0;
            gLakituState[c->cameraID].posHSpeed = 0;
            break;
        ...
        case SHAKE_SMALL_DAMAGE:
            if ((&gPlayerCameraState[c->cameraID])->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER)) {
                set_camera_yaw_shake(0x200, 0x10, 0x1000, c);
                set_camera_roll_shake(0x400, 0x20, 0x1000, c);
                set_fov_shake(0x100, 0x30, -0x8000, c);
            } else {
        ...
        case SHAKE_MED_DAMAGE:
            if ((&gPlayerCameraState[c->cameraID])->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER)) {
                ...
                set_fov_shake(0x180, 0x40, -0x8000, c);
            } else {
        ...
        case SHAKE_LARGE_DAMAGE:
            if ((&gPlayerCameraState[c->cameraID])->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER)) {
                ...
                set_fov_shake(0x200, 0x50, -0x8000, c);
            } else {
        ...
        case SHAKE_HIT_FROM_BELOW:
            gLakituState[c->cameraID].focHSpeed = 0.07;
            gLakituState[c->cameraID].posHSpeed = 0.07;
            break;

        case SHAKE_SHOCK:
            set_camera_pitch_shake(random_float() * 64.f, 0x8, 0x8000, c);
            set_camera_yaw_shake(random_float() * 64.f, 0x8, 0x8000, c);
            break;
    }
}
 ...
 */
void set_environmental_camera_shake(s16 shake, struct Camera *c) {
    switch (shake) {
        ...
        case SHAKE_ENV_FALLING_BITS_PLAT:
            set_camera_pitch_shake(0x40, 0x2, 0x8000, c);
            break;
...
f32 calc_y_to_curr_floor(f32 *posOff, f32 posMul, f32 posBound, f32 *focOff, f32 focMul, f32 focBound) {
    f32 floorHeight = sMarioGeometry[gCamera->cameraID].currFloorHeight;
    f32 waterHeight;
    UNUSED s32 filler;

    if (!((&gPlayerCameraState[gCamera->cameraID])->action & ACT_FLAG_METAL_WATER)) {
        //! @bug this should use sMarioGeometry[gCamera->cameraID].waterHeight
        if (floorHeight < (waterHeight = find_water_level(
            (&gPlayerCameraState[gCamera->cameraID])->pos[0], 
            (&gPlayerCameraState[gCamera->cameraID])->pos[2]))) {
            floorHeight = waterHeight;
        }
    }

    if ((&gPlayerCameraState[gCamera->cameraID])->action & ACT_FLAG_ON_POLE) {
        if (sMarioGeometry[gCamera->cameraID].currFloorHeight 
            >= gMarioStates[0].usedObj->oPosY 
            && (&gPlayerCameraState[gCamera->cameraID])->pos[1]
                   < 0.7f * gMarioStates[0].usedObj->hitboxHeight + gMarioStates[0].usedObj->oPosY) {
            posBound = 1200;
        }
    }

    *posOff = (floorHeight - (&gPlayerCameraState[gCamera->cameraID])->pos[1]) * posMul;
    ...
    *focOff = (floorHeight - (&gPlayerCameraState[gCamera->cameraID])->pos[1]) * focMul;
...
void focus_on_mario(Vec3f focus, Vec3f pos, f32 posYOff, f32 focYOff, f32 dist, s16 pitch, s16 yaw) {
    ...
    marioPos[0] = (&gPlayerCameraState[gCamera->cameraID])->pos[0];
    marioPos[1] = (&gPlayerCameraState[gCamera->cameraID])->pos[1] + posYOff;
    marioPos[2] = (&gPlayerCameraState[gCamera->cameraID])->pos[2];

    vec3f_set_dist_and_angle(marioPos, pos, dist, pitch + sLakituPitch[gCamera->cameraID], yaw);

    focus[0] = (&gPlayerCameraState[gCamera->cameraID])->pos[0];
    focus[1] = (&gPlayerCameraState[gCamera->cameraID])->pos[1] + focYOff;
    focus[2] = (&gPlayerCameraState[gCamera->cameraID])->pos[2];
}
...
void set_camera_height(struct Camera *c, f32 goalHeight) {
    ...
    f32 camFloorHeight;
    UNUSED u8 filler[8];
    UNUSED s16 action = (&gPlayerCameraState[c->cameraID])->action;
    f32 baseOff = 125.f;
    f32 camCeilHeight = find_ceil(c->pos[0], gLakituState[c->cameraID].goalPos[1] - 50.f, c->pos[2], &surface);

    if ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_HANGING) {
        marioCeilHeight = sMarioGeometry[c->cameraID].currCeilHeight;
        marioFloorHeight = sMarioGeometry[c->cameraID].currFloorHeight;
        ...
        if ((&gPlayerCameraState[c->cameraID])->pos[1] - 400 > goalHeight) {
            goalHeight = (&gPlayerCameraState[c->cameraID])->pos[1] - 400;
        }

        approach_camera_height(c, goalHeight, 5.f);
    } else {
        camFloorHeight = find_floor(c->pos[0], c->pos[1] + 100.f, c->pos[2], &surface) + baseOff;
        marioFloorHeight = baseOff + sMarioGeometry[c->cameraID].currFloorHeight;
        ...
        if ((&gPlayerCameraState[c->cameraID])->action == ACT_BUTT_STUCK_IN_GROUND ||
            (&gPlayerCameraState[c->cameraID])->action == ACT_HEAD_STUCK_IN_GROUND ||
            (&gPlayerCameraState[c->cameraID])->action == ACT_FEET_STUCK_IN_GROUND) {
        ...
        if (camCeilHeight != 20000.f) {
            ...
            if ((c->pos[1] > camCeilHeight && sMarioGeometry[c->cameraID].currFloorHeight + baseOff < camCeilHeight)
                || (sMarioGeometry[c->cameraID].currCeilHeight != 20000.f
                    && sMarioGeometry[c->cameraID].currCeilHeight > camCeilHeight && c->pos[1] > camCeilHeight)) {
...
s16 look_down_slopes(s16 camYaw) {
    ...
    f32 xOff = (&gPlayerCameraState[gCamera->cameraID])->pos[0] + sins(camYaw) * 40.f;
    f32 zOff = (&gPlayerCameraState[gCamera->cameraID])->pos[2] + coss(camYaw) * 40.f;

    floorDY = find_floor(xOff, (&gPlayerCameraState[gCamera->cameraID])->pos[1], zOff, &floor) - (&gPlayerCameraState[gCamera->cameraID])->pos[1];
 ...
 *      yaw = -yaw;
 *      pan[0] = sins((&gPlayerCameraState[c->cameraID])->faceAngle[1] + yaw) * sins(0xC00) * dist;
...
void pan_ahead_of_player(struct Camera *c) {
    ...
    vec3f_get_dist_and_angle(c->pos, (&gPlayerCameraState[c->cameraID])->pos, &dist, &pitch, &yaw);

    // The camera will pan ahead up to about 30% of the camera's distance to Mario.
    pan[2] = sins(0xC00) * dist;

    rotate_in_xz(pan, pan, (&gPlayerCameraState[c->cameraID])->faceAngle[1]);
    ...
    if ((&gPlayerCameraState[c->cameraID])->action == ACT_LONG_JUMP ||
       ((&gPlayerCameraState[c->cameraID])->action != ACT_TOP_OF_POLE && ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_ON_POLE))) {
        pan[0] = -pan[0];
    }

    // Slowly make the actual pan, sPanDistance[c->cameraID], approach the calculated pan
    // If Mario is sleeping, then don't pan
    if (sStatusFlags[c->cameraID] & CAM_FLAG_SLEEPING) {
        approach_f32_asymptotic_bool(&sPanDistance[c->cameraID], 0.f, 0.025f);
    } else {
        approach_f32_asymptotic_bool(&sPanDistance[c->cameraID], pan[0], 0.025f);
    }

    // Now apply the pan. It's a dir vector to the left or right, rotated by the camera's yaw to Mario
    pan[0] = sPanDistance[c->cameraID];
    yaw = -yaw;
    rotate_in_xz(pan, pan, yaw);
...
s32 update_radial_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    f32 cenDistX = (&gPlayerCameraState[c->cameraID])->pos[0] - c->areaCenX;
    f32 cenDistZ = (&gPlayerCameraState[c->cameraID])->pos[2] - c->areaCenZ;
    s16 camYaw = atan2s(cenDistZ, cenDistX) + sModeOffsetYaw[c->cameraID];
    ...
    sAreaYaw[c->cameraID] = camYaw - sModeOffsetYaw[c->cameraID];
    calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 0.9f, 200.f);
    focus_on_mario(focus, pos, posY + yOff, focusY + yOff, sLakituDist[c->cameraID] + baseDist, pitch, camYaw);
...
s32 update_8_directions_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    UNUSED f32 cenDistX = (&gPlayerCameraState[c->cameraID])->pos[0] - c->areaCenX;
    UNUSED f32 cenDistZ = (&gPlayerCameraState[c->cameraID])->pos[2] - c->areaCenZ;
    s16 camYaw = s8DirModeBaseYaw[c->cameraID] + s8DirModeYawOffset[c->cameraID];
    s16 pitch = look_down_slopes(camYaw);
    f32 posY;
    f32 focusY;
    UNUSED f32 unused1;
 ...
 *
 * If sModeOffsetYaw[c->cameraID] is 0, the camera points directly at the area center point.
 */
void radial_camera_move(struct Camera *c) {
    ...
    f32 areaDistX = (&gPlayerCameraState[c->cameraID])->pos[0] - c->areaCenX;
    f32 areaDistZ = (&gPlayerCameraState[c->cameraID])->pos[2] - c->areaCenZ;
    UNUSED s32 filler;

    // How much the camera's yaw changed
    s16 yawOffset = calculate_yaw((&gPlayerCameraState[c->cameraID])->pos, c->pos) - atan2s(areaDistZ, areaDistX);
    ...
    if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE)) {
        if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_CAMERA_MIDDLE
            && sMarioGeometry[c->cameraID].prevFloorType != SURFACE_CAMERA_MIDDLE) {
            gCameraMovementFlags[c->cameraID] |= (CAM_MOVE_RETURN_TO_MIDDLE | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
        if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_CAMERA_ROTATE_RIGHT
            && sMarioGeometry[c->cameraID].prevFloorType != SURFACE_CAMERA_ROTATE_RIGHT) {
            gCameraMovementFlags[c->cameraID] |= (CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
        if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_CAMERA_ROTATE_LEFT
            && sMarioGeometry[c->cameraID].prevFloorType != SURFACE_CAMERA_ROTATE_LEFT) {
            gCameraMovementFlags[c->cameraID] |= (CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
    }

    if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ENTERED_ROTATE_SURFACE) {
        rotateSpeed = 0x200;
    }
    ...
    if (avoidStatus == 3) {
        ...
        // We want to change sModeOffsetYaw[c->cameraID] so that the player is no longer obstructed by the wall.
    ...
    if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_RETURN_TO_MIDDLE) {
        if (camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], 0, rotateSpeed) == 0) {
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_RETURN_TO_MIDDLE;
        }
    } else {
        // Prevent the player from rotating into obstructing walls
        if ((gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT) && avoidStatus == 3
            && avoidYaw + 0x10 < sModeOffsetYaw[c->cameraID]) {
            sModeOffsetYaw[c->cameraID] = avoidYaw;
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
        if ((gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT) && avoidStatus == 3
            && avoidYaw - 0x10 > sModeOffsetYaw[c->cameraID]) {
            sModeOffsetYaw[c->cameraID] = avoidYaw;
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
        ...
        if (!(s2ndRotateFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT) && (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT)
            && camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], maxAreaYaw, rotateSpeed) == 0) {
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }
        if (!(s2ndRotateFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT) && (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT)
            && camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], minAreaYaw, rotateSpeed) == 0) {
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
        }

        // If it's the second time rotating, rotate all the way to +-105 degrees.
        if ((s2ndRotateFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT) && (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT)
            && camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], DEGREES(105), rotateSpeed) == 0) {
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_RIGHT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
            s2ndRotateFlags[c->cameraID] &= ~CAM_MOVE_ROTATE_RIGHT;
        }
        if ((s2ndRotateFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT) && (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT)
            && camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], DEGREES(-105), rotateSpeed) == 0) {
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_ROTATE_LEFT | CAM_MOVE_ENTERED_ROTATE_SURFACE);
            s2ndRotateFlags[c->cameraID] &= ~CAM_MOVE_ROTATE_LEFT;
        }
    }
    if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE)) {
        // If not rotating, rotate away from walls obscuring Mario from view
        if (avoidStatus == 3) {
            approach_s16_asymptotic_bool(&sModeOffsetYaw[c->cameraID], avoidYaw, 10);
        } else {
            if (c->mode == CAMERA_MODE_RADIAL) {
                // sModeOffsetYaw[c->cameraID] only updates when Mario is moving
                rotateSpeed = gMarioStates[0].forwardVel / 32.f * 128.f;
                camera_approach_s16_symmetric_bool(&sModeOffsetYaw[c->cameraID], yawOffset, rotateSpeed);
            }
            if (c->mode == CAMERA_MODE_OUTWARD_RADIAL) {
                sModeOffsetYaw[c->cameraID] = offset_yaw_outward_radial(c, atan2s(areaDistZ, areaDistX));
            }
        }
    }

    // Bound sModeOffsetYaw[c->cameraID] within (-120, 120) degrees
    if (sModeOffsetYaw[c->cameraID] > 0x5554) {
        sModeOffsetYaw[c->cameraID] = 0x5554;
    }
    if (sModeOffsetYaw[c->cameraID] < -0x5554) {
        sModeOffsetYaw[c->cameraID] = -0x5554;
    }
 ...
 * When C-Down mode is not active, sLakituDist[gCamera->cameraID] and sLakituPitch[gCamera->cameraID] decrease to 0.
 */
void lakitu_zoom(f32 rangeDist, s16 rangePitch) { // this function is fine using gCamera
    if (sLakituDist[gCamera->cameraID] < 0) {
        if ((sLakituDist[gCamera->cameraID] += 30) > 0) {
            sLakituDist[gCamera->cameraID] = 0;
        }
    } else if (rangeDist < sLakituDist[gCamera->cameraID]) {
        if ((sLakituDist[gCamera->cameraID] -= 30) < rangeDist) {
            sLakituDist[gCamera->cameraID] = rangeDist;
        }
    } else if (gCameraMovementFlags[gCamera->cameraID] & CAM_MOVE_ZOOMED_OUT) {
        if ((sLakituDist[gCamera->cameraID] += 30) > rangeDist) {
            sLakituDist[gCamera->cameraID] = rangeDist;
        }
    } else {
    ...
    if (gCameraMovementFlags[gCamera->cameraID] & CAM_MOVE_ZOOMED_OUT) {
        if ((sLakituPitch[gCamera->cameraID] += rangePitch / 13) > rangePitch) {
            sLakituPitch[gCamera->cameraID] = rangePitch;
        }
    } else {
        if ((sLakituPitch[gCamera->cameraID] -= rangePitch / 13) < 0) {
            sLakituPitch[gCamera->cameraID] = 0;
        }
    }
}
 ...
 */
void update_yaw_and_dist_from_c_up(struct Camera *c) {
    f32 dist = 1000.f;

    sModeOffsetYaw[c->cameraID] = sModeInfo[c->cameraID].transitionStart.yaw - sAreaYaw[c->cameraID];
    sLakituDist[c->cameraID] = sModeInfo[c->cameraID].transitionStart.dist - dist;
    // No longer in C-Up
    gCameraMovementFlags[c->cameraID] &= ~CAM_MOVING_INTO_MODE;
}
...
void mode_radial_camera(struct Camera *c) {
    Vec3f pos;
    UNUSED u8 unused1[8];
    s16 oldAreaYaw = sAreaYaw[c->cameraID];
    UNUSED u8 unused2[4];

    if (gCameraMovementFlags[c->cameraID] & CAM_MOVING_INTO_MODE) {
    ...
    c->pos[0] = pos[0];
    c->pos[2] = pos[2];
    sAreaYawChange[c->cameraID] = sAreaYaw[c->cameraID] - oldAreaYaw;
    if ((&gPlayerCameraState[c->cameraID])->action == ACT_RIDING_HOOT) {
        pos[1] += 500.f;
    }
...
void mode_8_directions_camera(struct Camera *c) {
    ...
    if (c->controller->buttonPressed & R_CBUTTONS) {
        s8DirModeYawOffset[c->cameraID] += DEGREES(45);
 ...
 * sModeOffsetYaw[c->cameraID] is calculated in radial_camera_move, which calls offset_yaw_outward_radial
 */
s32 update_outward_radial_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    f32 xDistFocToMario = (&gPlayerCameraState[c->cameraID])->pos[0] - c->areaCenX;
    f32 zDistFocToMario = (&gPlayerCameraState[c->cameraID])->pos[2] - c->areaCenZ;
    s16 camYaw = atan2s(zDistFocToMario, xDistFocToMario) + sModeOffsetYaw[c->cameraID] + DEGREES(180);
    s16 pitch = look_down_slopes(camYaw);
    f32 baseDist = 1000.f;
    ...
    sAreaYaw[c->cameraID] = camYaw - sModeOffsetYaw[c->cameraID] - DEGREES(180);
    calc_y_to_curr_floor(&posY, 1.f, 200.f, &focusY, 0.9f, 200.f);
    focus_on_mario(focus, pos, posY + yOff, focusY + yOff, sLakituDist[c->cameraID] + baseDist, pitch, camYaw);
 ...
 *
 * Uses the line between the next two points in sParTrackPath[c->cameraID]
...
s32 update_parallel_tracking_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    vec3f_copy(path[0], sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].pos);
    vec3f_copy(path[1], sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].pos);

    distThresh = sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].distThresh;
    zoom = sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].zoom;
    calc_y_to_curr_floor(&marioFloorDist, 1.f, 200.f, &marioFloorDist, 0.9f, 200.f);

    marioPos[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    // Mario's y pos + ~Mario's height + Mario's height above the floor
    marioPos[1] = (&gPlayerCameraState[c->cameraID])->pos[1] + 150.f + marioFloorDist;
    marioPos[2] = (&gPlayerCameraState[c->cameraID])->pos[2];
    ...
    if (sStatusFlags[c->cameraID] & CAM_FLAG_CHANGED_PARTRACK_INDEX) {
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_CHANGED_PARTRACK_INDEX;
        sParTrackTransOff[c->cameraID].pos[0] = oldPos[0] - c->pos[0];
        sParTrackTransOff[c->cameraID].pos[1] = oldPos[1] - c->pos[1];
        sParTrackTransOff[c->cameraID].pos[2] = oldPos[2] - c->pos[2];
    }
    // Slowly transition to the next path
    approach_f32_asymptotic_bool(&sParTrackTransOff[c->cameraID].pos[0], 0.f, 0.025f);
    approach_f32_asymptotic_bool(&sParTrackTransOff[c->cameraID].pos[1], 0.f, 0.025f);
    approach_f32_asymptotic_bool(&sParTrackTransOff[c->cameraID].pos[2], 0.f, 0.025f);
    vec3f_add(c->pos, sParTrackTransOff[c->cameraID].pos);

    // Check if the camera should go to the next path
    if (sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].startOfPath != 0) {
        // get Mario's distance to the next path
        calculate_angles(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].pos, sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 2].pos, &nextPitch, &nextYaw);
        vec3f_set_dist_and_angle(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].pos, nextPathPos, 400.f, nextPitch, nextYaw);
        distToPrev = calc_abs_dist(marioPos, nextPathPos);

        // get Mario's distance to the previous path
        calculate_angles(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].pos, sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].pos, &prevPitch, &prevYaw);
        vec3f_set_dist_and_angle(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID] + 1].pos, prevPathPos, 400.f, prevPitch, prevYaw);
        distToNext = calc_abs_dist(marioPos, prevPathPos);
        if (distToPrev < distToNext) {
            sParTrackIndex[c->cameraID]++;
            sStatusFlags[c->cameraID] |= CAM_FLAG_CHANGED_PARTRACK_INDEX;
        }
    }

    // Check if the camera should go to the previous path
    if (sParTrackIndex[c->cameraID] != 0) {
        // get Mario's distance to the next path
        calculate_angles((*(sParTrackPath[c->cameraID] + sParTrackIndex[c->cameraID])).pos, (*(sParTrackPath[c->cameraID] + sParTrackIndex[c->cameraID] + 1)).pos, &nextPitch, &nextYaw);
        vec3f_set_dist_and_angle(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].pos, nextPathPos, 700.f, nextPitch, nextYaw);
        distToPrev = calc_abs_dist(marioPos, nextPathPos);

        // get Mario's distance to the previous path
        calculate_angles((*(sParTrackPath[c->cameraID] + sParTrackIndex[c->cameraID])).pos, (*(sParTrackPath[c->cameraID] + sParTrackIndex[c->cameraID] - 1)).pos, &prevPitch, &prevYaw);
        vec3f_set_dist_and_angle(sParTrackPath[c->cameraID][sParTrackIndex[c->cameraID]].pos, prevPathPos, 700.f, prevPitch, prevYaw);
        distToNext = calc_abs_dist(marioPos, prevPathPos);
        if (distToPrev > distToNext) {
            sParTrackIndex[c->cameraID]--;
            sStatusFlags[c->cameraID] |= CAM_FLAG_CHANGED_PARTRACK_INDEX;
...
s32 update_fixed_camera(struct Camera *c, Vec3f focus, UNUSED Vec3f pos) {
    ...
    vec3f_copy(focus, (&gPlayerCameraState[c->cameraID])->pos);
    focus[1] += focusFloorOff + 125.f;
    vec3f_get_dist_and_angle(focus, c->pos, &distCamToFocus, &faceAngle[0], &faceAngle[1]);
    faceAngle[2] = 0;

    vec3f_copy(basePos, sFixedModeBasePosition[c->cameraID]);
    vec3f_add(basePos, sCastleEntranceOffset[c->cameraID]);

    if (sMarioGeometry[c->cameraID].currFloorType != SURFACE_DEATH_PLANE
        && sMarioGeometry[c->cameraID].currFloorHeight != -11000.f) {
        goalHeight = sMarioGeometry[c->cameraID].currFloorHeight + basePos[1] + heightOffset;
    } else {
        goalHeight = gLakituState[c->cameraID].goalPos[1];
    }
    ...
    if (sStatusFlags[c->cameraID] & CAM_FLAG_SMOOTH_MOVEMENT) {
        camera_approach_f32_symmetric_bool(&c->pos[1], goalHeight, 15.f);
    } else {
        if (goalHeight < (&gPlayerCameraState[c->cameraID])->pos[1] - 500.f) {
            goalHeight = (&gPlayerCameraState[c->cameraID])->pos[1] - 500.f;
        }
        c->pos[1] = goalHeight;
    }

    c->pos[0] = basePos[0] + ((&gPlayerCameraState[c->cameraID])->pos[0] - basePos[0]) * scaleToMario;
    c->pos[2] = basePos[2] + ((&gPlayerCameraState[c->cameraID])->pos[2] - basePos[2]) * scaleToMario;
...
s32 update_boss_fight_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    if ((&gPlayerCameraState[c->cameraID])->cameraEvent == CAM_EVENT_BOWSER_JUMP) {
        set_environmental_camera_shake(SHAKE_ENV_BOWSER_JUMP, c);
        (&gPlayerCameraState[c->cameraID])->cameraEvent = 0;
    }
    if ((&gPlayerCameraState[c->cameraID])->cameraEvent == CAM_EVENT_BOWSER_THROW_BOUNCE) {
        set_environmental_camera_shake(SHAKE_ENV_BOWSER_THROW_BOUNCE, c);
        (&gPlayerCameraState[c->cameraID])->cameraEvent = 0;
    }

    yaw = sModeOffsetYaw[c->cameraID] + DEGREES(45);
    ...
    // If no boss is there, just rotate around the area's center point.
        ...
        secondFocus[1] = (&gPlayerCameraState[c->cameraID])->pos[1];
        secondFocus[2] = c->areaCenZ;
        heldState = 0;
    }

    focusDistance = calc_abs_dist((&gPlayerCameraState[c->cameraID])->pos, secondFocus) * 1.6f;
    ...
    if (heldState == 1) {
        offset_rotated(secondFocus, (&gPlayerCameraState[c->cameraID])->pos, holdFocOffset, (&gPlayerCameraState[c->cameraID])->faceAngle);
    }

    // Set the camera focus to the average of Mario and secondFocus
    focus[0] = ((&gPlayerCameraState[c->cameraID])->pos[0] + secondFocus[0]) / 2.f;
    focus[1] = ((&gPlayerCameraState[c->cameraID])->pos[1] + secondFocus[1]) / 2.f + 125.f;
    focus[2] = ((&gPlayerCameraState[c->cameraID])->pos[2] + secondFocus[2]) / 2.f;
    ...
    if (sCSideButtonYaw[c->cameraID] < 0) {
        sModeOffsetYaw[c->cameraID] += 0x200;
    ...
    focus[1] = ((&gPlayerCameraState[c->cameraID])->pos[1] + secondFocus[1]) / 2.f + 100.f;
    if (heldState == 1) {
    ...
    // sLakituPitch[gCamera->cameraID] approaches 33.75 degrees.
    lakitu_zoom(focusDistance, 0x1800);

    // Move the camera position back as sLakituDist[c->cameraID] and sLakituPitch[c->cameraID] increase.
    // This doesn't zoom out of bounds because pos is set above each frame.
    // The constant 0x1000 doubles the pitch from the center when sLakituPitch[c->cameraID] is 0
    // When Lakitu is fully zoomed out, the pitch comes to 0x3800, or 78.75 degrees, up from the focus.
    vec3f_set_dist_and_angle(pos, pos, sLakituDist[c->cameraID], sLakituPitch[c->cameraID] + 0x1000, yaw);
...
/**
 * Parallel tracking mode, the camera faces perpendicular to a line defined by sParTrackPath[c->cameraID]
 *
...
void mode_parallel_tracking_camera(struct Camera *c) {
    ...
    radial_camera_input(c, 0.f);
    set_fov_function(CAM_FOV_DEFAULT, c);
...
void mode_fixed_camera(struct Camera *c) {
    ...
    if (gCurrLevelNum == LEVEL_BBH) {
        set_fov_function(CAM_FOV_BBH, c);
    } else {
        set_fov_function(CAM_FOV_APP_45, c);
    }
    c->nextYaw = update_fixed_camera(c, c->focus, c->pos);
    c->yaw = c->nextYaw;
    pan_ahead_of_player(c);
    vec3f_set(sCastleEntranceOffset[c->cameraID], 0.f, 0.f, 0.f);
}
...
s32 update_behind_mario_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    s16 pitch;
    s16 yaw;
    s16 goalPitch = -(&gPlayerCameraState[c->cameraID])->faceAngle[0];
    s16 marioYaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + DEGREES(180);
    s16 goalYawOff = 0;
    s16 yawSpeed;
    s16 pitchInc = 32;
    UNUSED u8 unused[12];
    ...
    if (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE) {
        maxDist = 350.f;
        focYOff = 120.f;
    }
    if (!((&gPlayerCameraState[c->cameraID])->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER))) {
    ...
    if (sBehindMarioSoundTimer[c->cameraID] != 0) {
        goalPitch = 0;
        camera_approach_s16_symmetric_bool(&sBehindMarioSoundTimer[c->cameraID], 0, 1);
        pitchInc = 0x800;
    }

    if (sBehindMarioSoundTimer[c->cameraID] == 28) {
        if (sCSideButtonYaw[c->cameraID] < 5 || sCSideButtonYaw[c->cameraID] > 28) {
            play_sound_cbutton_up();
        }
    }
    if (sCSideButtonYaw[c->cameraID] == 28) {
        if (sBehindMarioSoundTimer[c->cameraID] < 5 || sBehindMarioSoundTimer[c->cameraID] > 28) {
            play_sound_cbutton_up();
        }
    }
    ...
    // Rotate right
    if (sCButtonsPressed[c->cameraID] & L_CBUTTONS) {
        if (c->controller->buttonPressed & L_CBUTTONS) {
            play_sound_cbutton_side();
        }
        if (dist < maxDist) {
    ...
    // Rotate left
    if (sCButtonsPressed[c->cameraID] & R_CBUTTONS) {
        if (c->controller->buttonPressed & R_CBUTTONS) {
            play_sound_cbutton_side();
        }
        if (dist < maxDist) {
    ...
    // Rotate up
    if (sCButtonsPressed[c->cameraID] & D_CBUTTONS) {
        ...
        goalPitch = -0x3000;
        sBehindMarioSoundTimer[c->cameraID] = 30;
        pitchInc = 0x800;
    }
    // Rotate down
    if (sCButtonsPressed[c->cameraID] & U_CBUTTONS) {
        if (c->controller->buttonPressed & (U_CBUTTONS | D_CBUTTONS)) {
            play_sound_cbutton_side();
        }
        if (dist < maxDist) {
...
s16 update_slide_camera(struct Camera *c) {
    ...
    s16 goalYaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + DEGREES(180);

    // Zoom in when inside the CCM shortcut
    if (sStatusFlags[c->cameraID] & CAM_FLAG_CCM_SLIDE_SHORTCUT) {
        sLakituDist[c->cameraID] = approach_f32(sLakituDist[c->cameraID], -600.f, 20.f, 20.f);
    } else {
        sLakituDist[c->cameraID] = approach_f32(sLakituDist[c->cameraID], 0.f, 20.f, 20.f);
    }
    ...
    // Hoot mode
    if ((&gPlayerCameraState[c->cameraID])->action != ACT_RIDING_HOOT && sMarioGeometry[c->cameraID].currFloorType == SURFACE_DEATH_PLANE) {
        vec3f_set_dist_and_angle(c->focus, pos, maxCamDist + sLakituDist[c->cameraID], camPitch, camYaw);
        c->pos[0] = pos[0];
        c->pos[2] = pos[2];
        ...
        pitchScale = (distCamToFocus - maxCamDist + sLakituDist[c->cameraID]) / 10000.f;
        if (pitchScale > 1.f) {
            pitchScale = 1.f;
        }
    ...
    // Slide mode
        ...
        vec3f_set_dist_and_angle(c->focus, c->pos, maxCamDist + sLakituDist[c->cameraID], camPitch, camYaw);
        sStatusFlags[c->cameraID] |= CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
        ...
        vec3f_get_dist_and_angle(c->focus, c->pos, &distCamToFocus, &camPitch, &camYaw);
        if (distCamToFocus > maxCamDist + sLakituDist[c->cameraID]) {
            distCamToFocus = maxCamDist + sLakituDist[c->cameraID];
            vec3f_set_dist_and_angle(c->focus, c->pos, distCamToFocus, camPitch, camYaw);
        }
    }
...
s32 update_mario_camera(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    s16 yaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + sModeOffsetYaw[c->cameraID] + DEGREES(180);
    focus_on_mario(focus, pos, 125.f, 125.f, gCameraZoomDist[c->cameraID], 0x05B0, yaw);

    return (&gPlayerCameraState[c->cameraID])->faceAngle[1];
}
...
s16 update_default_camera(struct Camera *c) {
    ...
    s16 pitch;
    s16 yaw;
    s16 yawGoal = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + DEGREES(180);
    f32 posHeight;
    f32 focHeight;
    f32 distFromWater;
    s16 tempPitch;
    ...
    s32 avoidStatus = 0;
    s32 closeToMario = 0;
    f32 ceilHeight = find_ceil(gLakituState[c->cameraID].goalPos[0],
                               gLakituState[c->cameraID].goalPos[1],
                               gLakituState[c->cameraID].goalPos[2], &ceil);
    s16 yawDir;

    handle_c_button_movement(c);
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, &dist, &pitch, &yaw);
    ...
    if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT) {
        ...
        if (set_cam_angle(0, c) == CAM_ANGLE_MARIO) {
    ...
    if (((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_HANGING) ||
        (&gPlayerCameraState[c->cameraID])->action == ACT_RIDING_HOOT) {
        zoomDist *= 0.8f;
        set_handheld_shake(HAND_CAM_SHAKE_HANG_OWL, c);
    }

    // If not zooming out, only allow dist to decrease
    if (sZoomAmount[c->cameraID] == 0.f) {
        if (dist > zoomDist) {
        ...
        if ((sZoomAmount[c->cameraID] -= 30.f) < 0.f) {
            sZoomAmount[c->cameraID] = 0.f;
        }
        if (dist > zoomDist) {
    ...
    if (sCSideButtonYaw[c->cameraID] == 0) {
        ...
        camera_approach_s16_symmetric_bool(&sCSideButtonYaw[c->cameraID], 0, 0x100);
        nextYawVel = 0;
    }
    sYawSpeed[c->cameraID] = 0x400;
    xzDist = calc_hor_dist((&gPlayerCameraState[c->cameraID])->pos, c->pos);

    if (sStatusFlags[c->cameraID] & CAM_FLAG_BEHIND_MARIO_POST_DOOR) {
        if (xzDist >= 250) {
            sStatusFlags[c->cameraID] &= ~CAM_FLAG_BEHIND_MARIO_POST_DOOR;
        }
        if (ABS(((&gPlayerCameraState[c->cameraID])->faceAngle[1] - yaw) / 2) < 0x1800) {
            sStatusFlags[c->cameraID] &= ~CAM_FLAG_BEHIND_MARIO_POST_DOOR;
            yaw = sCameraYawAfterDoorCutscene[c->cameraID] + DEGREES(180);
            dist = 800.f;
            sStatusFlags[c->cameraID] |= CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
        }
    } else if (xzDist < 250) {
    ...
    if (avoidStatus == 3) {
        ...
        sAvoidYawVel[c->cameraID] = yaw;
        sStatusFlags[c->cameraID] |= CAM_FLAG_COLLIDED_WITH_WALL;
        //! Does nothing
        vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, cPos, &xzDist, &tempPitch, &tempYaw);
        // Rotate to avoid the wall
        approach_s16_asymptotic_bool(&yaw, avoidYaw, 10);
        //! Does nothing
        vec3f_set_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, cPos, xzDist, tempPitch, tempYaw);
        sAvoidYawVel[c->cameraID] = (sAvoidYawVel[c->cameraID] - yaw) / 0x100;
    } else {
        if (gMarioStates[0].forwardVel == 0.f) {
            if (sStatusFlags[c->cameraID] & CAM_FLAG_COLLIDED_WITH_WALL) {
                if ((yawGoal - yaw) / 0x100 >= 0) {
                    yawDir = -1;
                } else {
                    yawDir = 1;
                }
                if ((sAvoidYawVel[c->cameraID] > 0 && yawDir > 0) || (sAvoidYawVel[c->cameraID] < 0 && yawDir < 0)) {
                    yawVel = nextYawVel;
                }
            } else {
                yawVel = nextYawVel;
    ...
    if (avoidStatus == 0 && !(sStatusFlags[c->cameraID] & CAM_FLAG_COLLIDED_WITH_WALL)) {
        approach_f32_asymptotic_bool(&dist, zoomDist - 100.f, 0.05f);
    }
    vec3f_set_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, cPos, dist, pitch, yaw);
    ...
    c->focus[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    c->focus[1] = (&gPlayerCameraState[c->cameraID])->pos[1] + 125.f + focHeight;
    c->focus[2] = (&gPlayerCameraState[c->cameraID])->pos[2];

    marioFloorHeight = 125.f + sMarioGeometry[c->cameraID].currFloorHeight;
    marioFloor = sMarioGeometry[c->cameraID].currFloor;
    camFloorHeight = find_floor(cPos[0], cPos[1] + 50.f, cPos[2], &cFloor) + 125.f;
    for (scale = 0.1f; scale < 1.f; scale += 0.2f) {
        scale_along_line(tempPos, cPos, (&gPlayerCameraState[c->cameraID])->pos, scale);
    ...
    if (waterHeight != -11000.f) {
        ...
        if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_METAL_BELOW_WATER)) {
            if (distFromWater > 800.f && ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_METAL_WATER)) {
                gCameraMovementFlags[c->cameraID] |= CAM_MOVE_METAL_BELOW_WATER;
            }
        } else {
            if (distFromWater < 400.f || !((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_METAL_WATER)) {
                gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_METAL_BELOW_WATER;
            }
        }
        // If not wearing the metal cap, always stay above
        if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_METAL_BELOW_WATER) && camFloorHeight < waterHeight) {
    ...
    if ((gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT) && (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE)) {
        posHeight = 610.f;
    ...
    if ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_HANGING || (&gPlayerCameraState[c->cameraID])->action == ACT_RIDING_HOOT) {
        camFloorHeight = (&gPlayerCameraState[c->cameraID])->pos[1] + 400.f;
        if (c->mode == CAMERA_MODE_FREE_ROAM) {
    ...
    if ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_ON_POLE) {
        camFloorHeight = gMarioStates[0].usedObj->oPosY + 125.f;
        if ((&gPlayerCameraState[c->cameraID])->pos[1] - 100.f > camFloorHeight) {
            camFloorHeight = (&gPlayerCameraState[c->cameraID])->pos[1] - 100.f;
        }
        ceilHeight = 20000.f;
        vec3f_copy(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
    }
    ...
    c->pos[0] = cPos[0];
    c->pos[2] = cPos[2];
    cPos[0] = gLakituState[c->cameraID].goalPos[0];
    cPos[1] = c->pos[1];
    cPos[2] = gLakituState[c->cameraID].goalPos[2];
    ...
    if (ceilHeight != 20000.f) {
        if (c->pos[1] > (ceilHeight -= 150.f)
            && (avoidStatus = is_range_behind_surface(c->pos, (&gPlayerCameraState[c->cameraID])->pos, ceil, 0, -1)) == 1) {
            c->pos[1] = ceilHeight;
        }
    }
...
s32 update_spiral_stairs_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    vec3f_set(sFixedModeBasePosition[c->cameraID], -1280.f, 614.f, 1740.f);
    ...
    focus[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    focY = (&gPlayerCameraState[c->cameraID])->pos[1] + 125.f + focusHeight;
    focus[2] = (&gPlayerCameraState[c->cameraID])->pos[2];

    vec3f_copy(cPos, pos);
    vec3f_get_dist_and_angle(sFixedModeBasePosition[c->cameraID], focus, &dist, &focPitch, &focYaw);
    vec3f_get_dist_and_angle(sFixedModeBasePosition[c->cameraID], cPos, &dist, &posPitch, &posYaw);

    sSpiralStairsYawOffset[c->cameraID] = posYaw - focYaw;
    // posYaw will change if Mario is more than 90 degrees around the stairs, relative to the camera
    if (sSpiralStairsYawOffset[c->cameraID] < DEGREES(-90)) {
        sSpiralStairsYawOffset[c->cameraID] = DEGREES(-90);
    }
    if (sSpiralStairsYawOffset[c->cameraID] > DEGREES(90)) {
        sSpiralStairsYawOffset[c->cameraID] = DEGREES(90);
    }
    focYaw += sSpiralStairsYawOffset[c->cameraID];
    posYaw = focYaw;
    //! @bug unnecessary
    camera_approach_s16_symmetric_bool(&posYaw, focYaw, 0x1000);

    vec3f_set_dist_and_angle(sFixedModeBasePosition[c->cameraID], cPos, 300.f, 0, posYaw);
    ...
    if (floorHeight != -11000.f) {
        if (floorHeight < sMarioGeometry[c->cameraID].currFloorHeight) {
            floorHeight = sMarioGeometry[c->cameraID].currFloorHeight;
        }
...
void mode_slide_camera(struct Camera *c) {
    if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_CLOSE_CAMERA ||
        sMarioGeometry[c->cameraID].currFloorType == SURFACE_NO_CAM_COL_SLIPPERY) {
...
void store_lakitu_cam_info_for_c_up(struct Camera *c) {
    vec3f_copy(sCameraStoreCUp[c->cameraID].pos, c->pos);
    vec3f_sub(sCameraStoreCUp[c->cameraID].pos, (&gPlayerCameraState[c->cameraID])->pos);
    // Only store the y value, and as an offset from Mario, for some reason
    vec3f_set(sCameraStoreCUp[c->cameraID].focus, 0.f, c->focus[1] - (&gPlayerCameraState[c->cameraID])->pos[1], 0.f);
}
...
s32 set_mode_c_up(struct Camera *c) {
    if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_C_UP_MODE)) {
        ...
        sCameraSoundFlags[c->cameraID] &= ~CAM_SOUND_C_UP_PLAYED;
        return 1;
    }
    return 0;
}
...
s32 exit_c_up(struct Camera *c) {
    ...
    if ((gCameraMovementFlags[c->cameraID] & CAM_MOVE_C_UP_MODE) && !(gCameraMovementFlags[c->cameraID] & CAM_MOVE_STARTED_EXITING_C_UP)) {
        // Copy the stored pos and focus. This is unused.
        vec3f_copy(storePos, sCameraStoreCUp[c->cameraID].pos);
        vec3f_add(storePos, (&gPlayerCameraState[c->cameraID])->pos);
        vec3f_copy(storeFoc, sCameraStoreCUp[c->cameraID].focus);
        vec3f_add(storeFoc, (&gPlayerCameraState[c->cameraID])->pos);

        vec3f_copy(checkFoc, c->focus);
        checkFoc[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
        checkFoc[2] = (&gPlayerCameraState[c->cameraID])->pos[2];
        ...
        if (sModeInfo[c->cameraID].lastMode == CAMERA_MODE_SPIRAL_STAIRS || sModeInfo[c->cameraID].lastMode == CAMERA_MODE_CLOSE
            || sModeInfo[c->cameraID].lastMode == CAMERA_MODE_FREE_ROAM) {
            searching = 1;
            ...
            for (sector = 0; sector < 16 && searching == 1; sector++) {
                ...
                if (f32_find_wall_collision(&curPos[0], &curPos[1], &curPos[2], 20.f, 50.f) == 0) {
                    ...
                    // zoomed out distance
                    for (d = curDist; d < gCameraZoomDist[c->cameraID]; d += 20.f) {
                    ...
                    if (d >= gCameraZoomDist[c->cameraID]) {
                        searching = 0;
                    }
                }
            ...
            if (searching == 0) {
                vec3f_set_dist_and_angle(checkFoc, sCameraStoreCUp[c->cameraID].pos, gCameraZoomDist[c->cameraID], 0, curYaw + checkYaw);
                vec3f_copy(sCameraStoreCUp[c->cameraID].focus, checkFoc);
                vec3f_sub(sCameraStoreCUp[c->cameraID].pos, (&gPlayerCameraState[c->cameraID])->pos);
                vec3f_sub(sCameraStoreCUp[c->cameraID].focus, (&gPlayerCameraState[c->cameraID])->pos);
            }

            gCameraMovementFlags[c->cameraID] |= CAM_MOVE_STARTED_EXITING_C_UP;
            transition_next_state(c, 15);
        } else {
            // Let the next camera mode handle it
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_STARTED_EXITING_C_UP | CAM_MOVE_C_UP_MODE);
...
s32 update_c_up(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    s16 pitch = sCUpCameraPitch[c->cameraID];
    s16 yaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + sModeOffsetYaw[c->cameraID] + DEGREES(180);
 ...
 */
void move_mario_head_c_up(struct Camera *c) {
    UNUSED s16 pitch = sCUpCameraPitch[c->cameraID];
    UNUSED s16 yaw = sModeOffsetYaw[c->cameraID];

    sCUpCameraPitch[c->cameraID] += (s16)(c->controller->stickY * 10.f);
    sModeOffsetYaw[c->cameraID] -= (s16)(c->controller->stickX * 10.f);

    // Bound looking up to nearly 80 degrees.
    if (sCUpCameraPitch[c->cameraID] > 0x38E3) {
        sCUpCameraPitch[c->cameraID] = 0x38E3;
    }
    // Bound looking down to -45 degrees
    if (sCUpCameraPitch[c->cameraID] < -0x2000) {
        sCUpCameraPitch[c->cameraID] = -0x2000;
    }

    // Bound the camera yaw to +-120 degrees
    if (sModeOffsetYaw[c->cameraID] > 0x5555) {
        sModeOffsetYaw[c->cameraID] = 0x5555;
    }
    if (sModeOffsetYaw[c->cameraID] < -0x5555) {
        sModeOffsetYaw[c->cameraID] = -0x5555;
    }
    ...
    (&gPlayerCameraState[c->cameraID])->headRotation[0] = sCUpCameraPitch[c->cameraID] * 3 / 4;
    (&gPlayerCameraState[c->cameraID])->headRotation[1] = sModeOffsetYaw[c->cameraID] * 3 / 4;
}
...
void move_into_c_up(struct Camera *c) {
    struct LinearTransitionPoint *start = &sModeInfo[c->cameraID].transitionStart;
    struct LinearTransitionPoint *end = &sModeInfo[c->cameraID].transitionEnd;
    ...
    dist  = start->dist  + dist  * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;
    pitch = start->pitch + pitch * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;
    yaw   = start->yaw   + yaw   * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;

    // Linearly interpolate the focus from start to end
    c->focus[0] = start->focus[0] + (end->focus[0] - start->focus[0]) * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;
    c->focus[1] = start->focus[1] + (end->focus[1] - start->focus[1]) * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;
    c->focus[2] = start->focus[2] + (end->focus[2] - start->focus[2]) * sModeInfo[c->cameraID].frame / sModeInfo[c->cameraID].max;

    vec3f_add(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
    vec3f_set_dist_and_angle(c->focus, c->pos, dist, pitch, yaw);

    (&gPlayerCameraState[c->cameraID])->headRotation[0] = 0;
    (&gPlayerCameraState[c->cameraID])->headRotation[1] = 0;

    // Finished zooming in
    if (++sModeInfo[c->cameraID].frame == sModeInfo[c->cameraID].max) {
        gCameraMovementFlags[c->cameraID] &= ~CAM_MOVING_INTO_MODE;
    }
}
...
s32 mode_c_up_camera(struct Camera *c) {
    ...
    if (!(sCameraSoundFlags[c->cameraID] & CAM_SOUND_C_UP_PLAYED)) {
        play_sound_cbutton_up();
        sCameraSoundFlags[c->cameraID] |= CAM_SOUND_C_UP_PLAYED;
    }
    ...
    if (!(gCameraMovementFlags[c->cameraID] & CAM_MOVE_STARTED_EXITING_C_UP)) {
        // Normal update
        move_mario_head_c_up(c);
        update_c_up(c, c->focus, c->pos);
    } else {
        // Exiting C-Up
        if (sStatusFlags[c->cameraID] & CAM_FLAG_TRANSITION_OUT_OF_C_UP) {
            // Retrieve the previous position and focus
            vec3f_copy(c->pos, sCameraStoreCUp[c->cameraID].pos);
            vec3f_add(c->pos, (&gPlayerCameraState[c->cameraID])->pos);
            vec3f_copy(c->focus, sCameraStoreCUp[c->cameraID].focus);
            vec3f_add(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
            // Make Mario look forward
            camera_approach_s16_symmetric_bool(&(&gPlayerCameraState[c->cameraID])->headRotation[0], 0, 1024);
            camera_approach_s16_symmetric_bool(&(&gPlayerCameraState[c->cameraID])->headRotation[1], 0, 1024);
        } else {
            // Finished exiting C-Up
            gCameraMovementFlags[c->cameraID] &= ~(CAM_MOVE_STARTED_EXITING_C_UP | CAM_MOVE_C_UP_MODE);
        }
    }
    sPanDistance[c->cameraID] = 0.f;
...
s32 update_in_cannon(struct Camera *c, Vec3f focus, Vec3f pos) {
    focus_on_mario(pos, focus, 125.f + sCannonYOffset[c->cameraID], 125.f, 800.f,
                                    (&gPlayerCameraState[c->cameraID])->faceAngle[0], (&gPlayerCameraState[c->cameraID])->faceAngle[1]);
    return (&gPlayerCameraState[c->cameraID])->faceAngle[1];
}

/**
 * Updates the camera when Mario is in a cannon.
 * sCannonYOffset[c->cameraID] is used to make the camera rotate down when Mario has just entered the cannon
 */
void mode_cannon_camera(struct Camera *c) {
    UNUSED u8 unused[24];

    sLakituPitch[c->cameraID] = 0;
    gCameraMovementFlags[c->cameraID] &= ~CAM_MOVING_INTO_MODE;
    ...
    if (c->controller->buttonPressed & A_BUTTON) {
        ...
        sPanDistance[c->cameraID] = 0.f;
        sCannonYOffset[c->cameraID] = 0.f;
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
    } else {
        sCannonYOffset[c->cameraID] = approach_f32(sCannonYOffset[c->cameraID], 0.f, 100.f, 100.f);
    }
}
...
void transition_next_state(struct Camera *c, s16 frames) {
    if (!(sStatusFlags[c->cameraID] & CAM_FLAG_FRAME_AFTER_CAM_INIT)) {
        sStatusFlags[c->cameraID] |= (CAM_FLAG_START_TRANSITION | CAM_FLAG_TRANSITION_OUT_OF_C_UP);
        sModeTransition[c->cameraID].framesLeft = frames;
    }
}
...
void transition_to_camera_mode(struct Camera *c, s16 newMode, s16 numFrames) {
    if (c->mode != newMode) {
        sModeInfo[c->cameraID].newMode = (newMode != -1) ? newMode : sModeInfo[c->cameraID].lastMode;
        sModeInfo[c->cameraID].lastMode = c->mode;
        c->mode = sModeInfo[c->cameraID].newMode;

        // Clear movement flags that would affect the transition
        gCameraMovementFlags[c->cameraID] &= (u16)~(CAM_MOVE_RESTRICT | CAM_MOVE_ROTATE);
        if (!(sStatusFlags[c->cameraID] & CAM_FLAG_FRAME_AFTER_CAM_INIT)) {
            transition_next_state(c, numFrames);
            sCUpCameraPitch[c->cameraID] = 0;
            sModeOffsetYaw[c->cameraID] = 0;
            sLakituDist[c->cameraID] = 0;
            sLakituPitch[c->cameraID] = 0;
            sAreaYawChange[c->cameraID] = 0;
            sPanDistance[c->cameraID] = 0.f;
            sCannonYOffset[c->cameraID] = 0.f;
        }
    }
 ...
 *
 * Stores the current pos and focus in sModeInfo[c->cameraID]->transitionStart, and
 * stores the next pos and focus into sModeInfo[c->cameraID]->transitionEnd. These two fields are used in
 * move_into_c_up().
 *
...
void set_camera_mode(struct Camera *c, s16 mode, s16 frames) {
    ...
    if (mode == CAMERA_MODE_WATER_SURFACE && gCurrLevelArea == AREA_TTM_OUTSIDE) {
        ...
        // Clear movement flags that would affect the transition
        gCameraMovementFlags[c->cameraID] &= (u16)~(CAM_MOVE_RESTRICT | CAM_MOVE_ROTATE);
        // gCameraMovementFlags[c->cameraID] |= CAM_MOVING_INTO_MODE // TAKING THIS OUT FIXES THE C UP BUG
        ...
        sModeInfo[c->cameraID].newMode = (mode != -1) ? mode : sModeInfo[c->cameraID].lastMode;
        sModeInfo[c->cameraID].lastMode = c->mode;
        sModeInfo[c->cameraID].max = frames;
        sModeInfo[c->cameraID].frame = 1;

        c->mode = sModeInfo[c->cameraID].newMode;
        gLakituState[c->cameraID].mode = c->mode;

        vec3f_copy(end->focus, c->focus);
        vec3f_sub(end->focus, (&gPlayerCameraState[c->cameraID])->pos);

        vec3f_copy(end->pos, c->pos);
        vec3f_sub(end->pos, (&gPlayerCameraState[c->cameraID])->pos);

        sAreaYaw[c->cameraID] = sModeTransitions[sModeInfo[c->cameraID].newMode](c, end->focus, end->pos);

        // End was updated by sModeTransitions
        vec3f_sub(end->focus, (&gPlayerCameraState[c->cameraID])->pos);
        vec3f_sub(end->pos, (&gPlayerCameraState[c->cameraID])->pos);

        vec3f_copy(start->focus, gLakituState[c->cameraID].curFocus);
        vec3f_sub(start->focus, (&gPlayerCameraState[c->cameraID])->pos);

        vec3f_copy(start->pos, gLakituState[c->cameraID].curPos);
        vec3f_sub(start->pos, (&gPlayerCameraState[c->cameraID])->pos);
...
void update_lakitu(struct Camera *c) {
    ...
    if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_PAUSE_SCREEN) {
        ...
        if (TRUE) {
            newYaw = next_lakitu_state(newPos, newFoc, c->pos, c->focus, sOldPosition[c->cameraID], sOldFocus[c->cameraID],
                                       c->nextYaw, c);
            set_or_approach_s16_symmetric(&c->yaw, newYaw, sYawSpeed[c->cameraID], c);
            sStatusFlags[c->cameraID] &= ~CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
        } else {
        ...
        // Update old state
        vec3f_copy(sOldPosition[c->cameraID], newPos);
        vec3f_copy(sOldFocus[c->cameraID], newFoc);

        gLakituState[c->cameraID].yaw = c->yaw;
        gLakituState[c->cameraID].nextYaw = c->nextYaw;
        vec3f_copy(gLakituState[c->cameraID].goalPos, c->pos);
        vec3f_copy(gLakituState[c->cameraID].goalFocus, c->focus);

        // Simulate Lakitu flying to the new position and turning towards the new focus
        set_or_approach_vec3f_asymptotic(gLakituState[c->cameraID].curPos, newPos,
                                         gLakituState[c->cameraID].posHSpeed, gLakituState[c->cameraID].posVSpeed,
                                         gLakituState[c->cameraID].posHSpeed, c);
        set_or_approach_vec3f_asymptotic(gLakituState[c->cameraID].curFocus, newFoc,
                                         gLakituState[c->cameraID].focHSpeed, gLakituState[c->cameraID].focVSpeed,
                                         gLakituState[c->cameraID].focHSpeed, c);
        // Adjust Lakitu's speed back to normal
        set_or_approach_f32_asymptotic(&gLakituState[c->cameraID].focHSpeed, 0.8f, 0.05f, c);
        set_or_approach_f32_asymptotic(&gLakituState[c->cameraID].focVSpeed, 0.3f, 0.05f, c);
        set_or_approach_f32_asymptotic(&gLakituState[c->cameraID].posHSpeed, 0.3f, 0.05f, c);
        set_or_approach_f32_asymptotic(&gLakituState[c->cameraID].posVSpeed, 0.3f, 0.05f, c);

        // Turn on smooth movement when it hasn't been blocked for 2 frames
        if (sStatusFlags[c->cameraID] & CAM_FLAG_BLOCK_SMOOTH_MOVEMENT) {
            sStatusFlags[c->cameraID] &= ~CAM_FLAG_BLOCK_SMOOTH_MOVEMENT;
        } else {
            sStatusFlags[c->cameraID] |= CAM_FLAG_SMOOTH_MOVEMENT;
        }
        // debug point
        // debugstub();
        vec3f_copy(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].curPos); // this makes the game crash, somehow unless is et
                                                     // both to 0 but then it doesnt render well either
        vec3f_copy(gLakituState[c->cameraID].focus, gLakituState[c->cameraID].curFocus);

        if (c->cutscene) {
            vec3f_add(gLakituState[c->cameraID].focus, sPlayer2FocusOffset[c->cameraID]);
            vec3f_set(sPlayer2FocusOffset[c->cameraID], 0, 0, 0);
        }

        vec3f_get_dist_and_angle(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].focus, &gLakituState[c->cameraID].focusDistance,
                                 &gLakituState[c->cameraID].oldPitch, &gLakituState[c->cameraID].oldYaw);

        gLakituState[c->cameraID].roll = 0;

        // Apply camera shakes
        shake_camera_pitch(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].focus);
        shake_camera_yaw(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].focus);
        shake_camera_roll(&gLakituState[c->cameraID].roll);
        shake_camera_handheld(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].focus, c);

        if ((&gPlayerCameraState[c->cameraID])->action == ACT_DIVE && gLakituState[c->cameraID].lastFrameAction != ACT_DIVE) {
            set_camera_shake_from_hit(SHAKE_HIT_FROM_BELOW, c);
        }

        gLakituState[c->cameraID].roll += sHandheldShakeRoll[c->cameraID];
        gLakituState[c->cameraID].roll += gLakituState[c->cameraID].keyDanceRoll;

        if (c->mode != CAMERA_MODE_C_UP && c->cutscene == 0) {
            gCheckingSurfaceCollisionsForCamera = TRUE;
            distToFloor = find_floor(gLakituState[c->cameraID].pos[0],
                                     gLakituState[c->cameraID].pos[1] + 20.0f,
                                     gLakituState[c->cameraID].pos[2], &floor);
            if (distToFloor != -11000.f) {
                if (gLakituState[c->cameraID].pos[1] < (distToFloor += 100.0f)) {
                    gLakituState[c->cameraID].pos[1] = distToFloor;
                } else {
        ...
        vec3f_copy(sModeTransition[c->cameraID].marioPos, (&gPlayerCameraState[c->cameraID])->pos);
    }
    clamp_pitch(gLakituState[c->cameraID].pos, gLakituState[c->cameraID].focus, 0x3E00, -0x3E00);
    gLakituState[c->cameraID].mode = c->mode;
    gLakituState[c->cameraID].defMode = c->defMode;
}
...
void update_camera(struct Camera *c) {
    ...
    gCamera = c;
    gLakituState[c->cameraID].cameraID = c->cameraID;
    update_camera_hud_status(c);
    if (c->cutscene == 0) {
        // Only process R_TRIG if 'fixed' is not selected in the menu
        if (cam_select_alt_mode(0, c) == CAM_SELECTION_MARIO) {
            if (c->controller->buttonPressed & R_TRIG) {
                if (set_cam_angle(0, c) == CAM_ANGLE_LAKITU) {
                    set_cam_angle(CAM_ANGLE_MARIO, c);
                } else {
                    set_cam_angle(CAM_ANGLE_LAKITU, c);
                }
            }
        }
        play_sound_if_cam_switched_to_lakitu_or_mario(c);
    }

    // Initialize the camera
    sStatusFlags[c->cameraID] &= ~CAM_FLAG_FRAME_AFTER_CAM_INIT;
    ...
    sMarioGeometry[c->cameraID].prevFloorHeight = sMarioGeometry[c->cameraID].currFloorHeight;
    sMarioGeometry[c->cameraID].prevCeilHeight = sMarioGeometry[c->cameraID].currCeilHeight;
    sMarioGeometry[c->cameraID].prevFloor = sMarioGeometry[c->cameraID].currFloor;
    sMarioGeometry[c->cameraID].prevCeil = sMarioGeometry[c->cameraID].currCeil;
    sMarioGeometry[c->cameraID].prevFloorType = sMarioGeometry[c->cameraID].currFloorType;
    sMarioGeometry[c->cameraID].prevCeilType = sMarioGeometry[c->cameraID].currCeilType;

    find_mario_floor_and_ceil(&sMarioGeometry[c->cameraID], c);
    gCheckingSurfaceCollisionsForCamera = TRUE;
    vec3f_copy(c->pos, gLakituState[c->cameraID].goalPos);
    vec3f_copy(c->focus, gLakituState[c->cameraID].goalFocus);

    c->yaw = gLakituState[c->cameraID].yaw;
    c->nextYaw = gLakituState[c->cameraID].nextYaw;
    c->mode = gLakituState[c->cameraID].mode;
    c->defMode = gLakituState[c->cameraID].defMode;

    camera_course_processing(c);
    sCButtonsPressed[c->cameraID] = find_c_buttons_pressed(sCButtonsPressed[c->cameraID], c->controller->buttonPressed,
                                              c->controller->buttonDown);

    if (c->cutscene != 0) {
        sYawSpeed[c->cameraID] = 0;
        play_cutscene(c);
        sFramesSinceCutsceneEnded[c->cameraID] = 0;
    } else {
        // Clear the recent cutscene after 8 frames
        if (gRecentCutscene[c->cameraID] != 0 && sFramesSinceCutsceneEnded[c->cameraID] < 8) {
            sFramesSinceCutsceneEnded[c->cameraID]++;
            if (sFramesSinceCutsceneEnded[c->cameraID] >= 8) {
                gRecentCutscene[c->cameraID] = 0;
                sFramesSinceCutsceneEnded[c->cameraID] = 0;
            }
        }
    }
    ...
    if (c->cutscene == 0) { // the crash happens AS SOON AS THIS IS TRUE
        ...
        if (gMarioStates[c->cameraID].action
            ...
            mode_8_directions_camera(c); // passing c crashes? stub doesnt, but empty update does
        } else if (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE) {
            switch (c->mode) {
    ...
    if (gCurrLevelNum != LEVEL_CASTLE) {
        ...
        // trigger is held
        if ((c->cutscene == 0 &&
            (c->controller->buttonDown & R_TRIG) && cam_select_alt_mode(0, c) == CAM_SELECTION_FIXED)
            || (gCameraMovementFlags[c->cameraID] & CAM_MOVE_FIX_IN_PLACE)
            || ((&gPlayerCameraState[c->cameraID])->action) == ACT_GETTING_BLOWN) {

            // If this is the first frame that R_TRIG is held, play the "click" sound
            if (c->cutscene == 0 && (c->controller->buttonPressed & R_TRIG)
                && cam_select_alt_mode(0, c) == CAM_SELECTION_FIXED) {
                sCameraSoundFlags[c->cameraID] |= CAM_SOUND_FIXED_ACTIVE;
            ...
            gLakituState[c->cameraID].posHSpeed = 0.f;
            gLakituState[c->cameraID].posVSpeed = 0.f;

            c->nextYaw = calculate_yaw(gLakituState[c->cameraID].focus, gLakituState[c->cameraID].pos);
            c->yaw = c->nextYaw;
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_FIX_IN_PLACE;
        } else {
            // Play the "click" sound when fixed mode is released
            if (sCameraSoundFlags[c->cameraID] & CAM_SOUND_FIXED_ACTIVE) {
                play_sound_rbutton_changed();
                sCameraSoundFlags[c->cameraID] &= ~CAM_SOUND_FIXED_ACTIVE;
            }
        }
    } else {
        if ((c->controller->buttonPressed & R_TRIG) && cam_select_alt_mode(0, c) == CAM_SELECTION_FIXED) {
    ...
    gLakituState[c->cameraID].lastFrameAction = (&gPlayerCameraState[c->cameraID])->action;
}
...
void reset_camera(struct Camera *c) {
    ...
    UNUSED struct LinearTransitionPoint *start = &sModeInfo[c->cameraID].transitionStart;
    UNUSED struct LinearTransitionPoint *end = &sModeInfo[c->cameraID].transitionEnd;

    gCamera = c;
    gCameraMovementFlags[c->cameraID] = 0;
    s2ndRotateFlags[c->cameraID] = 0;
    sStatusFlags[c->cameraID] = 0;
    gCutsceneTimer[c->cameraID] = 0;
    sCutsceneShot[c->cameraID] = 0;
    gCutsceneObjSpawn[c->cameraID] = 0;
    gObjCutsceneDone[c->cameraID] = FALSE;
    gCutsceneFocus[c->cameraID] = NULL;
//    unused8032CFC8 = 0;
//    unused8032CFCC = 0;
    gSecondCameraFocus[c->cameraID] = NULL;
    sCButtonsPressed[c->cameraID] = 0;
    vec3f_copy(sModeTransition[c->cameraID].marioPos, (&gPlayerCameraState[c->cameraID])->pos);
    sModeTransition[c->cameraID].framesLeft = 0;
//    unused8032CFCC = -1;
//    unused8032CFC8 = -1;
    gCameraMovementFlags[c->cameraID] = 0;
    gCameraMovementFlags[c->cameraID] |= CAM_MOVE_INIT_CAMERA;
//    unused8033B316 = 0;
    sStatusFlags[c->cameraID] = 0;
//    unused8033B31A = 0;
    sCameraSoundFlags[c->cameraID] = 0;
    sCUpCameraPitch[c->cameraID] = 0;
    sModeOffsetYaw[c->cameraID] = 0;
    sSpiralStairsYawOffset[c->cameraID] = 0;
    sLakituDist[c->cameraID] = 0;
    sLakituPitch[c->cameraID] = 0;
    sAreaYaw[c->cameraID] = 0;
    sAreaYawChange[c->cameraID] = 0.f;
    sPanDistance[c->cameraID] = 0.f;
    sCannonYOffset[c->cameraID] = 0.f;
    sZoomAmount[c->cameraID] = 0.f;
    sZeroZoomDist[c->cameraID] = 0.f;
    sBehindMarioSoundTimer[c->cameraID] = 0;
    sCSideButtonYaw[c->cameraID] = 0;
    s8DirModeBaseYaw[c->cameraID] = 0;
    s8DirModeYawOffset[c->cameraID] = 0;
    ...
    (&gPlayerCameraState[c->cameraID])->cameraEvent = 0;
    (&gPlayerCameraState[c->cameraID])->usedObj = NULL;
    gLakituState[c->cameraID].shakeMagnitude[0] = 0;
    gLakituState[c->cameraID].shakeMagnitude[1] = 0;
    gLakituState[c->cameraID].shakeMagnitude[2] = 0;
    gLakituState[c->cameraID].unusedVec2[0] = 0;
    gLakituState[c->cameraID].unusedVec2[1] = 0;
    gLakituState[c->cameraID].unusedVec2[2] = 0;
    gLakituState[c->cameraID].unusedVec1[0] = 0.f;
    gLakituState[c->cameraID].unusedVec1[1] = 0.f;
    gLakituState[c->cameraID].unusedVec1[2] = 0.f;
    gLakituState[c->cameraID].lastFrameAction = 0;
    set_fov_function(CAM_FOV_DEFAULT, c);
    sFOVState[c->cameraID].fov = 45.f;
    sFOVState[c->cameraID].fovOffset = 0.f;
    sFOVState[c->cameraID].unusedIsSleeping = 0;
    sFOVState[c->cameraID].shakeAmplitude = 0.f;
    sFOVState[c->cameraID].shakePhase = 0;
    sObjectCutscene[c->cameraID] = 0;
    gRecentCutscene[c->cameraID] = 0;
//    unused8033B30C = 0;
//    unused8033B310 = 0;
}
...
void init_camera(struct Camera *c) {
    ...
    sCreditsPlayer2Pitch[c->cameraID] = 0;
    sCreditsPlayer2Yaw[c->cameraID] = 0;
    gPrevLevel = gCurrLevelArea / 16;
    gCurrLevelArea = gCurrLevelNum * 16 + gCurrentArea->index;
    sSelectionFlags[c->cameraID] &= CAM_MODE_MARIO_SELECTED;
    sFramesPaused = 0;
    gLakituState[c->cameraID].mode = c->mode;
    gLakituState[c->cameraID].defMode = c->defMode;
    gLakituState[c->cameraID].posHSpeed = 0.3f;
    gLakituState[c->cameraID].posVSpeed = 0.3f;
    gLakituState[c->cameraID].focHSpeed = 0.8f;
    gLakituState[c->cameraID].focHSpeed = 0.3f; // @bug set focHSpeed back-to-back
    gLakituState[c->cameraID].roll = 0;
    gLakituState[c->cameraID].keyDanceRoll = 0;
    gLakituState[c->cameraID].unused = 0;
    sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
    vec3f_set(sCastleEntranceOffset[c->cameraID], 0.f, 0.f, 0.f);
    vec3f_set(sPlayer2FocusOffset[c->cameraID], 0.f, 0.f, 0.f);
    find_mario_floor_and_ceil(&sMarioGeometry[c->cameraID], c);
    sMarioGeometry[c->cameraID].prevFloorHeight = sMarioGeometry[c->cameraID].currFloorHeight;
    sMarioGeometry[c->cameraID].prevCeilHeight = sMarioGeometry[c->cameraID].currCeilHeight;
    sMarioGeometry[c->cameraID].prevFloor = sMarioGeometry[c->cameraID].currFloor;
    ...
    sMarioGeometry[c->cameraID].prevCeilType = sMarioGeometry[c->cameraID].currCeilType;
    for (i = 0; i < 32; i++) {
        sCurCreditsSplinePos[c->cameraID][i].index = -1;
        sCurCreditsSplineFocus[c->cameraID][i].index = -1;
    }
    sCutsceneSplineSegment[c->cameraID] = 0;
    sCutsceneSplineSegmentProgress[c->cameraID] = 0.f;
//    unused8033B6E8 = 0;
    sHandheldShakeInc[c->cameraID] = 0.f;
    sHandheldShakeTimer[c->cameraID] = 0.f;
    sHandheldShakeMag[c->cameraID] = 0;
    for (i = 0; i < 4; i++) {
        sHandheldShakeSpline[i][c->cameraID].index = -1;
    }
    sHandheldShakePitch[c->cameraID] = 0;
    sHandheldShakeYaw[c->cameraID] = 0;
    sHandheldShakeRoll[c->cameraID] = 0;
    c->cutscene = 0;
    marioOffset[0] = 0.f;
    marioOffset[1] = 125.f;
    marioOffset[2] = 400.f;
    ...
    switch (gCurrLevelNum) {
        ...
        case LEVEL_CASTLE_GROUNDS:
            ...
            gLakituState[c->cameraID].mode = CAMERA_MODE_FREE_ROAM;
            break;
        case LEVEL_SA:
            marioOffset[2] = 200.f;
            break;
        ...
        case LEVEL_CASTLE:
            ...
            break;
        case LEVEL_RR:
            vec3f_set(sFixedModeBasePosition[c->cameraID], -2985.f, 478.f, -5568.f);
            break;
    }
    ...
    switch (gCurrLevelArea) {
        ...
        case AREA_TTM_OUTSIDE:
            gLakituState[c->cameraID].mode = CAMERA_MODE_RADIAL;
            break;
    }

    // Set the camera pos to marioOffset (relative to Mario), added to Mario's position
    offset_rotated(c->pos, (&gPlayerCameraState[c->cameraID])->pos, marioOffset, (&gPlayerCameraState[c->cameraID])->faceAngle);
    if (c->mode != CAMERA_MODE_BEHIND_MARIO) {
        c->pos[1] = find_floor((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 100.f,
                               (&gPlayerCameraState[c->cameraID])->pos[2], &floor) + 125.f;
    }
    vec3f_copy(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
    vec3f_copy(gLakituState[c->cameraID].curPos, c->pos);
    vec3f_copy(gLakituState[c->cameraID].curFocus, c->focus);
    vec3f_copy(gLakituState[c->cameraID].goalPos, c->pos);
    vec3f_copy(gLakituState[c->cameraID].goalFocus, c->focus);
    vec3f_copy(gLakituState[c->cameraID].pos, c->pos);
    vec3f_copy(gLakituState[c->cameraID].focus, c->focus);
    if (c->mode == CAMERA_MODE_FIXED) {
        set_fixed_cam_axis_sa_lobby(c->mode, c);
    }
    store_lakitu_cam_info_for_c_up(c);
    gLakituState[c->cameraID].yaw = calculate_yaw(c->focus, c->pos);
    gLakituState[c->cameraID].nextYaw = gLakituState[c->cameraID].yaw;
    c->yaw = gLakituState[c->cameraID].yaw;
    c->nextYaw = gLakituState[c->cameraID].yaw;
}
...
void zoom_out_if_paused_and_outside(struct GraphNodeCamera *camera) {
    ...
    if (gCameraMovementFlags[gCamera->cameraID] & CAM_MOVE_PAUSE_SCREEN) {
        if (sFramesPaused >= 2) {
            if (sZoomOutAreaMasks[areaMaskIndex] & areaBit) {

                camera->focus[0] = gCamera->areaCenX;
                camera->focus[1] = ((&gPlayerCameraState[gCamera->cameraID])->pos[1] + gCamera->areaCenY) / 2;
                camera->focus[2] = gCamera->areaCenZ;
                vec3f_get_dist_and_angle(camera->focus, (&gPlayerCameraState[gCamera->cameraID])->pos, &dist, &pitch, &yaw);
                vec3f_set_dist_and_angle((&gPlayerCameraState[gCamera->cameraID])->pos, camera->pos, 6000.f, 0x1000, yaw);
...
void select_mario_cam_mode(void) {
    sSelectionFlags[0] = CAM_MODE_MARIO_SELECTED;
    sSelectionFlags[1] = CAM_MODE_MARIO_SELECTED;
}
 ...
 */
void update_graph_node_camera(struct GraphNodeCamera *gc) { // this is fine i guess
    UNUSED u8 unused[8];
    UNUSED struct Camera *c = gc->config.camera;

    gc->rollScreen = gLakituState[gCamera->cameraID].roll;
    vec3f_copy(gc->pos, gLakituState[gCamera->cameraID].pos);
    vec3f_copy(gc->focus, gLakituState[gCamera->cameraID].focus);
    zoom_out_if_paused_and_outside(gc);
}

Gfx *geo_camera_main(s32 callContext, struct GraphNode *g, void *context) { // very fine
 ...
 */
s32 cam_select_alt_mode(s32 selection, struct Camera *c) {
    s32 mode = CAM_SELECTION_FIXED;

    if (selection == CAM_SELECTION_MARIO) {
        if (!(sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_SELECTED)) {
            sSelectionFlags[c->cameraID] |= CAM_MODE_MARIO_SELECTED;
        }
        sCameraSoundFlags[c->cameraID] |= CAM_SOUND_UNUSED_SELECT_MARIO;
    }

    // The alternate mode is up-close, but the player just selected fixed in the pause menu
    if (selection == CAM_SELECTION_FIXED && (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_SELECTED)) {
        // So change to normal mode in case the user paused in up-close mode
        set_cam_angle(CAM_ANGLE_LAKITU, c);
        sSelectionFlags[c->cameraID] &= ~CAM_MODE_MARIO_SELECTED;
        sCameraSoundFlags[c->cameraID] |= CAM_SOUND_UNUSED_SELECT_FIXED;
    }
 ...
 */
s32 set_cam_angle(s32 mode, struct Camera *c) {
    s32 curMode = CAM_ANGLE_LAKITU;

    // Switch to Mario mode
    if (mode == CAM_ANGLE_MARIO && !(sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE)) {
        sSelectionFlags[c->cameraID] |= CAM_MODE_MARIO_ACTIVE;
        if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT) {
            sSelectionFlags[c->cameraID] |= CAM_MODE_LAKITU_WAS_ZOOMED_OUT;
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_ZOOMED_OUT;
        }
        sCameraSoundFlags[c->cameraID] |= CAM_SOUND_MARIO_ACTIVE;
    }

    // Switch back to normal mode
    if (mode == CAM_ANGLE_LAKITU && (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE)) {
        sSelectionFlags[c->cameraID] &= ~CAM_MODE_MARIO_ACTIVE;
        if (sSelectionFlags[c->cameraID] & CAM_MODE_LAKITU_WAS_ZOOMED_OUT) {
            sSelectionFlags[c->cameraID] &= ~CAM_MODE_LAKITU_WAS_ZOOMED_OUT;
            gCameraMovementFlags[c->cameraID] |= CAM_MOVE_ZOOMED_OUT;
        } else {
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_ZOOMED_OUT;
        }
        sCameraSoundFlags[c->cameraID] |= CAM_SOUND_NORMAL_ACTIVE;
    }
    if (sSelectionFlags[c->cameraID] & CAM_MODE_MARIO_ACTIVE) {
 ...
 */
void set_handheld_shake(u8 mode, struct Camera *c) {
    switch (mode) {
        // They're not in numerical order because that would be too simple...
        case HAND_CAM_SHAKE_CUTSCENE: // Lowest increment
            sHandheldShakeMag[c->cameraID] = 0x600;
            sHandheldShakeInc[c->cameraID] = 0.04f;
            break;
        case HAND_CAM_SHAKE_LOW: // Lowest magnitude
            sHandheldShakeMag[c->cameraID] = 0x300;
            sHandheldShakeInc[c->cameraID] = 0.06f;
            break;
        case HAND_CAM_SHAKE_HIGH: // Highest mag and inc
            sHandheldShakeMag[c->cameraID] = 0x1000;
            sHandheldShakeInc[c->cameraID] = 0.1f;
            break;
        case HAND_CAM_SHAKE_UNUSED: // Never used
            sHandheldShakeMag[c->cameraID] = 0x600;
            sHandheldShakeInc[c->cameraID] = 0.07f;
            break;
        ...
        case HAND_CAM_SHAKE_STAR_DANCE: // Slightly steadier than HANG_OWL and UNUSED
            sHandheldShakeMag[c->cameraID] = 0x400;
            sHandheldShakeInc[c->cameraID] = 0.07f;
            break;
        default:
            sHandheldShakeMag[c->cameraID] = 0x0;
            sHandheldShakeInc[c->cameraID] = 0.f;
    }
}

/**
 * When sHandheldShakeMag[c->cameraID] is nonzero, this function adds small random offsets to `focus` every time
 * sHandheldShakeTimer[c->cameraID] increases above 1.0, simulating the camera shake caused by unsteady hands.
 *
 ...
 */
void shake_camera_handheld(Vec3f pos, Vec3f focus, struct Camera *c) {
    s32 i;
    Vec3f shakeOffset;
    Vec3f shakeSpline[4];
    f32 dist;
    ...
    if (sHandheldShakeMag[c->cameraID] == 0) {
        vec3f_set(shakeOffset, 0.f, 0.f, 0.f);
    } else {
        for (i = 0; i < 4; i++) {
            shakeSpline[i][0] = sHandheldShakeSpline[i][c->cameraID].point[0];
            shakeSpline[i][1] = sHandheldShakeSpline[i][c->cameraID].point[1];
            shakeSpline[i][2] = sHandheldShakeSpline[i][c->cameraID].point[2];
        }
        evaluate_cubic_spline(sHandheldShakeTimer[c->cameraID], shakeOffset, shakeSpline[0],
                              shakeSpline[1], shakeSpline[2], shakeSpline[3]);
        if (1.f <= (sHandheldShakeTimer[c->cameraID] += sHandheldShakeInc[c->cameraID])) {
            // The first 3 control points are always (0,0,0), so the random spline is always just a
            // straight line
            for (i = 0; i < 3; i++) {
                vec3s_copy(sHandheldShakeSpline[i][c->cameraID].point, sHandheldShakeSpline[i + 1][c->cameraID].point);
            }
            random_vec3s(sHandheldShakeSpline[3][c->cameraID].point, sHandheldShakeMag[c->cameraID], sHandheldShakeMag[c->cameraID], sHandheldShakeMag[c->cameraID] / 2);
            sHandheldShakeTimer[c->cameraID] -= 1.f;

            // Code dead, this is set to be 0 before it is used.
            sHandheldShakeInc[c->cameraID] = random_float() * 0.5f;
            if (sHandheldShakeInc[c->cameraID] < 0.02f) {
                sHandheldShakeInc[c->cameraID] = 0.02f;
            }
        }
    }

    approach_s16_asymptotic_bool(&sHandheldShakePitch[c->cameraID], shakeOffset[0], 0x08);
    approach_s16_asymptotic_bool(&sHandheldShakeYaw[c->cameraID], shakeOffset[1], 0x08);
    approach_s16_asymptotic_bool(&sHandheldShakeRoll[c->cameraID], shakeOffset[2], 0x08);

    if (sHandheldShakePitch[c->cameraID] | sHandheldShakeYaw[c->cameraID]) {
        vec3f_get_dist_and_angle(pos, focus, &dist, &pitch, &yaw);
        pitch += sHandheldShakePitch[c->cameraID];
        yaw += sHandheldShakeYaw[c->cameraID];
...
s32 update_camera_hud_status(struct Camera *c) {
    ...
    } else if (set_cam_angle(0, c) == CAM_ANGLE_MARIO) {
...
s32 is_within_100_units_of_mario(f32 posX, f32 posY, f32 posZ, struct Camera *c) {
    s32 isCloseToMario = 0;
    Vec3f pos;

    vec3f_set(pos, posX, posY, posZ);
    if (calc_abs_dist((&gPlayerCameraState[c->cameraID])->pos, pos) < 100.f) {
        isCloseToMario = 1;
    }
    return isCloseToMario;
}

s32 set_or_approach_f32_asymptotic(f32 *dst, f32 goal, f32 scale, struct Camera *c) {
    if (sStatusFlags[c->cameraID] & CAM_FLAG_SMOOTH_MOVEMENT) {
 ...
 * given vector.
 */
void set_or_approach_vec3f_asymptotic(Vec3f dst, Vec3f goal, f32 xMul, f32 yMul, f32 zMul, struct Camera *c) {
    set_or_approach_f32_asymptotic(&dst[0], goal[0], xMul, c);
    set_or_approach_f32_asymptotic(&dst[1], goal[1], yMul, c);
    set_or_approach_f32_asymptotic(&dst[2], goal[2], zMul, c);
}
...
s32 set_or_approach_s16_symmetric(s16 *current, s16 target, s16 increment, struct Camera *c) {
    if (sStatusFlags[c->cameraID] & CAM_FLAG_SMOOTH_MOVEMENT) {
 ...
 */
s16 reduce_by_dist_from_camera(s16 value, f32 maxDist, f32 posX, f32 posY, f32 posZ, struct Camera *c) {
    Vec3f pos;
    f32 dist;
    s16 pitch;
    s16 yaw;
    ...
    f32 goalDX = gLakituState[c->cameraID].goalPos[0] - posX;
    f32 goalDY = gLakituState[c->cameraID].goalPos[1] - posY;
    f32 goalDZ = gLakituState[c->cameraID].goalPos[2] - posZ;
    ...
    if (maxDist > dist) {
        ...
        pos[1] = posY;
        pos[2] = posZ;
        vec3f_get_dist_and_angle(gLakituState[c->cameraID].goalPos, pos, &dist, &pitch, &yaw);
        if (dist < maxDist) {
            calculate_angles(gLakituState[c->cameraID].goalPos, gLakituState[c->cameraID].goalFocus, &goalPitch, &goalYaw);
            //! Must be same line to match EU
...
s32 is_mario_behind_surface(struct Camera *c, struct Surface *surf) {
    s32 behindSurface = is_behind_surface((&gPlayerCameraState[c->cameraID])->pos, surf);
 ...
 */
void set_camera_pitch_shake(s16 mag, s16 decay, s16 inc, struct Camera *c) {
    if (gLakituState[c->cameraID].shakeMagnitude[0] < mag) {
        gLakituState[c->cameraID].shakeMagnitude[0] = mag;
        gLakituState[c->cameraID].shakePitchDecay = decay;
        gLakituState[c->cameraID].shakePitchVel = inc;
    }
}
 ...
 */
void set_camera_yaw_shake(s16 mag, s16 decay, s16 inc, struct Camera *c) {
    if (ABS(mag) > ABS(gLakituState[c->cameraID].shakeMagnitude[1])) {
        gLakituState[c->cameraID].shakeMagnitude[1] = mag;
        gLakituState[c->cameraID].shakeYawDecay = decay;
        gLakituState[c->cameraID].shakeYawVel = inc;
    }
}
 ...
 */
void set_camera_roll_shake(s16 mag, s16 decay, s16 inc, struct Camera *c) {
    if (gLakituState[c->cameraID].shakeMagnitude[2] < mag) {
        gLakituState[c->cameraID].shakeMagnitude[2] = mag;
        gLakituState[c->cameraID].shakeRollDecay = decay;
        gLakituState[c->cameraID].shakeRollVel = inc;
    }
}
...
void set_pitch_shake_from_point(s16 mag, s16 decay, s16 inc, f32 maxDist, f32 posX, f32 posY, f32 posZ) {
    ...
    pos[1] = posY;
    pos[2] = posZ;
    vec3f_get_dist_and_angle(gLakituState[gMarioObject->oAnimState].goalPos, pos, &dist, &dummyPitch, &dummyYaw);
    mag = reduce_by_dist_from_camera(mag, maxDist, posX, posY, posZ, gMarioStates[gMarioObject->oAnimState].thisPlayerCamera);
    if (mag != 0) {
        set_camera_pitch_shake(mag, decay, inc, gMarioStates[gMarioObject->oAnimState].thisPlayerCamera);
    }
}
...
void set_yaw_shake_from_point(s16 mag, s16 decay, s16 inc, f32 maxDist, f32 posX, f32 posY, f32 posZ) {
    ...
    mag = reduce_by_dist_from_camera(mag, maxDist, posX, posY, posZ, gMarioStates[gMarioObject->oAnimState].thisPlayerCamera);
    if (mag != 0) {
        set_camera_yaw_shake(mag, decay, inc, gMarioStates[gMarioObject->oAnimState].thisPlayerCamera);
    }
}
 */
...
void shake_camera_pitch(Vec3f pos, Vec3f focus) {
    ...
    if (gLakituState[gMarioObject->oAnimState].shakeMagnitude[0] | gLakituState[gMarioObject->oAnimState].shakeMagnitude[1]) {
        vec3f_get_dist_and_angle(pos, focus, &dist, &pitch, &yaw);
        pitch += gLakituState[gMarioObject->oAnimState].shakeMagnitude[0] * sins(gLakituState[gMarioObject->oAnimState].shakePitchPhase);
        vec3f_set_dist_and_angle(pos, focus, dist, pitch, yaw);
        increment_shake_offset(&gLakituState[gMarioObject->oAnimState].shakePitchPhase, gLakituState[gMarioObject->oAnimState].shakePitchVel);
        if (camera_approach_s16_symmetric_bool(&gLakituState[gMarioObject->oAnimState].shakeMagnitude[0], 0,
                                               gLakituState[gMarioObject->oAnimState].shakePitchDecay) == 0) {
            gLakituState[gMarioObject->oAnimState].shakePitchPhase = 0;
        }
    }
}
...
void shake_camera_yaw(Vec3f pos, Vec3f focus) {
    ...
    if (gLakituState[gMarioObject->oAnimState].shakeMagnitude[1] != 0) {
        vec3f_get_dist_and_angle(pos, focus, &dist, &pitch, &yaw);
        yaw += gLakituState[gMarioObject->oAnimState].shakeMagnitude[1] * sins(gLakituState[gMarioObject->oAnimState].shakeYawPhase);
        vec3f_set_dist_and_angle(pos, focus, dist, pitch, yaw);
        increment_shake_offset(&gLakituState[gMarioObject->oAnimState].shakeYawPhase, gLakituState[gMarioObject->oAnimState].shakeYawVel);
        if (camera_approach_s16_symmetric_bool(&gLakituState[gMarioObject->oAnimState].shakeMagnitude[1], 0,
                                               gLakituState[gMarioObject->oAnimState].shakeYawDecay) == 0) {
            gLakituState[gMarioObject->oAnimState].shakeYawPhase = 0;
        }
    }
}
...
void shake_camera_roll(s16 *roll) { // this is fine
    ...
    if (gLakituState[gMarioObject->oAnimState].shakeMagnitude[2] != 0) {
        increment_shake_offset(&gLakituState[gMarioObject->oAnimState].shakeRollPhase, gLakituState[gMarioObject->oAnimState].shakeRollVel);
        *roll += gLakituState[gMarioObject->oAnimState].shakeMagnitude[2] * sins(gLakituState[gMarioObject->oAnimState].shakeRollPhase);
        if (camera_approach_s16_symmetric_bool(&gLakituState[gMarioObject->oAnimState].shakeMagnitude[2], 0,
                                               gLakituState[gMarioObject->oAnimState].shakeRollDecay) == 0) {
            gLakituState[gMarioObject->oAnimState].shakeRollPhase = 0;
        }
    }
}
...
s32 offset_yaw_outward_radial(struct Camera *c, s16 areaYaw) {
    ...
    s16 yaw = sModeOffsetYaw[c->cameraID];
    ...
    switch (gCurrLevelArea) {
        case AREA_TTC:
            ...
            areaCenter[1] = (&gPlayerCameraState[c->cameraID])->pos[1];
            areaCenter[2] = c->areaCenZ;
            distFromAreaCenter = calc_abs_dist(areaCenter, (&gPlayerCameraState[c->cameraID])->pos);
    ...
    if (sAreaYawChange[c->cameraID] < 0) {
...
void play_sound_if_cam_switched_to_lakitu_or_mario(struct Camera *c) {
    if (sCameraSoundFlags[c->cameraID] & CAM_SOUND_MARIO_ACTIVE) {
        play_sound_rbutton_changed();
    }
    if (sCameraSoundFlags[c->cameraID] & CAM_SOUND_NORMAL_ACTIVE) {
        play_sound_rbutton_changed();
    }
    sCameraSoundFlags[c->cameraID] &= ~(CAM_SOUND_MARIO_ACTIVE | CAM_SOUND_NORMAL_ACTIVE);
}
...
s32 radial_camera_input(struct Camera *c, UNUSED f32 unused) {
    ...
    if ((gCameraMovementFlags[c->cameraID] & CAM_MOVE_ENTERED_ROTATE_SURFACE) || !(gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE)) {

        // If C-L or C-R are pressed, the camera is rotating
        if (c->controller->buttonPressed & (L_CBUTTONS | R_CBUTTONS)) {
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_ENTERED_ROTATE_SURFACE;
        ...
        if (c->controller->buttonPressed & R_CBUTTONS) {
            if (sModeOffsetYaw[c->cameraID] > -0x800) {
                ...
                if (c->mode == CAMERA_MODE_RADIAL) {
                    ...
                    if (sModeOffsetYaw[c->cameraID] > 0x22AA) {
                        s2ndRotateFlags[c->cameraID] |= CAM_MOVE_ROTATE_RIGHT;
                    }

                    if (sModeOffsetYaw[c->cameraID] == DEGREES(105)) {
                        play_sound_button_change_blocked();
                    } else {
                        play_sound_cbutton_side();
                    }
                } else {
                    if (sModeOffsetYaw[c->cameraID] == DEGREES(60)) {
                        play_sound_button_change_blocked();
                    } else {
                        play_sound_cbutton_side();
                    }
    ...
    if (c->controller->buttonPressed & D_CBUTTONS) {
        if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT) {
            gCameraMovementFlags[c->cameraID] |= CAM_MOVE_ALREADY_ZOOMED_OUT;
#ifndef VERSION_JP
            play_camera_buzz_if_cdown(c);
#endif
        } else {
...
s32 trigger_cutscene_dialog(s32 trigger) {
    ...
    if (trigger == 1) {
        start_object_cutscene_without_focus(CUTSCENE_READ_MESSAGE, gCamera);
    }
    if (trigger == 2) {
    }
    return result;
...
void handle_c_button_movement(struct Camera *c) {
    ...
    // Zoom in
    if (c->controller->buttonPressed & U_CBUTTONS) {
        if (c->mode != CAMERA_MODE_FIXED && (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT)) {
            gCameraMovementFlags[c->cameraID] &= ~CAM_MOVE_ZOOMED_OUT;
            play_sound_cbutton_up();
        } else {
            set_mode_c_up(c);
            if (sZeroZoomDist[c->cameraID] > gCameraZoomDist[c->cameraID]) {
                sZoomAmount[c->cameraID] = -gCameraZoomDist[c->cameraID];
            } else {
                sZoomAmount[c->cameraID] = gCameraZoomDist[c->cameraID];
            }
        }
    ...
    if (c->mode != CAMERA_MODE_FIXED) {
        ...
        if (c->controller->buttonPressed & D_CBUTTONS) {
            if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ZOOMED_OUT) {
                gCameraMovementFlags[c->cameraID] |= CAM_MOVE_ALREADY_ZOOMED_OUT;
                sZoomAmount[c->cameraID] = gCameraZoomDist[c->cameraID] + 400.f;
#ifndef VERSION_JP
                play_camera_buzz_if_cdown(c);
#endif
            } else {
        ...
        if (c->controller->buttonPressed & R_CBUTTONS) {
            if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_LEFT) {
                ...
                sCSideButtonYaw[c->cameraID] = -cSideYaw;
            }
        }
        if (c->controller->buttonPressed & L_CBUTTONS) {
            if (gCameraMovementFlags[c->cameraID] & CAM_MOVE_ROTATE_RIGHT) {
 ...
 * Zero the 10 cvars.
 */
void clear_cutscene_vars(struct Camera *c) {
    s32 i;

    for (i = 0; i < 10; i++) {
        sCutsceneVars[i][c->cameraID].unused1 = 0;
        vec3f_set(sCutsceneVars[i][c->cameraID].point, 0.f, 0.f, 0.f);
        vec3f_set(sCutsceneVars[i][c->cameraID].unusedPoint, 0.f, 0.f, 0.f);
        vec3s_set(sCutsceneVars[i][c->cameraID].angle, 0, 0, 0);
        sCutsceneVars[i][c->cameraID].unused2 = 0;
    }
}
 ...
 */
u8 open_door_cutscene(u8 pullResult, u8 pushResult, struct Camera *c) {
    s16 result;

    if ((&gPlayerCameraState[c->cameraID])->action == ACT_PULLING_DOOR) {
        result = pullResult;
    }
    if ((&gPlayerCameraState[c->cameraID])->action == ACT_PUSHING_DOOR) {
        result = pushResult;
    }
    return result;
}
...
u8 get_cutscene_from_mario_status(struct Camera *c) {
    ...
    if (cutscene == 0) {
        ...
        cutscene = sObjectCutscene[c->cameraID];
        sObjectCutscene[c->cameraID] = 0;
        if ((&gPlayerCameraState[c->cameraID])->cameraEvent == CAM_EVENT_DOOR) {
            switch (gCurrLevelArea) {
                case AREA_CASTLE_LOBBY:
                    ...
                    if (c->mode == CAMERA_MODE_SPIRAL_STAIRS || c->mode == CAMERA_MODE_CLOSE
                        ...
                        cutscene = open_door_cutscene(CUTSCENE_DOOR_PULL_MODE, CUTSCENE_DOOR_PUSH_MODE, c);
                    } else {
                        cutscene = open_door_cutscene(CUTSCENE_DOOR_PULL, CUTSCENE_DOOR_PUSH, c);
                    }
                    break;
                case AREA_BBH:
        ...
        if ((&gPlayerCameraState[c->cameraID])->cameraEvent == CAM_EVENT_DOOR_WARP) {
            cutscene = CUTSCENE_DOOR_WARP;
        }
        if ((&gPlayerCameraState[c->cameraID])->cameraEvent == CAM_EVENT_CANNON) {
            cutscene = CUTSCENE_ENTER_CANNON;
        }
        if (SURFACE_IS_PAINTING_WARP(sMarioGeometry[c->cameraID].currFloorType)) {
            cutscene = CUTSCENE_ENTER_PAINTING;
        }
        switch ((&gPlayerCameraState[c->cameraID])->action) {
            case ACT_DEATH_EXIT:
        ...
        switch ((&gPlayerCameraState[c->cameraID])->cameraEvent) {
 ...
 */
void warp_camera(f32 displacementX, f32 displacementY, f32 displacementZ, struct MarioState *m) {
    Vec3f displacement;
    struct MarioState *marioStates = &gMarioStates[m->thisPlayerCamera->cameraID];
    struct LinearTransitionPoint *start = &sModeInfo[m->thisPlayerCamera->cameraID].transitionStart;
    struct LinearTransitionPoint *end = &sModeInfo[m->thisPlayerCamera->cameraID].transitionEnd;
    areaWarped = 1;
    gCurrLevelArea = gCurrLevelNum * 16 + gCurrentArea->index;
    displacement[0] = displacementX;
    displacement[1] = displacementY;
    displacement[2] = displacementZ;
    vec3f_add(gLakituState[m->thisPlayerCamera->cameraID].curPos, displacement);
    vec3f_add(gLakituState[m->thisPlayerCamera->cameraID].curFocus, displacement);
    vec3f_add(gLakituState[m->thisPlayerCamera->cameraID].goalPos, displacement);
    vec3f_add(gLakituState[m->thisPlayerCamera->cameraID].goalFocus, displacement);
...
void set_focus_rel_mario(struct Camera *c, f32 leftRight, f32 yOff, f32 forwBack, s16 yawOff) {
    ...
    yaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + yawOff;
    c->focus[2] = (&gPlayerCameraState[c->cameraID])->pos[2] + forwBack * coss(yaw) - leftRight * sins(yaw);
    c->focus[0] = (&gPlayerCameraState[c->cameraID])->pos[0] + forwBack * sins(yaw) + leftRight * coss(yaw);
    c->focus[1] = (&gPlayerCameraState[c->cameraID])->pos[1] + yOff + focFloorYOff;
}
...
void determine_pushing_or_pulling_door(s16 *rotation, struct Camera *c) {
    if ((&gPlayerCameraState[c->cameraID])->action == ACT_PULLING_DOOR) {
        *rotation = 0;
    } else {
...
s16 next_lakitu_state(Vec3f newPos, Vec3f newFoc, Vec3f curPos, Vec3f curFoc,
                      Vec3f oldPos, Vec3f oldFoc, s16 yaw, struct Camera *c) {
    s16 yawVelocity;
    s16 pitchVelocity;
    f32 distVelocity;
    f32 goalDist;
    ...
    f32 distTimer = sModeTransition[c->cameraID].framesLeft;
    s16 angleTimer = sModeTransition[c->cameraID].framesLeft;
    ...
    if (sStatusFlags[c->cameraID] & CAM_FLAG_START_TRANSITION) {
        for (i = 0; i < 3; i++) {
            ...
            startPos[i] = oldPos[i] + (&gPlayerCameraState[c->cameraID])->pos[i] - sModeTransition[c->cameraID].marioPos[i];
            startFoc[i] = oldFoc[i] + (&gPlayerCameraState[c->cameraID])->pos[i] - sModeTransition[c->cameraID].marioPos[i];
        }


        vec3f_get_dist_and_angle(curFoc, startFoc, &sModeTransition[c->cameraID].focDist, &sModeTransition[c->cameraID].focPitch,
                                 &sModeTransition[c->cameraID].focYaw);
        vec3f_get_dist_and_angle(curFoc, startPos, &sModeTransition[c->cameraID].posDist, &sModeTransition[c->cameraID].posPitch,
                                 &sModeTransition[c->cameraID].posYaw);
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_START_TRANSITION;
    }

    // Transition from the last mode to the current one
    if (sModeTransition[c->cameraID].framesLeft > 0) {
        inTransition = 1;

        vec3f_get_dist_and_angle(curFoc, curPos, &goalDist, &goalPitch, &goalYaw);
        distVelocity = ABS(goalDist - sModeTransition[c->cameraID].posDist) / distTimer;
        pitchVelocity = ABS(goalPitch - sModeTransition[c->cameraID].posPitch) / angleTimer;
        yawVelocity = ABS(goalYaw - sModeTransition[c->cameraID].posYaw) / angleTimer;

        camera_approach_f32_symmetric_bool(&sModeTransition[c->cameraID].posDist, goalDist, distVelocity);
        camera_approach_s16_symmetric_bool(&sModeTransition[c->cameraID].posYaw, goalYaw, yawVelocity);
        camera_approach_s16_symmetric_bool(&sModeTransition[c->cameraID].posPitch, goalPitch, pitchVelocity);
        vec3f_set_dist_and_angle(curFoc, nextPos, sModeTransition[c->cameraID].posDist, sModeTransition[c->cameraID].posPitch,
                                 sModeTransition[c->cameraID].posYaw);

        vec3f_get_dist_and_angle(curPos, curFoc, &goalDist, &goalPitch, &goalYaw);
        pitchVelocity = sModeTransition[c->cameraID].focPitch / (s16) sModeTransition[c->cameraID].framesLeft;
        yawVelocity = sModeTransition[c->cameraID].focYaw / (s16) sModeTransition[c->cameraID].framesLeft;
        distVelocity = sModeTransition[c->cameraID].focDist / sModeTransition[c->cameraID].framesLeft;

        camera_approach_s16_symmetric_bool(&sModeTransition[c->cameraID].focPitch, goalPitch, pitchVelocity);
        camera_approach_s16_symmetric_bool(&sModeTransition[c->cameraID].focYaw, goalYaw, yawVelocity);
        camera_approach_f32_symmetric_bool(&sModeTransition[c->cameraID].focDist, 0, distVelocity);
        vec3f_set_dist_and_angle(curFoc, nextFoc, sModeTransition[c->cameraID].focDist, sModeTransition[c->cameraID].focPitch,
                                 sModeTransition[c->cameraID].focYaw);

        vec3f_copy(newFoc, nextFoc);
        vec3f_copy(newPos, nextPos);

        if (gCamera->cutscene != 0 || !(gCameraMovementFlags[c->cameraID] & CAM_MOVE_C_UP_MODE)) {
        ...
        sModeTransition[c->cameraID].framesLeft--;
        yaw = calculate_yaw(newFoc, newPos);
    } else {
        sModeTransition[c->cameraID].posDist = 0.f;
        sModeTransition[c->cameraID].posPitch = 0;
        sModeTransition[c->cameraID].posYaw = 0;
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_TRANSITION_OUT_OF_C_UP;
    }
    vec3f_copy(sModeTransition[c->cameraID].marioPos, (&gPlayerCameraState[c->cameraID])->pos);
    return yaw;
}
...
s32 set_camera_mode_fixed(struct Camera *c, s16 x, s16 y, s16 z) {
    ...
    if (sFixedModeBasePosition[c->cameraID][0] != posX || sFixedModeBasePosition[c->cameraID][1] != posY
        || sFixedModeBasePosition[c->cameraID][2] != posZ) {
        basePosSet = TRUE;
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
    }
    vec3f_set(sFixedModeBasePosition[c->cameraID], posX, posY, posZ);
    if (c->mode != CAMERA_MODE_FIXED) {
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
        c->mode = CAMERA_MODE_FIXED;
        vec3f_set(c->pos, sFixedModeBasePosition[c->cameraID][0], (&gPlayerCameraState[c->cameraID])->pos[1],
                  sFixedModeBasePosition[c->cameraID][2]);
    }
    return basePosSet;
}
...
void set_camera_mode_boss_fight(struct Camera *c) {
    if (c->mode != CAMERA_MODE_BOSS_FIGHT) {
        ...
        sModeOffsetYaw[c->cameraID] = c->nextYaw - DEGREES(45);
    }
}

void set_camera_mode_close_cam(u8 *mode, struct Camera *c) {
...
void set_camera_mode_radial(struct Camera *c, s16 transitionTime) {
    ...
    focus[0] = c->areaCenX;
    focus[1] = (&gPlayerCameraState[c->cameraID])->pos[1];
    focus[2] = c->areaCenZ;
    if (c->mode != CAMERA_MODE_RADIAL) {
        yaw = calculate_yaw(focus, (&gPlayerCameraState[c->cameraID])->pos) - calculate_yaw(c->focus, c->pos) + DEGREES(90);
        if (yaw > 0) {
...
void parallel_tracking_init(struct Camera *c, struct ParallelTrackingPoint *path) {
    if (c->mode != CAMERA_MODE_PARALLEL_TRACKING) {
        sParTrackPath[c->cameraID] = path;
        sParTrackIndex[c->cameraID] = 0;
        sParTrackTransOff[c->cameraID].pos[0] = 0.f;
        sParTrackTransOff[c->cameraID].pos[1] = 0.f;
        sParTrackTransOff[c->cameraID].pos[2] = 0.f;
        // Place the camera in the middle of the path
        c->pos[0] = (sParTrackPath[c->cameraID][0].pos[0] + sParTrackPath[c->cameraID][1].pos[0]) / 2;
        c->pos[1] = (sParTrackPath[c->cameraID][0].pos[1] + sParTrackPath[c->cameraID][1].pos[1]) / 2;
        c->pos[2] = (sParTrackPath[c->cameraID][0].pos[2] + sParTrackPath[c->cameraID][1].pos[2]) / 2;
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
 ...
 */
void set_fixed_cam_axis_sa_lobby(UNUSED s16 preset, struct Camera *c) {
    switch (gCurrLevelArea) {
        case AREA_SA:
            vec3f_set(sFixedModeBasePosition[c->cameraID], 646.f, 143.f, -1513.f);
            break;

        case AREA_CASTLE_LOBBY:
            vec3f_set(sFixedModeBasePosition[c->cameraID], -577.f, 143.f, 1443.f);
            break;
    }
}
 ...
 */
void check_blocking_area_processing(const u8 *mode, struct Camera *c) {
    if ((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_METAL_WATER ||
                        *mode == CAMERA_MODE_BEHIND_MARIO || *mode == CAMERA_MODE_WATER_SURFACE) {
        sStatusFlags[c->cameraID] |= CAM_FLAG_BLOCK_AREA_PROCESSING;
    }
    ...
    if ((*mode == CAMERA_MODE_BEHIND_MARIO &&
            !((&gPlayerCameraState[c->cameraID])->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER))) ||
...
BAD_RETURN(s32) cam_rr_exit_building_side(struct Camera *c) {
    ...
    s8DirModeBaseYaw[c->cameraID] = DEGREES(90);
}
...
BAD_RETURN(s32) cam_rr_exit_building_top(struct Camera *c) {
    ...
    if (c->pos[1] < 6343.f) {
        c->pos[1] = 7543.f;
        gLakituState[c->cameraID].goalPos[1] = c->pos[1];
        gLakituState[c->cameraID].curPos[1] = c->pos[1];
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
    }
}
...
BAD_RETURN(s32) cam_sl_snowman_head_8dir(struct Camera *c) {
    ...
    s8DirModeBaseYaw[c->cameraID] = 0x1D27;
}
...
void move_camera_through_floor_while_descending(struct Camera *c, f32 height) {
    ...
    if ((sMarioGeometry[c->cameraID].currFloorHeight < height - 100.f)
        && (sMarioGeometry[c->cameraID].prevFloorHeight > sMarioGeometry[c->cameraID].currFloorHeight)) {
        c->pos[1] = height - 400.f;
        gLakituState[c->cameraID].curPos[1] = height - 400.f;
        gLakituState[c->cameraID].goalPos[1] = height - 400.f;
    }
}
...
BAD_RETURN(s32) cam_hmc_enter_maze(struct Camera *c) {
    ...
    if (c->pos[1] > -102.f) {
        vec3f_get_dist_and_angle(c->focus, gLakituState[c->cameraID].goalPos, &dist, &pitch, &yaw);
        vec3f_set_dist_and_angle(c->focus, gLakituState[c->cameraID].goalPos, 300.f, pitch, yaw);
        gLakituState[c->cameraID].goalPos[1] = -800.f;
#ifndef VERSION_JP
        c->pos[1] = gLakituState[c->cameraID].goalPos[1];
        gLakituState[c->cameraID].curPos[1] = gLakituState[c->cameraID].goalPos[1];
#endif
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
    }
}
 ...
 */
BAD_RETURN(s32) cam_ssl_enter_pyramid_top(struct Camera *c) {
    start_object_cutscene_without_focus(CUTSCENE_ENTER_PYRAMID_TOP, c);
}
...
/**
 * Moves the camera to through the tunnel by forcing sModeOffsetYaw[c->cameraID]
 */
BAD_RETURN(s32) cam_thi_move_cam_through_tunnel(UNUSED struct Camera *c) {
    if (sModeOffsetYaw[c->cameraID] < DEGREES(60)) {
        sModeOffsetYaw[c->cameraID] = DEGREES(60);
    }
}
...
BAD_RETURN(s32) cam_thi_look_through_tunnel(UNUSED struct Camera *c) {
    // ~82.5 degrees
    if (sModeOffsetYaw[c->cameraID] > 0x3AAA) {
        sModeOffsetYaw[c->cameraID] = 0x3AAA;
    }
}
...
BAD_RETURN(s32) cam_castle_hmc_start_pool_cutscene(struct Camera *c) {
    if (((&gPlayerCameraState[c->cameraID])->action != ACT_SPECIAL_DEATH_EXIT)
        && ((&gPlayerCameraState[c->cameraID])->action != ACT_SPECIAL_EXIT_AIRBORNE)) {
...
BAD_RETURN(s32) cam_castle_lobby_entrance(UNUSED struct Camera *c) {
    vec3f_set(sCastleEntranceOffset[c->cameraID], -813.f - sFixedModeBasePosition[c->cameraID][0],
              378.f - sFixedModeBasePosition[c->cameraID][1], 1103.f - sFixedModeBasePosition[c->cameraID][2]);
}
...
BAD_RETURN(s32) cam_castle_look_upstairs(struct Camera *c) {
    ...
    if ((sMarioGeometry[c->cameraID].currFloorHeight > 1229.f) && (floorHeight < 1229.f)
        && (sCSideButtonYaw[c->cameraID] == 0)) {
...
BAD_RETURN(s32) cam_castle_close_mode(struct Camera *c) {
    set_camera_mode_close_cam(&c->mode, c);
}
...
BAD_RETURN(s32) cam_bbh_fall_into_pool(struct Camera *c) {
    ...
    offset_rotated(gLakituState[c->cameraID].goalPos, (&gPlayerCameraState[c->cameraID])->pos, dir, (&gPlayerCameraState[c->cameraID])->faceAngle);
    gLakituState[c->cameraID].goalPos[1] = -2300.f;
    vec3f_copy(c->pos, gLakituState[c->cameraID].goalPos);
    sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
}
...
BAD_RETURN(s32) cam_bbh_elevator(struct Camera *c) {
    if (c->mode == CAMERA_MODE_FIXED) {
        set_camera_mode_close_cam(&c->mode, c);
        c->pos[1] = -405.f;
        gLakituState[c->cameraID].goalPos[1] = -405.f;
    }
}
...
BAD_RETURN(s32) cam_ccm_enter_slide_shortcut(UNUSED struct Camera *c) {
    sStatusFlags[c->cameraID] |= CAM_FLAG_CCM_SLIDE_SHORTCUT;
}
...
u32 surface_type_modes(struct Camera *c) {
    ...
    switch (sMarioGeometry[c->cameraID].currFloorType) {
...
s16 camera_course_processing(struct Camera *c) {
    ...
    if (c->mode == CAMERA_MODE_C_UP) {
        c->mode = (u8) sModeInfo[c->cameraID].lastMode;
    }
    check_blocking_area_processing(&c->mode, c);
    ...
    if (sCameraTriggers[level] != NULL) {
        ...
        while (sCameraTriggers[level][b].event != NULL) {
            ...
            if (sCameraTriggers[level][b].area == area) {
                ...
                if (is_pos_in_bounds((&gPlayerCameraState[c->cameraID])->pos, center, bounds,
                    ...
                    if (!(sStatusFlags[c->cameraID] & CAM_FLAG_BLOCK_AREA_PROCESSING)) {
    ...
    if (!(sStatusFlags[c->cameraID] & CAM_FLAG_BLOCK_AREA_PROCESSING)) {
        switch (gCurrLevelArea) {
            ...
            case AREA_BBH:
                ...
                if (vec3f_compare(sFixedModeBasePosition[c->cameraID], 210.f, 420.f, 3109.f) == 1)
                {
                    if ((&gPlayerCameraState[c->cameraID])->pos[1] < 1800.f) {
            ...
            case AREA_BOB:
                if (set_mode_if_not_set_by_surface(c, CAMERA_MODE_NONE) == 0) {
                    if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_BOSS_FIGHT_CAMERA) {
                        set_camera_mode_boss_fight(c);
                    } else {
                        if (c->mode == CAMERA_MODE_CLOSE) {
            ...
            case AREA_WDW_MAIN:
                if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_INSTANT_WARP_1B) {
                    if (0) {
                    }
            ...
            case AREA_WDW_TOWN:
                if (sMarioGeometry[c->cameraID].currFloorType == SURFACE_INSTANT_WARP_1C) {
                    if (0) {
                    }
            ...
            case AREA_DDD_WHIRLPOOL:
                ...
                gLakituState[c->cameraID].defMode = CAMERA_MODE_OUTWARD_RADIAL;
                break;

            case AREA_DDD_SUB:
                if ((c->mode != CAMERA_MODE_BEHIND_MARIO)
                    && (c->mode != CAMERA_MODE_WATER_SURFACE)) {
                    if ((((&gPlayerCameraState[c->cameraID])->action & ACT_FLAG_ON_POLE) != 0)
                        || (sMarioGeometry[c->cameraID].currFloorHeight > 800.f)) {
                        transition_to_camera_mode(c, CAMERA_MODE_8_DIRECTIONS, 60);

                    } else {
                        if ((&gPlayerCameraState[c->cameraID])->pos[1] < 800.f) {
                            transition_to_camera_mode(c, CAMERA_MODE_FREE_ROAM, 60);
                        }
                    }
                }
                //! @bug this does nothing
                gLakituState[c->cameraID].defMode = CAMERA_MODE_FREE_ROAM;
                break;
        }
    }
...
s32 rotate_camera_around_walls(struct Camera *c, Vec3f cPos, s16 *avoidYaw, s16 yawRange) {
    ...
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, cPos, &dummyDist, &dummyPitch, &yawFromMario);
    sStatusFlags[c->cameraID] &= ~CAM_FLAG_CAM_NEAR_WALL;
    ...
    for (step = 0; step < 8; step++) {
        ...
        colData.x = (&gPlayerCameraState[c->cameraID])->pos[0] + ((cPos[0] - (&gPlayerCameraState[c->cameraID])->pos[0]) * checkDist);
        colData.y = (&gPlayerCameraState[c->cameraID])->pos[1] + ((cPos[1] - (&gPlayerCameraState[c->cameraID])->pos[1]) * checkDist);
        colData.z = (&gPlayerCameraState[c->cameraID])->pos[2] + ((cPos[2] - (&gPlayerCameraState[c->cameraID])->pos[2]) * checkDist);
        ...
        if (find_wall_collisions(&colData) != 0) {
            ...
            if (find_wall_collisions(&colData) != 0) {
                ...
                if ((is_range_behind_surface((&gPlayerCameraState[c->cameraID])->pos, cPos, wall, yawRange, SURFACE_WALL_MISC) == 0)
 ...
 */
void find_mario_floor_and_ceil(struct PlayerGeometry *pg, struct Camera *c) {
    struct Surface *surf;
    s16 tempCheckingSurfaceCollisionsForCamera = gCheckingSurfaceCollisionsForCamera;
    gCheckingSurfaceCollisionsForCamera = TRUE;

    if (find_floor((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 10.f,
                   (&gPlayerCameraState[c->cameraID])->pos[2], &surf) != -11000.f) {
        pg->currFloorType = surf->type;
    } else {
        pg->currFloorType = 0;
    }

    if (find_ceil((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] - 10.f,
                  (&gPlayerCameraState[c->cameraID])->pos[2], &surf) != 20000.f) {
    ...
    gCheckingSurfaceCollisionsForCamera = FALSE;
    pg->currFloorHeight = find_floor((&gPlayerCameraState[c->cameraID])->pos[0],
                                     (&gPlayerCameraState[c->cameraID])->pos[1] + 10.f,
                                     (&gPlayerCameraState[c->cameraID])->pos[2], &pg->currFloor);
    pg->currCeilHeight = find_ceil((&gPlayerCameraState[c->cameraID])->pos[0],
                                   (&gPlayerCameraState[c->cameraID])->pos[1] - 10.f,
                                   (&gPlayerCameraState[c->cameraID])->pos[2], &pg->currCeil);
    pg->waterHeight = find_water_level((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[2]);
 ...
 */
void start_object_cutscene(u8 cutscene, struct Object *o, struct Camera *c) {
    sObjectCutscene[c->cameraID] = cutscene;
    gRecentCutscene[c->cameraID] = 0;
 ...
 * This will play if nothing else happened in the same frame, like exiting or warping.
 */
u8 start_object_cutscene_without_focus(u8 cutscene, struct Camera *c) {
    sObjectCutscene[c->cameraID] = cutscene;
    sCutsceneDialogResponse[c->cameraID] = 0;
    return 0;
}
    ...
    for (i = 0; i < gActivePlayers; i++) {
        if ((gMarioStates[i].thisPlayerCamera->cutscene == 0) && (sObjectCutscene[i] == 0)) {
            if (gRecentCutscene[i] != cutscene) {
                ...
                if (dialogID != -1) {
                    sCutsceneDialogID[i] = dialogID;
                } else {
                    sCutsceneDialogID[i] = DIALOG_001;
                }
            } else {
                response = sCutsceneDialogResponse[i];
            }
...
void cutscene_reset_spline(struct Camera *c) {
    sCutsceneSplineSegment[c->cameraID] = 0;
    sCutsceneSplineSegmentProgress[c->cameraID] = 0;
}

void stop_cutscene_and_retrieve_stored_info(struct Camera *c) {
    gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
    c->cutscene = 0;
    vec3f_copy(c->focus, sCameraStoreCutscene[c->cameraID].focus);
    vec3f_copy(c->pos, sCameraStoreCutscene[c->cameraID].pos);
}
 ...
 *
 * @return if Mario left the dialog state, return CUTSCENE_LOOP, else return gCutsceneTimer[c->cameraID]
 */
s16 cutscene_common_set_dialog_state(s32 state, struct Camera *c) {
...
/// Unused SSL cutscene?
//static UNUSED void unused_cutscene_mario_dialog_looking_down(struct Camera *c) {
//    gCutsceneTimer[c->cameraID] = cutscene_common_set_dialog_state(3, c);
//}
...
/// Unused SSL cutscene?
//static UNUSED void unused_cutscene_mario_dialog_looking_up(struct Camera *c) {
//    gCutsceneTimer[c->cameraID] = cutscene_common_set_dialog_state(2, c);
//}
...
void reset_pan_distance(struct Camera *c) {
    sPanDistance[c->cameraID] = 0;
}
...
void player2_rotate_cam(struct Camera *c, s16 minPitch, s16 maxPitch, s16 minYaw, s16 maxYaw) {
    ...
    approach_s16_asymptotic_bool(&sCreditsPlayer2Yaw[c->cameraID], -(s16)(gPlayer2Controller->stickX * 250.f), 4);
    approach_s16_asymptotic_bool(&sCreditsPlayer2Pitch[c->cameraID], -(s16)(gPlayer2Controller->stickY * 265.f), 4);
    ...
    if (sCreditsPlayer2Pitch[c->cameraID] > maxPitch) {
        sCreditsPlayer2Pitch[c->cameraID] = maxPitch;
    }
    if (sCreditsPlayer2Pitch[c->cameraID] < minPitch) {
        sCreditsPlayer2Pitch[c->cameraID] = minPitch;
    }

    if (sCreditsPlayer2Yaw[c->cameraID] > maxYaw) {
        sCreditsPlayer2Yaw[c->cameraID] = maxYaw;
    }
    if (sCreditsPlayer2Yaw[c->cameraID] < minYaw) {
        sCreditsPlayer2Yaw[c->cameraID] = minYaw;
    }
...
void store_info_cannon(struct Camera *c) {
    vec3f_copy(sCameraStoreCutscene[c->cameraID].pos, c->pos);
    vec3f_copy(sCameraStoreCutscene[c->cameraID].focus, c->focus);
    sCameraStoreCutscene[c->cameraID].panDist = sPanDistance[c->cameraID];
    sCameraStoreCutscene[c->cameraID].cannonYOffset = sCannonYOffset[c->cameraID];
}
...
void retrieve_info_cannon(struct Camera *c) {
    vec3f_copy(c->pos, sCameraStoreCutscene[c->cameraID].pos);
    vec3f_copy(c->focus, sCameraStoreCutscene[c->cameraID].focus);
    sPanDistance[c->cameraID] = sCameraStoreCutscene[c->cameraID].panDist;
    sCannonYOffset[c->cameraID] = sCameraStoreCutscene[c->cameraID].cannonYOffset;
}
...
void store_info_star(struct Camera *c) {
    ...
    vec3f_copy(sCameraStoreCutscene[c->cameraID].pos, c->pos);
    sCameraStoreCutscene[c->cameraID].focus[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    sCameraStoreCutscene[c->cameraID].focus[1] = c->focus[1];
    sCameraStoreCutscene[c->cameraID].focus[2] = (&gPlayerCameraState[c->cameraID])->pos[2];
}
...
BAD_RETURN(s32) cutscene_shake_explosion(struct Camera *c) {
    set_environmental_camera_shake(SHAKE_ENV_EXPLOSION, c);
    cutscene_set_fov_shake_preset(1, c);
}
...
BAD_RETURN(s32) cutscene_ending_mario_fall_focus_mario(struct Camera *c) {
    ...
    offset[2] = ABS((&gPlayerCameraState[c->cameraID])->pos[1] - c->pos[1]) * -0.1f;
    if (offset[2] > -100.f) {
        offset[2] = -100.f;
    }

    offset_rotated(c->focus, (&gPlayerCameraState[c->cameraID])->pos, offset, (&gPlayerCameraState[c->cameraID])->faceAngle);
}
 ...
 */
BAD_RETURN(s32) cutscene_ending_reset_spline(struct Camera *c) {
    sCutsceneVars[9][c->cameraID].point[0] = 0.f;
    cutscene_reset_spline(c);
}
...
BAD_RETURN(s32) cutscene_ending_fly_up_to_window(struct Camera *c) {
    move_point_along_spline(c->pos, sEndingFlyToWindowPos, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    move_point_along_spline(c->focus, sEndingFlyToWindowFocus, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
}
...
BAD_RETURN(s32) cutscene_ending_peach_appear_closeup(struct Camera *c) {
    ...
    vec3f_set(c->focus, gCutsceneFocus[c->cameraID]->oPosX, gCutsceneFocus[c->cameraID]->oPosY + 125.f, gCutsceneFocus[c->cameraID]->oPosZ);
}
...
BAD_RETURN(s32) cutscene_ending_peach_appears(struct Camera *c) {
    ...
    approach_f32_asymptotic_bool(&c->pos[1], gCutsceneFocus[c->cameraID]->oPosY + 35.f, 0.02f);
    approach_f32_asymptotic_bool(&c->focus[1], gCutsceneFocus[c->cameraID]->oPosY + 125.f, 0.15f);
    player2_rotate_cam(c, -0x2000, 0x2000, -0x2000, 0x2000);
}
...
BAD_RETURN(s32) cutscene_ending_peach_descends_start(UNUSED struct Camera *c) {
    cutscene_reset_spline(c);
    sCutsceneVars[2][c->cameraID].point[1] = 150.f;
}
...
BAD_RETURN(s32) cutscene_ending_follow_peach_descent(struct Camera *c) {
    move_point_along_spline(c->pos, sEndingPeachDescentCamPos, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    c->pos[1] += gCutsceneFocus[c->cameraID]->oPosY + sCutsceneVars[3][c->cameraID].point[1];
}
...
BAD_RETURN(s32) cutscene_ending_peach_descent_lower_focus(UNUSED struct Camera *c) {
    camera_approach_f32_symmetric_bool(&(sCutsceneVars[2][c->cameraID].point[1]), 90.f, 0.5f);
}
...
BAD_RETURN(s32) cutscene_ending_peach_descent_back_to_mario(struct Camera *c) {
    ...
    move_point_along_spline(pos, sEndingPeachDescentCamPos, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    c->pos[0] = pos[0];
    c->pos[2] = pos[2];
    approach_f32_asymptotic_bool(&c->pos[1], (pos[1] += gCutsceneFocus[c->cameraID]->oPosY), 0.07f);
}
...
BAD_RETURN(s32) cutscene_ending_peach_descends(struct Camera *c) {
    ...
    vec3f_set(c->focus, gCutsceneFocus[c->cameraID]->oPosX, sCutsceneVars[2][c->cameraID].point[1] + gCutsceneFocus[c->cameraID]->oPosY,
...
BAD_RETURN(s32) cutscene_ending_mario_to_peach(struct Camera *c) {
    cutscene_event(cutscene_ending_reset_spline, c, 0, 0);
    move_point_along_spline(c->pos, sEndingMarioToPeachPos, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    move_point_along_spline(c->focus, sEndingMarioToPeachFocus, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    player2_rotate_cam(c, -0x2000, 0x2000, -0x2000, 0x2000);
}
...
BAD_RETURN(s32) cutscene_ending_look_up_at_castle(UNUSED struct Camera *c) {
    move_point_along_spline(c->focus, sEndingLookUpAtCastle, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
}
...
BAD_RETURN(s32) cutscene_ending_peach_wakeup(struct Camera *c) {
    ...
    cutscene_spawn_obj(7, 315, c);
    cutscene_spawn_obj(9, 355, c);
#else
    cutscene_event(cutscene_ending_look_up_at_castle, c, 250, -1);
    cutscene_spawn_obj(7, 300, c);
    cutscene_spawn_obj(9, 340, c);
#endif
...
BAD_RETURN(s32) cutscene_ending_kiss_closeup(struct Camera *c) {
    set_fov_function(CAM_FOV_SET_29, c);
...
BAD_RETURN(s32) cutscene_ending_look_at_sky(struct Camera *c) {
    move_point_along_spline(c->focus, sEndingLookAtSkyFocus, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
...
BAD_RETURN(s32) cutscene_ending_cake_for_mario(struct Camera *c) {
    ...
    cutscene_spawn_obj(8, 600, c);
    cutscene_spawn_obj(8, 608, c);
    cutscene_spawn_obj(8, 624, c);
    cutscene_spawn_obj(8, 710, c);
}
...
BAD_RETURN(s32) cutscene_ending_stop(struct Camera *c) {
    set_fov_function(CAM_FOV_SET_45, c);
    c->cutscene = 0;
    gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
}
 ...
 */
BAD_RETURN(s32) cutscene_grand_star_start(struct Camera *c) {
    vec3f_set(sCutsceneVars[0][c->cameraID].point, 0.f, 150.f, -600.f);
    offset_rotated(sCutsceneVars[1][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[0][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->faceAngle);
    sCutsceneVars[1][c->cameraID].point[1] = 457.f;
}
...
BAD_RETURN(s32) cutscene_grand_star_front_of_mario(struct Camera *c) {
    ...
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[1][c->cameraID].point, &goalDist, &goalPitch, &goalYaw);
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, &dist, &pitch, &yaw);
    ...
    vec3f_set_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, dist, pitch, yaw);
}

/**
 * Started shortly after Mario starts the triple jump. Stores Mario's face angle and zeros cvar2.
 */
BAD_RETURN(s32) cutscene_grand_star_mario_jump(struct Camera *c) {
    vec3s_set(sCutsceneVars[0][c->cameraID].angle, 0, (&gPlayerCameraState[c->cameraID])->faceAngle[1], 0);
    vec3f_set(sCutsceneVars[2][c->cameraID].point, 0.f, 0.f, 0.f);
}

/**
 * Accelerate cvar2 to point back and to the left (relative to the camera).
 */
BAD_RETURN(s32) cutscene_grand_star_accel_cvar2(struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[2][c->cameraID].point[2], -40.f, 2.0f);
    sCutsceneVars[2][c->cameraID].point[0] = 5.0f;
}
...
BAD_RETURN(s32) cutscene_grand_star_approach_mario(struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[2][c->cameraID].point[2], 0.f, 2.f);
    sCutsceneVars[2][c->cameraID].point[0] = 0.f;
    approach_f32_asymptotic_bool(&c->pos[0], (&gPlayerCameraState[c->cameraID])->pos[0], 0.01f);
    approach_f32_asymptotic_bool(&c->pos[2], (&gPlayerCameraState[c->cameraID])->pos[2], 0.01f);
}
...
BAD_RETURN(s32) cutscene_grand_star_move_cvar2(struct Camera *c) {
    offset_rotated(c->pos, c->pos, sCutsceneVars[2][c->cameraID].point, sCutsceneVars[0][c->cameraID].angle);
}

BAD_RETURN(s32) cutscene_grand_star_focus_mario(struct Camera *c) {
    Vec3f foc;

    vec3f_set(foc, (&gPlayerCameraState[c->cameraID])->pos[0], ((&gPlayerCameraState[c->cameraID])->pos[1] - 307.f) * 0.5f + 407.f, (&gPlayerCameraState[c->cameraID])->pos[2]);
...
BAD_RETURN(s32) cutscene_grand_star_fly_start(struct Camera *c) {
    ...
    vec3f_set(sCutsceneVars[7][c->cameraID].point, 0.5f, 0.5f, 0.5f);
    //! cvar6 is unused in grand star
    vec3f_set(sCutsceneVars[6][c->cameraID].point, 0.01f, 0.01f, 0.01f);
    vec3f_set(sCutsceneVars[4][c->cameraID].point, 0.f, 0.f, 0.f);
    vec3f_set(sCutsceneVars[5][c->cameraID].point, 0.f, c->focus[1] - (&gPlayerCameraState[c->cameraID])->pos[1], 0.f);
    sCutsceneVars[8][c->cameraID].point[2] = 0.f;
    sCutsceneVars[8][c->cameraID].point[0] = 0.f;
}

/**
 * Decrease the cvar offsets so that Lakitu flies closer to Mario.
 */
BAD_RETURN(s32) cutscene_grand_star_fly_move_to_mario(struct Camera *c) {
    Vec3f posOff;

    vec3f_set(posOff, -600.f, 0.f, -400.f);
    approach_vec3f_asymptotic(sCutsceneVars[4][c->cameraID].point, posOff, 0.05f, 0.05f, 0.05f);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[5][c->cameraID].point[1], 0.f, 2.f);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[5][c->cameraID].point[2], -200.f, 6.f);
}
 ...
 */
BAD_RETURN(s32) cutscene_grand_star_fly_mario_offscreen(struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[8][c->cameraID].point[0], 15.f, 0.1f);

    camera_approach_f32_symmetric_bool(&sCutsceneVars[4][c->cameraID].point[0], -2000.f, sCutsceneVars[8][c->cameraID].point[0]);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[4][c->cameraID].point[1], 1200.f, sCutsceneVars[8][c->cameraID].point[0] / 10.f);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[4][c->cameraID].point[2], 1000.f, sCutsceneVars[8][c->cameraID].point[0] / 10.f);

    camera_approach_f32_symmetric_bool(&sCutsceneVars[5][c->cameraID].point[0], 0.f, sCutsceneVars[8][c->cameraID].point[0]);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[5][c->cameraID].point[1], 1200.f, sCutsceneVars[8][c->cameraID].point[0] / 2);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[5][c->cameraID].point[2], 1000.f, sCutsceneVars[8][c->cameraID].point[0] / 1.5f);
}
...
BAD_RETURN(s32) cutscene_grand_star_fly_app_cvars(struct Camera *c) {
    ...
    camera_approach_f32_symmetric_bool(&sCutsceneVars[8][c->cameraID].point[2], 90.f, 2.5f);
    offset_rotated(goalPos, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[4][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->faceAngle);
    offset_rotated(goalFoc, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[5][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->faceAngle);

    // Move towards goalPos by cvar8's Z speed
    vec3f_get_dist_and_angle(goalPos, c->pos, &dist, &pitch, &yaw);
    camera_approach_f32_symmetric_bool(&dist, 0, sCutsceneVars[8][c->cameraID].point[2]);
...
void focus_in_front_of_mario(struct Camera *c, f32 dist, f32 speed) {
    ...
    offset_rotated(goalFocus, (&gPlayerCameraState[c->cameraID])->pos, offset, (&gPlayerCameraState[c->cameraID])->faceAngle);
...
BAD_RETURN(s32) cutscene_dance_rotate(struct Camera *c) {
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, 0, 0, 0x200);
}

BAD_RETURN(s32) cutscene_dance_rotate_move_back(struct Camera *c) {
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, -15.f, 0, 0);
}

BAD_RETURN(s32) cutscene_dance_rotate_move_towards_mario(struct Camera *c) {
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, 20.f, 0, 0);
}

/**
 * Speculated to be dance-related due to its proximity to the other dance functions
 */
BAD_RETURN(s32) cutscene_dance_unused(UNUSED struct Camera *c) {
}
...
BAD_RETURN(s32) cutscene_dance_shake_fov(UNUSED struct Camera *c) {
    set_fov_shake(0x200, 0x28, 0x8000, c);
}
...
BAD_RETURN(s32) cutscene_dance_default_rotate(struct Camera *c) {
    ...
    if (c->cutscene != CUTSCENE_DANCE_DEFAULT) { // CUTSCENE_DANCE_ROTATE
        ...
        if (((&gPlayerCameraState[c->cameraID])->action != ACT_STAR_DANCE_NO_EXIT)
            && ((&gPlayerCameraState[c->cameraID])->action != ACT_STAR_DANCE_WATER)
            && ((&gPlayerCameraState[c->cameraID])->action != ACT_STAR_DANCE_EXIT)) {
            gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
            c->cutscene = 0;
            transition_next_state(c, 20);
            sStatusFlags[c->cameraID] |= CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
...
BAD_RETURN(s32) star_dance_bound_yaw(struct Camera *c, s16 absYaw, s16 yawMax) {
    ...
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, &distCamToMario, &dummyPitch, &yaw);
...
BAD_RETURN(s32) cutscene_dance_closeup_start(struct Camera *c) {
    ...
    vec3f_copy(sCutsceneVars[9][c->cameraID].point, c->focus);
    //! cvar8 is unused in the closeup cutscene
    sCutsceneVars[8][c->cameraID].angle[0] = 0x2000;
}
...
BAD_RETURN(s32) cutscene_dance_closeup_focus_mario(struct Camera *c) {
    ...
    vec3f_set(marioPos, (&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 125.f, (&gPlayerCameraState[c->cameraID])->pos[2]);
    approach_vec3f_asymptotic(sCutsceneVars[9][c->cameraID].point, marioPos, 0.2f, 0.2f, 0.2f);
    vec3f_copy(c->focus, sCutsceneVars[9][c->cameraID].point);
}
...
BAD_RETURN(s32) cutscene_dance_closeup_zoom(UNUSED struct Camera *c) {
    set_fov_function(CAM_FOV_APP_80, c);
}
...
BAD_RETURN(s32) cutscene_dance_closeup_shake_fov(UNUSED struct Camera *c) {
    set_fov_shake(0x300, 0x30, 0x8000, c);
}
...
BAD_RETURN(s32) cutscene_dance_closeup(struct Camera *c) {
    ...
    if ((&gPlayerCameraState[c->cameraID])->action == ACT_STAR_DANCE_WATER) {
    ...
    set_handheld_shake(HAND_CAM_SHAKE_CUTSCENE, c);
}
...
BAD_RETURN(s32) cutscene_dance_fly_away_start(struct Camera *c) {
    ...
    vec3f_copy(sCutsceneVars[9][c->cameraID].point, c->focus);
    sCutsceneVars[8][c->cameraID].point[2] = 65.f;
...
void cutscene_pan_cvar9(struct Camera *c) {
    vec3f_copy(c->focus, sCutsceneVars[9][c->cameraID].point);
    sCutsceneVars[9][c->cameraID].angle[0] -= 29;
    sCutsceneVars[9][c->cameraID].angle[1] += 29;
    pan_camera(c, sCutsceneVars[9][c->cameraID].angle[0], sCutsceneVars[9][c->cameraID].angle[1]);
}
...
BAD_RETURN(s32) cutscene_dance_fly_rotate_around_mario(struct Camera *c) {
    ...
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[8][c->cameraID].point[2], 0, 0);
}
...
BAD_RETURN(s32) cutscene_dance_fly_away_rotate_while_flying(struct Camera *c) {
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, 0, 0, 0x80);
}

BAD_RETURN(s32) cutscene_dance_fly_away_shake_fov(UNUSED struct Camera *c) {
    set_fov_shake(0x400, 0x30, 0x8000, c);
}
...
BAD_RETURN(s32) cutscene_dance_fly_away(struct Camera *c) {
    ...
    set_fov_function(CAM_FOV_DEFAULT, c);
    set_handheld_shake(HAND_CAM_SHAKE_STAR_DANCE, c);
}
...
BAD_RETURN(s32) cutscene_key_dance_jump_cvar(struct Camera *c) {
    offset_rotated(c->pos, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[8][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->faceAngle);
    offset_rotated(c->focus, (&gPlayerCameraState[c->cameraID])->pos, sCutsceneVars[7][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->faceAngle);
}

/**
 * Jump to a closeup view of Mario and the key.
 */
BAD_RETURN(s32) cutscene_key_dance_jump_closeup(struct Camera *c) {
    vec3f_set(sCutsceneVars[8][c->cameraID].point, 38.f, 171.f, -248.f);
    vec3f_set(sCutsceneVars[7][c->cameraID].point, -57.f, 51.f, 187.f);
}

/**
 * Jump to a view from the lower left (Mario's right).
 */
BAD_RETURN(s32) cutscene_key_dance_jump_lower_left(struct Camera *c) {
    vec3f_set(sCutsceneVars[8][c->cameraID].point, -178.f, 62.f, -132.f);
    vec3f_set(sCutsceneVars[7][c->cameraID].point, 299.f, 91.f, 58.f);
}

/**
 * Jump to a rotated view from above.
 */
BAD_RETURN(s32) cutscene_key_dance_jump_above(struct Camera *c) {
    gLakituState[c->cameraID].keyDanceRoll = 0x2800;
    vec3f_set(sCutsceneVars[8][c->cameraID].point, 89.f, 373.f, -304.f);
    vec3f_set(sCutsceneVars[7][c->cameraID].point, 0.f, 127.f, 0.f);
}

/**
 * Finally, jump to a further view, slightly to Mario's left.
 */
BAD_RETURN(s32) cutscene_key_dance_jump_last(struct Camera *c) {
    gLakituState[c->cameraID].keyDanceRoll = 0;
    vec3f_set(sCutsceneVars[8][c->cameraID].point, 135.f, 158.f, -673.f);
    vec3f_set(sCutsceneVars[7][c->cameraID].point, -20.f, 135.f, -198.f);
}

BAD_RETURN(s32) cutscene_key_dance_shake_fov(UNUSED struct Camera *c) {
    set_fov_shake(0x180, 0x30, 0x8000, c);
}

BAD_RETURN(s32) cutscene_key_dance_handheld_shake(struct Camera *c) {
    set_handheld_shake(HAND_CAM_SHAKE_CUTSCENE, c);
}
...
BAD_RETURN(s32) cutscene_bowser_area_shake_fov(UNUSED struct Camera *c) {
    cutscene_set_fov_shake_preset(2, c);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_set_pos(struct Camera *c) {
    vec3f_set_dist_and_angle(sCutsceneVars[2][c->cameraID].point, c->pos, sCutsceneVars[3][c->cameraID].point[2],
                                  sCutsceneVars[3][c->cameraID].angle[0], sCutsceneVars[3][c->cameraID].angle[1]);
    vec3f_set(sCutsceneVars[2][c->cameraID].point, gSecondCameraFocus[c->cameraID]->oPosX, gSecondCameraFocus[c->cameraID]->oPosY,
              gSecondCameraFocus[c->cameraID]->oPosZ);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_focus_sine(UNUSED struct Camera *c) {
    ...
    yOff = sins(sCutsceneVars[4][c->cameraID].angle[1]) * 120.0f + 120.0f;
    sCutsceneVars[4][c->cameraID].angle[1] -= 0x200;
    approach_f32_asymptotic_bool(&sCutsceneVars[0][c->cameraID].point[1], yOff, 0.5f);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_set_focus(struct Camera *c) {
    offset_rotated(c->focus, sCutsceneVars[2][c->cameraID].point, sCutsceneVars[0][c->cameraID].point, sCutsceneVars[2][c->cameraID].angle);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_adjust_offsets(UNUSED struct Camera *c) {
    approach_s16_asymptotic_bool(&sCutsceneVars[3][c->cameraID].angle[0], 0x6C8, 30);
    approach_f32_asymptotic_bool(&sCutsceneVars[0][c->cameraID].point[2], -200.f, 0.02f);
    approach_f32_asymptotic_bool(&sCutsceneVars[3][c->cameraID].point[2], 550.f, 0.02f);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_pan_left(UNUSED struct Camera *c) {
    approach_f32_asymptotic_bool(&sCutsceneVars[0][c->cameraID].point[2], 0.f, 0.05f);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_mario_dialog(UNUSED struct Camera *c) {
    cutscene_common_set_dialog_state(1, c);
}

void cutscene_stop_dialog(UNUSED struct Camera *c) {
    cutscene_common_set_dialog_state(0, c);
}
...
BAD_RETURN(s32) cutscene_bowser_arena_start(struct Camera *c) {
    sCutsceneVars[3][c->cameraID].point[2] = 430.f;
    sCutsceneVars[3][c->cameraID].angle[1] = gSecondCameraFocus[c->cameraID]->oMoveAngleYaw - DEGREES(45);
    sCutsceneVars[3][c->cameraID].angle[0] = 0xD90;
    ...
    vec3f_set(sCutsceneVars[0][c->cameraID].point, 0.f, 120.f, -800.f);
    vec3s_set(sCutsceneVars[2][c->cameraID].angle, gSecondCameraFocus[c->cameraID]->oMoveAnglePitch,
              gSecondCameraFocus[c->cameraID]->oMoveAngleYaw, gSecondCameraFocus[c->cameraID]->oMoveAngleRoll);
...
BAD_RETURN(s32) cutscene_bowser_arena_dialog(struct Camera *c) {
    ...
    if (get_dialog_id() == -1) {
        gCutsceneTimer[c->cameraID] = CUTSCENE_LOOP;
    }
}
...
BAD_RETURN(s32) cutscene_bowser_arena_end(struct Camera *c) {
    ...
    transition_next_state(c, 20);
    sStatusFlags[c->cameraID] |= CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
    sModeOffsetYaw[c->cameraID] = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + DEGREES(90);
...
BAD_RETURN(s32) cutscene_star_spawn_focus_star(struct Camera *c) {
    ...
    if (gCutsceneFocus[c->cameraID] != NULL) {
        object_pos_to_vec3f(starPos, gCutsceneFocus[c->cameraID]);
        starPos[1] += gCutsceneFocus[c->cameraID]->hitboxHeight;
...
BAD_RETURN(s32) cutscene_star_spawn(struct Camera *c) {
    ...
    if (gObjCutsceneDone[c->cameraID]) {
...
BAD_RETURN(s32) cutscene_star_spawn_back(struct Camera *c) {
    if ((c->mode == CAMERA_MODE_BOSS_FIGHT) && (set_cam_angle(0, c) == CAM_ANGLE_LAKITU)) {
...
BAD_RETURN(s32) cutscene_exit_to_castle_grounds_focus_mario(struct Camera *c) {
    vec3f_copy(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
    c->focus[1] = c->pos[1] + ((&gPlayerCameraState[c->cameraID])->pos[1] + 125.f - c->pos[1]) * 0.5f;
    approach_vec3f_asymptotic(c->focus, (&gPlayerCameraState[c->cameraID])->pos, 0.05f, 0.4f, 0.05f);
}
...
BAD_RETURN(s32) cutscene_red_coin_star_start(struct Camera *c) {
    object_pos_to_vec3f(sCutsceneVars[1][c->cameraID].point, gCutsceneFocus[c->cameraID]);
    store_info_star(c);
    // Store the default fov for after the cutscene
    sCutsceneVars[2][c->cameraID].point[2] = sFOVState[c->cameraID].fov;
}
...
BAD_RETURN(s32) cutscene_red_coin_star_focus_xz(struct Camera *c) {
    approach_f32_asymptotic_bool(&c->focus[0], gCutsceneFocus[c->cameraID]->oPosX, 0.15f);
    approach_f32_asymptotic_bool(&c->focus[2], gCutsceneFocus[c->cameraID]->oPosZ, 0.15f);
}
...
BAD_RETURN(s32) cutscene_red_coin_star_focus_y(struct Camera *c) {
    approach_f32_asymptotic_bool(&c->focus[1], gCutsceneFocus[c->cameraID]->oPosY, 0.1f);
}
...
BAD_RETURN(s32) cutscene_red_coin_star_look_up_at_star(struct Camera *c) {
    c->focus[1] = sCutsceneVars[1][c->cameraID].point[1] + (gCutsceneFocus[c->cameraID]->oPosY - sCutsceneVars[1][c->cameraID].point[1]) * 0.8f;
}
...
BAD_RETURN(s32) cutscene_red_coin_star_warp(struct Camera *c) {
    ...
    vec3f_set(sCutsceneVars[1][c->cameraID].point, o->oHomeX, o->oHomeY, o->oHomeZ);
    vec3f_get_dist_and_angle(sCutsceneVars[1][c->cameraID].point, c->pos, &dist, &pitch, &yaw);
    posYaw = calculate_yaw(sCutsceneVars[1][c->cameraID].point, c->pos);
    yaw = calculate_yaw(sCutsceneVars[1][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    ...
    vec3f_set_dist_and_angle(sCutsceneVars[1][c->cameraID].point, c->pos, 400.f, 0x1000, yaw);
    sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;
}
...
BAD_RETURN(s32) cutscene_red_coin_star_end(struct Camera *c) {
    ...
    sFOVState[c->cameraID].fov = sCutsceneVars[2][c->cameraID].point[2];
}

/**
 * Moves the camera towards the cutscene's focus, stored in sCutsceneVars[3][c->cameraID].point
 *
 * sCutsceneVars[3][c->cameraID].point is used as the target point
 * sCutsceneVars[0][c->cameraID].point is used as the current camera focus during the transition
 *
...
void cutscene_goto_cvar_pos(struct Camera *c, f32 goalDist, s16 goalPitch, s16 rotPitch, s16 rotYaw) {
    ...
    s16 curPitch, curYaw;
    UNUSED f64 unused2;
    vec3f_get_dist_and_angle(sCutsceneVars[3][c->cameraID].point, c->pos, &nextDist, &nextPitch, &nextYaw);
    ...
    if ((nextDist > 8000.f) && (c->cutscene == CUTSCENE_PREPARE_CANNON)) {
        ...
        nextPitch = goalPitch;
        vec3f_copy(sCutsceneVars[0][c->cameraID].point, sCutsceneVars[3][c->cameraID].point);
        sStatusFlags[c->cameraID] &= ~CAM_FLAG_SMOOTH_MOVEMENT;

        if (gCurrLevelNum == LEVEL_TTM) {
            nextYaw = atan2s(sCutsceneVars[3][c->cameraID].point[2] - c->areaCenZ,
                             sCutsceneVars[3][c->cameraID].point[0] - c->areaCenX);
        }
    } else {
        if (c->cutscene == CUTSCENE_PREPARE_CANNON) {
            vec3f_get_dist_and_angle(c->pos, sCutsceneVars[0][c->cameraID].point, &curDist, &curPitch, &curYaw);
            vec3f_get_dist_and_angle(c->pos, sCutsceneVars[3][c->cameraID].point, &cannonDist, &cannonPitch, &cannonYaw);
            ...
            // Move the current focus, sCutsceneVars[0][c->cameraID].point, in the direction towards the cannon
            vec3f_set_dist_and_angle(c->pos, sCutsceneVars[0][c->cameraID].point, curDist, curPitch, curYaw);
        } else {
            approach_vec3f_asymptotic(sCutsceneVars[0][c->cameraID].point, sCutsceneVars[3][c->cameraID].point, 0.1f, 0.1f, 0.1f);
        }
    }
    ...
    vec3f_set_dist_and_angle(sCutsceneVars[3][c->cameraID].point, c->pos, nextDist, nextPitch, nextYaw);
    vec3f_copy(c->focus, sCutsceneVars[0][c->cameraID].point);
...
BAD_RETURN(s32) cutscene_prepare_cannon_start(struct Camera *c) {
    store_info_cannon(c);
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, c->focus);
    sCutsceneVars[2][c->cameraID].point[0] = 30.f;
    // Store the cannon door's position in sCutsceneVars[3][c->cameraID]'s point
    object_pos_to_vec3f(sCutsceneVars[3][c->cameraID].point, gCutsceneFocus[c->cameraID]);
    vec3s_set(sCutsceneVars[5][c->cameraID].angle, 0, 0, 0);
}
...
BAD_RETURN(s32) cutscene_prepare_cannon_fly_to_cannon(struct Camera *c) {
    cutscene_goto_cvar_pos(c, 300.f, 0x2000, 0, sCutsceneVars[5][c->cameraID].angle[1]);
    camera_approach_s16_symmetric_bool(&sCutsceneVars[5][c->cameraID].angle[1], 0x400, 17);
    set_handheld_shake(HAND_CAM_SHAKE_CUTSCENE, c);
}
 ...
 */
void cannon_approach_prev(f32 *value, f32 target, struct Camera *c) {
    f32 inc = ABS(target - *value) / sCutsceneVars[2][c->cameraID].point[0];
    camera_approach_f32_symmetric_bool(value, target, inc);
}

/**
 * Fly or warp back to the previous pos and focus, stored in sCameraStoreCutscene[c->cameraID].
 */
BAD_RETURN(s32) cutscene_prepare_cannon_fly_back(struct Camera *c) {
    f32 distToPrevPos = calc_abs_dist(c->pos, sCameraStoreCutscene[c->cameraID].pos);

    if (distToPrevPos < 8000.f) {
        cannon_approach_prev(&c->pos[0], sCameraStoreCutscene[c->cameraID].pos[0], c);
        cannon_approach_prev(&c->pos[1], sCameraStoreCutscene[c->cameraID].pos[1], c);
        cannon_approach_prev(&c->pos[2], sCameraStoreCutscene[c->cameraID].pos[2], c);
        cannon_approach_prev(&c->focus[0], sCameraStoreCutscene[c->cameraID].focus[0], c);
        cannon_approach_prev(&c->focus[1], sCameraStoreCutscene[c->cameraID].focus[1], c);
        cannon_approach_prev(&c->focus[2], sCameraStoreCutscene[c->cameraID].focus[2], c);
    } else {
    ...
    if (sCutsceneVars[2][c->cameraID].point[0] > 1.f) {
        sCutsceneVars[2][c->cameraID].point[0] -= 1.f;
    }
}
...
void water_death_move_to_mario_side(struct Camera *c) {
    ...
    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, &dist, &pitch, &yaw);
    approach_s16_asymptotic_bool(&yaw, ((&gPlayerCameraState[c->cameraID])->faceAngle[1] - 0x3000), 8);
    vec3f_set_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, dist, pitch, yaw);
}
...
BAD_RETURN(s32) cutscene_death_standing_start(struct Camera *c) {
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, c->focus);
    vec3f_copy(sCutsceneVars[3][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    sCutsceneVars[3][c->cameraID].point[1] += 70.f;
}
...
BAD_RETURN(s32) cutscene_death_standing_goto_mario(struct Camera *c) {
    death_goto_mario(c);
    set_handheld_shake(HAND_CAM_SHAKE_HIGH, c);
}
...
BAD_RETURN(s32) cutscene_death_stomach_start(struct Camera *c) {
    ...
    offset_rotated(sCutsceneVars[3][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, offset, (&gPlayerCameraState[c->cameraID])->faceAngle);
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, c->focus);
}
...
BAD_RETURN(s32) cutscene_bbh_death_start(struct Camera *c) {
    ...
    offset_rotated(sCutsceneVars[3][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, dir, (&gPlayerCameraState[c->cameraID])->faceAngle);
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, c->focus);
}
...
BAD_RETURN(s32) cutscene_quicksand_death(struct Camera *c) {
    sCutsceneVars[3][c->cameraID].point[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    sCutsceneVars[3][c->cameraID].point[1] = (&gPlayerCameraState[c->cameraID])->pos[1] + 20.f;
    sCutsceneVars[3][c->cameraID].point[2] = (&gPlayerCameraState[c->cameraID])->pos[2];
...
BAD_RETURN(s32) cutscene_suffocation_fly_away(UNUSED struct Camera *c) {
    ...
    offset_rotated(target, (&gPlayerCameraState[c->cameraID])->pos, offset, (&gPlayerCameraState[c->cameraID])->faceAngle);
    approach_vec3f_asymptotic(sCutsceneVars[3][c->cameraID].point, target, 0.1f, 0.1f, 0.1f);
}
...
BAD_RETURN(s32) cutscene_suffocation_stay_above_gas(struct Camera *c) {
    ...
    cutscene_goto_cvar_pos(c, 400.f, 0x2800, 0x200, 0);
    gasLevel = find_poison_gas_level((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[2]);
...
BAD_RETURN(s32) cutscene_enter_pool_start(struct Camera *c) {
    ...
    if (gCurrLevelNum == LEVEL_CASTLE) { // entering HMC
        vec3f_set(sCutsceneVars[3][c->cameraID].point, 2485.f, -1589.f, -2659.f);
    }
    if (gCurrLevelNum == LEVEL_HMC) { // entering CotMC
        vec3f_set(sCutsceneVars[3][c->cameraID].point, 3350.f, -4589.f, 4800.f);
    }
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_start(struct Camera *c) {
    ...
    vec3f_copy(sCutsceneVars[1][c->cameraID].point, c->focus);
    vec3f_set(sCutsceneVars[3][c->cameraID].point, c->areaCenX, 1280.f, c->areaCenZ);
}
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_zoom_in(UNUSED struct Camera *c) {
    set_fov_function(CAM_FOV_APP_30, c);
}
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_focus(struct Camera *c) {
    approach_vec3f_asymptotic(c->focus, sCutsceneVars[3][c->cameraID].point, 0.02f, 0.02f, 0.02f);
    sStatusFlags[c->cameraID] |= CAM_FLAG_SMOOTH_MOVEMENT;
}
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_warp(struct Camera *c) {
    ...
    set_fov_function(CAM_FOV_DEFAULT, c);
    sFOVState[c->cameraID].fov = 35.f;

    vec3f_copy(sCutsceneVars[4][c->cameraID].point, c->pos);
    vec3f_copy(sCutsceneVars[5][c->cameraID].point, c->focus);
    vec3f_copy(c->focus, sCutsceneVars[3][c->cameraID].point);

    vec3f_get_dist_and_angle(sCutsceneVars[3][c->cameraID].point, (&gPlayerCameraState[c->cameraID])[0].pos, &dist, &pitch, &yaw);
    vec3f_set_dist_and_angle(sCutsceneVars[3][c->cameraID].point, c->pos, 2000.f, 0, yaw);
    c->pos[1] += 500.f;
}
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_closeup(struct Camera *c) {
    ...
    vec3f_get_dist_and_angle(sCutsceneVars[3][c->cameraID].point, c->pos, &dist, &pitch, &yaw);
    approach_f32_asymptotic_bool(&dist, 2000.f, 0.1f);
    vec3f_set_dist_and_angle(sCutsceneVars[3][c->cameraID].point, c->pos, dist, pitch, yaw);
 ...
 */
BAD_RETURN(s32) cutscene_pyramid_top_explode_cam_shake(struct Camera *c) {
    set_environmental_camera_shake(SHAKE_ENV_PYRAMID_EXPLODE, c);
}
...
BAD_RETURN(s32) cutscene_pyramid_top_explode_warp_back(struct Camera *c) {
    ...
    vec3f_copy(c->pos, sCutsceneVars[4][c->cameraID].point);
    vec3f_copy(c->focus, sCutsceneVars[5][c->cameraID].point);
    set_environmental_camera_shake(SHAKE_ENV_BOWSER_JUMP, c);
}
...
BAD_RETURN(s32) cutscene_enter_pyramid_top(struct Camera *c) {
    ...
    if ((&gPlayerCameraState[c->cameraID])->pos[1] > 1250.f) {
 ...
 *
 * cvar9.point is gCutsceneFocus[c->cameraID]'s position
 * cvar9.angle[1] is the yaw between Mario and the gCutsceneFocus[c->cameraID]
 */
BAD_RETURN(s32) cutscene_dialog_start(struct Camera *c) {
    ...
    sCutsceneVars[8][c->cameraID].angle[0] = 0;
    vec3f_copy(sCutsceneVars[8][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    sCutsceneVars[8][c->cameraID].point[1] += 125.f;

    // Store gCutsceneFocus[c->cameraID]'s position and yaw
    object_pos_to_vec3f(sCutsceneVars[9][c->cameraID].point, gCutsceneFocus[c->cameraID]);
    sCutsceneVars[9][c->cameraID].point[1] += gCutsceneFocus[c->cameraID]->hitboxHeight + 200.f;
    sCutsceneVars[9][c->cameraID].angle[1] = calculate_yaw(sCutsceneVars[8][c->cameraID].point, sCutsceneVars[9][c->cameraID].point);

    yaw = calculate_yaw((&gPlayerCameraState[c->cameraID])->pos, gLakituState[c->cameraID].curPos);
    if ((yaw - sCutsceneVars[9][c->cameraID].angle[1]) & 0x8000) {
        sCutsceneVars[9][c->cameraID].angle[1] -= 0x6000;
    } else {
        sCutsceneVars[9][c->cameraID].angle[1] += 0x6000;
    }
}
...
BAD_RETURN(s32) cutscene_dialog_move_mario_shoulder(struct Camera *c) {
    ...
    scale_along_line(focus, sCutsceneVars[9][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, 0.7f);
    vec3f_get_dist_and_angle(c->pos, focus, &dist, &pitch, &yaw);
    pitch = calculate_pitch(c->pos, sCutsceneVars[9][c->cameraID].point);
    vec3f_set_dist_and_angle(c->pos, pos, dist, pitch, yaw);

    focus[1] = focus[1] + (sCutsceneVars[9][c->cameraID].point[1] - focus[1]) * 0.1f;
    approach_vec3f_asymptotic(c->focus, focus, 0.2f, 0.2f, 0.2f);

    vec3f_copy(pos, c->pos);

    // Set y pos to cvar8's y (top of focus object)
    pos[1] = sCutsceneVars[8][c->cameraID].point[1];
    vec3f_get_dist_and_angle(sCutsceneVars[8][c->cameraID].point, pos, &dist, &pitch, &yaw);
    approach_s16_asymptotic_bool(&yaw, sCutsceneVars[9][c->cameraID].angle[1], 0x10);
    approach_f32_asymptotic_bool(&dist, 180.f, 0.05f);
    vec3f_set_dist_and_angle(sCutsceneVars[8][c->cameraID].point, pos, dist, pitch, yaw);

    // Move up if Mario is below the focus object, down is Mario is above
    pos[1] = sCutsceneVars[8][c->cameraID].point[1]
              + sins(calculate_pitch(sCutsceneVars[9][c->cameraID].point, sCutsceneVars[8][c->cameraID].point)) * 100.f;
...
BAD_RETURN(s32) cutscene_dialog_create_dialog_box(struct Camera *c) {
    if (c->cutscene == CUTSCENE_RACE_DIALOG) {
        create_dialog_box_with_response(sCutsceneDialogID[c->cameraID]);
    } else {
        create_dialog_box(sCutsceneDialogID[c->cameraID]);
    }

    //! Unused. This may have been used before sCutsceneDialogResponse was implemented.
    sCutsceneVars[8][c->cameraID].angle[0] = 3;
}
...
BAD_RETURN(s32) cutscene_dialog(struct Camera *c) {
    ...
    if (gDialogResponse != 0) {
        sCutsceneDialogResponse[c->cameraID] = gDialogResponse;
    }

    if ((get_dialog_id() == -1) && (sCutsceneVars[8][c->cameraID].angle[0] != 0)) {
        if (c->cutscene != CUTSCENE_RACE_DIALOG) {
            sCutsceneDialogResponse[c->cameraID] = 3;
        }
...
BAD_RETURN(s32) cutscene_read_message_start(struct Camera *c) {
    ...
    sCutsceneVars[1][c->cameraID].angle[0] = sCUpCameraPitch[c->cameraID];
    sCutsceneVars[1][c->cameraID].angle[1] = sModeOffsetYaw[c->cameraID];
    sCUpCameraPitch[c->cameraID] = -0x830;
    sModeOffsetYaw[c->cameraID] = 0;
    sCutsceneVars[0][c->cameraID].angle[0] = 0;
}
...
BAD_RETURN(s32) cutscene_read_message(struct Camera *c) {
    ...
    switch (sCutsceneVars[0][c->cameraID].angle[0]) {
        // Do nothing until message is gone.
        case 0:
            if (get_dialog_id() != -1) {
                sCutsceneVars[0][c->cameraID].angle[0] += 1;
                set_time_stop_flags(TIME_STOP_ENABLED | TIME_STOP_DIALOG);
            }
            break;
        // Leave the dialog.
        case 1:
            ...
            if (get_dialog_id() == -1) {
                ...
                sCUpCameraPitch[c->cameraID] = sCutsceneVars[1][c->cameraID].angle[0];
                sModeOffsetYaw[c->cameraID] = sCutsceneVars[1][c->cameraID].angle[1];
                cutscene_unsoften_music(c);
            }
    }
    sStatusFlags[c->cameraID] |= CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
...
BAD_RETURN(s32) cutscene_exit_succ_start(UNUSED struct Camera *c) {
    vec3f_copy(sCutsceneVars[7][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    vec3s_copy(sCutsceneVars[7][c->cameraID].angle, (&gPlayerCameraState[c->cameraID])->faceAngle);
    vec3f_set(sCutsceneVars[6][c->cameraID].point, 6.f, 363.f, 543.f);
    vec3f_set(sCutsceneVars[5][c->cameraID].point, 137.f, 226.f, 995.f);
}
...
BAD_RETURN(s32) cutscene_non_painting_set_cam_pos(struct Camera *c) {
    ...
    switch (gPrevLevel) {
        ...
        default:
            offset_rotated(c->pos, sCutsceneVars[7][c->cameraID].point, sCutsceneVars[5][c->cameraID].point, sCutsceneVars[7][c->cameraID].angle);
...
BAD_RETURN(s32) cutscene_non_painting_set_cam_focus(struct Camera *c) {
    offset_rotated(c->focus, sCutsceneVars[7][c->cameraID].point, sCutsceneVars[6][c->cameraID].point, sCutsceneVars[7][c->cameraID].angle);

    if ((gPrevLevel == LEVEL_COTMC) || (gPrevLevel == LEVEL_HMC) || (gPrevLevel == LEVEL_RR)
        || (gPrevLevel == LEVEL_WMOTR)) {
        c->focus[0] = c->pos[0] + ((&gPlayerCameraState[c->cameraID])->pos[0] - c->pos[0]) * 0.7f;
        c->focus[1] = c->pos[1] + ((&gPlayerCameraState[c->cameraID])->pos[1] - c->pos[1]) * 0.4f;
        c->focus[2] = c->pos[2] + ((&gPlayerCameraState[c->cameraID])->pos[2] - c->pos[2]) * 0.7f;
    } else {
        c->focus[1] = c->pos[1] + ((&gPlayerCameraState[c->cameraID])->pos[1] - c->pos[1]) * 0.2f;
    }
}
...
BAD_RETURN(s32) cutscene_exit_bowser_succ_focus_left(UNUSED struct Camera *c) {
    approach_f32_asymptotic_bool(&sCutsceneVars[6][c->cameraID].point[0], -24.f, 0.05f);
}
 ...
 */
BAD_RETURN(s32) cutscene_exit_succ_shake_landing(struct Camera *c) {
    set_environmental_camera_shake(SHAKE_ENV_EXPLOSION, c);
}
...
BAD_RETURN(s32) cutscene_exit_non_painting_succ_override_cvar(UNUSED struct Camera *c) {
    vec3f_set(sCutsceneVars[5][c->cameraID].point, 137.f, 246.f, 1115.f);
}
...
BAD_RETURN(s32) cutscene_non_painting_death_start(UNUSED struct Camera *c) {
    vec3f_copy(sCutsceneVars[7][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    vec3s_copy(sCutsceneVars[7][c->cameraID].angle, (&gPlayerCameraState[c->cameraID])->faceAngle);
    vec3f_set(sCutsceneVars[6][c->cameraID].point, -42.f, 350.f, 727.f);
    // This is always overwritten, except in the unused cutscene_exit_bowser_death()
    vec3f_set(sCutsceneVars[5][c->cameraID].point, 107.f, 226.f, 1187.f);
}
...
BAD_RETURN(s32) cutscene_non_painting_death_override_offset(UNUSED struct Camera *c) {
    switch (gPrevLevel) {
        case LEVEL_HMC:
            vec3f_set(sCutsceneVars[5][c->cameraID].point, 187.f, 369.f, -197.f);
            break;
        case LEVEL_COTMC:
            vec3f_set(sCutsceneVars[5][c->cameraID].point, 187.f, 369.f, -197.f);
            break;
        default:
            vec3f_set(sCutsceneVars[5][c->cameraID].point, 107.f, 246.f, 1307.f);
            break;
    }
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_start(struct Camera *c) {
    ...
    store_info_star(c);
    yaw = calculate_yaw((&gPlayerCameraState[c->cameraID])->pos, c->pos);
    sCutsceneVars[3][c->cameraID].angle[1] = 0x1200;
    // Basically the amount of rotation to get from behind Mario to in front of Mario
    sCutsceneVars[1][c->cameraID].angle[1] = (yaw - ((&gPlayerCameraState[c->cameraID])->faceAngle[1] + sCutsceneVars[3][c->cameraID].angle[1])) & 0xFF00;
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_rotate_around_mario(struct Camera *c) {
    f32 dist;
    s16 pitch, yaw;
    UNUSED s16 unusedYaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + 0x1000;
    UNUSED s16 unused;
    UNUSED s32 cvar1Yaw = sCutsceneVars[1][c->cameraID].angle[1];

    vec3f_get_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, &dist, &pitch, &yaw);

    // cvar3 wraps around until it reaches 0x1000
    if (sCutsceneVars[3][c->cameraID].angle[1] != 0x1000) {
        sCutsceneVars[3][c->cameraID].angle[1] += 0x100;
    }

    // cvar1 wraps until 0
    if (sCutsceneVars[1][c->cameraID].angle[1] != 0) {
        sCutsceneVars[1][c->cameraID].angle[1] += 0x100;
    }

    yaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + sCutsceneVars[3][c->cameraID].angle[1] + sCutsceneVars[1][c->cameraID].angle[1];
    vec3f_set_dist_and_angle((&gPlayerCameraState[c->cameraID])->pos, c->pos, dist, pitch, yaw);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_lower_cam(struct Camera *c) {
    rotate_and_move_vec3f(c->pos, (&gPlayerCameraState[c->cameraID])->pos, 0, -0x20, 0);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_approach_mario(struct Camera *c) {
    ...
    approach_f32_asymptotic_bool(&c->focus[0], (&gPlayerCameraState[c->cameraID])->pos[0], 0.1f);
    approach_f32_asymptotic_bool(&c->focus[1], (&gPlayerCameraState[c->cameraID])->pos[1] + 110.f, 0.1f);
    approach_f32_asymptotic_bool(&c->focus[2], (&gPlayerCameraState[c->cameraID])->pos[2], 0.1f);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_pan_left(struct Camera *c) {
    vec3f_copy(c->focus, (&gPlayerCameraState[c->cameraID])->pos);
    c->focus[1] += 110.f;
    camera_approach_s16_symmetric_bool(&sCutsceneVars[0][c->cameraID].angle[1], 0x800, 0x20);
    pan_camera(c, sCutsceneVars[0][c->cameraID].angle[0], sCutsceneVars[0][c->cameraID].angle[1]);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press_create_dialog(UNUSED struct Camera *c) {
    create_dialog_box_with_response(gCutsceneFocus[c->cameraID]->oBehParams2ndByte + DIALOG_010);
}
...
BAD_RETURN(s32) cutscene_cap_switch_press(struct Camera *c) {
    ...
    if (gDialogResponse != 0) {
        sCutsceneVars[4][c->cameraID].angle[0] = gDialogResponse;
    }

    if ((get_dialog_id() == -1) && (sCutsceneVars[4][c->cameraID].angle[0] != 0)) {
        sCutsceneDialogResponse[c->cameraID] = sCutsceneVars[4][c->cameraID].angle[0];
        if (sCutsceneVars[4][c->cameraID].angle[0] == 1) {
...
BAD_RETURN(s32) cutscene_unlock_key_door_start(struct Camera *c) {
    ...
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, c->pos);
    vec3f_copy(sCutsceneVars[1][c->cameraID].point, c->focus);
    vec3f_set(posOff, -206.f, 108.f, 234.f);
    vec3f_set(focusOff, 48.f, 104.f, -193.f);
    offset_rotated(sCutsceneVars[2][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, posOff, (&gPlayerCameraState[c->cameraID])->faceAngle);
    offset_rotated(sCutsceneVars[3][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos, focusOff, (&gPlayerCameraState[c->cameraID])->faceAngle);
}
...
BAD_RETURN(s32) cutscene_unlock_key_door_approach_mario(struct Camera *c) {
    approach_vec3f_asymptotic(c->pos, sCutsceneVars[2][c->cameraID].point, 0.1f, 0.1f, 0.1f);
    approach_vec3f_asymptotic(c->focus, sCutsceneVars[3][c->cameraID].point, 0.1f, 0.1f, 0.1f);
}
...
BAD_RETURN(s32) cutscene_unlock_key_door_focus_lock(UNUSED struct Camera *c) {
    approach_f32_asymptotic_bool(&sCutsceneVars[3][c->cameraID].point[1], (&gPlayerCameraState[c->cameraID])->pos[1] + 140.f, 0.07f);
}
...
BAD_RETURN(s32) cutscene_unlock_key_door_fly_back(struct Camera *c) {
    approach_vec3f_asymptotic(c->pos, sCutsceneVars[0][c->cameraID].point, 0.1f, 0.1f, 0.1f);
    approach_vec3f_asymptotic(c->focus, sCutsceneVars[1][c->cameraID].point, 0.1f, 0.1f, 0.1f);
}
 ...
 * `focusSpline`. sCutsceneSplineSegmentProgress[c->cameraID] is updated after pos and focus are calculated.
 */
s32 intro_peach_move_camera_start_to_pipe(struct Camera *c, struct CutsceneSplinePoint positionSpline[],
    ...
    /**
     * The position spline's speed parameters are all 0, so sCutsceneSplineSegmentProgress[c->cameraID] doesn't get
     ...
     */
    posReturn = move_point_along_spline(c->pos, positionSpline, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    focusReturn = move_point_along_spline(c->focus, focusSpline, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
...
BAD_RETURN(s32) cutscene_intro_peach_dialog(struct Camera *c) {
    if (get_dialog_id() == -1) {
        vec3f_copy(gLakituState[c->cameraID].goalPos, c->pos);
        vec3f_copy(gLakituState[c->cameraID].goalFocus, c->focus);
        sStatusFlags[c->cameraID] |= (CAM_FLAG_SMOOTH_MOVEMENT | CAM_FLAG_UNUSED_CUTSCENE_ACTIVE);
        gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
        c->cutscene = 0;
    }
}
...
BAD_RETURN(s32) cutscene_intro_peach_follow_pipe_spline(struct Camera *c) {
    move_point_along_spline(c->pos, sIntroPipeToDialogPosition, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    move_point_along_spline(c->focus, sIntroPipeToDialogFocus, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
}
...
BAD_RETURN(s32) cutscene_intro_peach_zoom_fov(UNUSED struct Camera *c) {
    sFOVState[c->cameraID].fov = 8.f;
    set_fov_function(CAM_FOV_ZOOM_30, c);
}

/**
 * Reset the spline progress, turn on handheld shake.
 */
BAD_RETURN(s32) cutscene_intro_peach_reset_spline(struct Camera *c) {
    sCutsceneSplineSegment[c->cameraID] = 0;
    sCutsceneSplineSegmentProgress[c->cameraID] = 0.1f;
...
BAD_RETURN(s32) cutscene_intro_peach_handheld_shake_off(UNUSED struct Camera *c) {
    set_handheld_shake(HAND_CAM_SHAKE_OFF, c);
}
...
BAD_RETURN(s32) cutscene_intro_peach_fly_to_pipe(struct Camera *c) {
    ...
    sCutsceneVars[1][c->cameraID].point[1] = 400.f;
}
...
BAD_RETURN(s32) cutscene_intro_peach_mario_appears(struct Camera *c) {
    ...
    approach_f32_asymptotic_bool(&sCutsceneVars[1][c->cameraID].point[1], 80.f + sMarioGeometry[c->cameraID].currFloorHeight +
                                 ((&gPlayerCameraState[c->cameraID])->pos[1] - sMarioGeometry[c->cameraID].currFloorHeight) * 1.1f, 0.4f);

    // Make the camera look up as Mario jumps out of the pipe
    if (c->focus[1] < sCutsceneVars[1][c->cameraID].point[1]) {
        c->focus[1] = sCutsceneVars[1][c->cameraID].point[1];
    }
...
BAD_RETURN(s32) cutscene_end_waving(struct Camera *c) {
    ...
    move_point_along_spline(c->pos, gEndWavingPos, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    move_point_along_spline(c->focus, gEndWavingFocus, &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    cutscene_spawn_obj(6, 120, c);
}
...
BAD_RETURN(s32) cutscene_credits(struct Camera *c) {
    ...
    copy_spline_segment(sCurCreditsSplinePos[c->cameraID], pos);
    copy_spline_segment(sCurCreditsSplineFocus[c->cameraID], focus);
    move_point_along_spline(c->pos, sCurCreditsSplinePos[c->cameraID], &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
    move_point_along_spline(c->focus, sCurCreditsSplineFocus[c->cameraID], &sCutsceneSplineSegment[c->cameraID], &sCutsceneSplineSegmentProgress[c->cameraID]);
...
BAD_RETURN(s32) cutscene_sliding_doors_open_start(struct Camera *c) {
    ...
    if (dist < 500.f) {
        dist = 500.f;
        yaw = (&gPlayerCameraState[c->cameraID])->faceAngle[1] + 0x8800;
        pitch = 0x800;
    }
...
BAD_RETURN(s32) cutscene_sliding_doors_open_set_cvars(UNUSED struct Camera *c) {
    vec3f_copy(sCutsceneVars[1][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    vec3s_copy(sCutsceneVars[0][c->cameraID].angle, (&gPlayerCameraState[c->cameraID])->faceAngle);
    vec3f_set(sCutsceneVars[0][c->cameraID].point, 80.f, 325.f, 200.f);
}
...
BAD_RETURN(s32) cutscene_sliding_doors_go_under_doorway(UNUSED struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[0][c->cameraID].point[1], 75.f, 10.f);
}
...
BAD_RETURN(s32) cutscene_sliding_doors_fly_back_up(UNUSED struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[0][c->cameraID].point[1], 125.f, 10.f);
}
...
BAD_RETURN(s32) cutscene_sliding_doors_follow_mario(struct Camera *c) {
    ...
    sCutsceneVars[1][c->cameraID].point[0] = (&gPlayerCameraState[c->cameraID])->pos[0];
    sCutsceneVars[1][c->cameraID].point[2] = (&gPlayerCameraState[c->cameraID])->pos[2];

    // Decrease cvar0's offsets, moving the camera behind Mario at his eye height.
    approach_f32_asymptotic_bool(&sCutsceneVars[0][c->cameraID].point[0], 0, 0.1f);
    camera_approach_f32_symmetric_bool(&sCutsceneVars[0][c->cameraID].point[2], 125.f, 50.f);
    // Update cvar0's angle
    approach_vec3s_asymptotic(sCutsceneVars[0][c->cameraID].angle, (&gPlayerCameraState[c->cameraID])->faceAngle, 16, 16, 16);

    // Apply the offset to the camera's position
    offset_rotated(pos, sCutsceneVars[1][c->cameraID].point, sCutsceneVars[0][c->cameraID].point, sCutsceneVars[0][c->cameraID].angle);
...
BAD_RETURN(s32) cutscene_enter_painting(struct Camera *c) {
    ...
    // Zoom in
    set_fov_function(CAM_FOV_APP_20, c);
    sStatusFlags[c->cameraID] |= CAM_FLAG_SMOOTH_MOVEMENT;
    ...
    if (gRipplingPainting != NULL) {
        ...
        find_floor((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 50.f, (&gPlayerCameraState[c->cameraID])->pos[2], &floor);
...
BAD_RETURN(s32) cutscene_exit_painting_start(struct Camera *c) {
    ...
    vec3f_set(sCutsceneVars[2][c->cameraID].point, 258.f, -352.f, 1189.f);
    vec3f_set(sCutsceneVars[1][c->cameraID].point, 65.f, -155.f, 444.f);

    if (gPrevLevel == LEVEL_TTM) {
        sCutsceneVars[1][c->cameraID].point[1] = 0.f;
        sCutsceneVars[1][c->cameraID].point[2] = 0.f;
    }
    vec3f_copy(sCutsceneVars[0][c->cameraID].point, (&gPlayerCameraState[c->cameraID])->pos);
    sCutsceneVars[0][c->cameraID].angle[0] = 0;
    sCutsceneVars[0][c->cameraID].angle[1] = (&gPlayerCameraState[c->cameraID])->faceAngle[1];
    sCutsceneVars[0][c->cameraID].angle[2] = 0;
    offset_rotated(c->focus, sCutsceneVars[0][c->cameraID].point, sCutsceneVars[1][c->cameraID].point, sCutsceneVars[0][c->cameraID].angle);
    offset_rotated(c->pos, sCutsceneVars[0][c->cameraID].point, sCutsceneVars[2][c->cameraID].point, sCutsceneVars[0][c->cameraID].angle);
...
BAD_RETURN(s32) cutscene_exit_painting_move_to_mario(struct Camera *c) {
    ...
    approach_f32_asymptotic_bool(&sCutsceneVars[2][c->cameraID].point[0], 178.f, 0.05f);
    approach_f32_asymptotic_bool(&sCutsceneVars[2][c->cameraID].point[2], 889.f, 0.05f);
    offset_rotated(pos, sCutsceneVars[0][c->cameraID].point, sCutsceneVars[2][c->cameraID].point, sCutsceneVars[0][c->cameraID].angle);
    c->pos[0] = pos[0];
    c->pos[2] = pos[2];
}
...
BAD_RETURN(s32) cutscene_exit_painting_move_to_floor(struct Camera *c) {
    ...
    vec3f_copy(floorHeight, (&gPlayerCameraState[c->cameraID])->pos);
    floorHeight[1] = find_floor((&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 10.f, (&gPlayerCameraState[c->cameraID])->pos[2], &floor);

    if (floor != NULL) {
        floorHeight[1] = floorHeight[1] + ((&gPlayerCameraState[c->cameraID])->pos[1] - floorHeight[1]) * 0.7f + 125.f;
...
BAD_RETURN(s32) cutscene_unused_exit_start(struct Camera *c) {
    ...
    vec3s_set(marioAngle, 0, (&gPlayerCameraState[c->cameraID])->faceAngle[1], 0);
    offset_rotated(c->pos, (&gPlayerCameraState[c->cameraID])->pos, offset, marioAngle);
...
BAD_RETURN(s32) cutscene_unused_exit_focus_mario(struct Camera *c) {
    ...
    vec3f_set(focus, (&gPlayerCameraState[c->cameraID])->pos[0], (&gPlayerCameraState[c->cameraID])->pos[1] + 125.f, (&gPlayerCameraState[c->cameraID])->pos[2]);
    set_focus_rel_mario(c, 0.f, 125.f, 0.f, 0);
...
BAD_RETURN(s32) cutscene_enter_cannon_end(struct Camera *c) {
    ...
    c->cutscene = 0;
    sCannonYOffset[c->cameraID] = 800.f;
}
...
BAD_RETURN(s32) cutscene_enter_cannon_raise(struct Camera *c) {
    ...
    sStatusFlags[c->cameraID] |= CAM_FLAG_SMOOTH_MOVEMENT;
    camera_approach_s16_symmetric_bool(&sCutsceneVars[1][c->cameraID].angle[0], 0, 0x80);
    camera_approach_s16_symmetric_bool(&sCutsceneVars[2][c->cameraID].angle[0], 0, 0x80);
    // Move the camera around the cannon, gradually rotating and moving closer
    vec3f_set_dist_and_angle(sCutsceneVars[0][c->cameraID].point, c->pos, sCutsceneVars[1][c->cameraID].point[2], sCutsceneVars[1][c->cameraID].angle[0],
                             sCutsceneVars[1][c->cameraID].angle[1]);
    sCutsceneVars[1][c->cameraID].point[2] = approach_f32(sCutsceneVars[1][c->cameraID].point[2], 400.f, 5.f, 5.f);
    sCutsceneVars[1][c->cameraID].angle[1] += 0x40;
    sCutsceneVars[3][c->cameraID].point[1] += 2.f;
    c->pos[1] += sCutsceneVars[3][c->cameraID].point[1];

    if ((o = (&gPlayerCameraState[c->cameraID])->usedObj) != NULL) {
        sCutsceneVars[0][c->cameraID].point[1] = o->oPosY;
...
BAD_RETURN(s32) cutscene_enter_cannon_start(struct Camera *c) {
    ...
    if ((o = (&gPlayerCameraState[c->cameraID])->usedObj) != NULL) {
        sCutsceneVars[0][c->cameraID].point[0] = o->oPosX;
        sCutsceneVars[0][c->cameraID].point[1] = o->oPosY;
        sCutsceneVars[0][c->cameraID].point[2] = o->oPosZ;
        sCutsceneVars[0][c->cameraID].angle[0] = o->oMoveAnglePitch;
        sCutsceneVars[0][c->cameraID].angle[1] = o->oMoveAngleYaw;
        sCutsceneVars[0][c->cameraID].angle[2] = o->oMoveAngleRoll;
    }

    // Store the camera's polar offset from the cannon in cvar1
    vec3f_get_dist_and_angle(sCutsceneVars[0][c->cameraID].point, c->pos, &sCutsceneVars[1][c->cameraID].point[2],
                             &sCutsceneVars[1][c->cameraID].angle[0], &sCutsceneVars[1][c->cameraID].angle[1]);
    sCutsceneVars[3][c->cameraID].point[1] = 0.f;
    //! cvar4 is unused in this cutscene
    sCutsceneVars[4][c->cameraID].point[1] = 0.f;
}
...
BAD_RETURN(s32) cutscene_door_fix_cam(struct Camera *c) {
    vec3f_copy(c->pos, sCutsceneVars[0][c->cameraID].point);
    vec3f_copy(c->focus, sCutsceneVars[1][c->cameraID].point);
}
...
BAD_RETURN(s32) cutscene_door_loop(struct Camera *c) {
    ...
    if (((&gPlayerCameraState[c->cameraID])->action != ACT_PULLING_DOOR) & ((&gPlayerCameraState[c->cameraID])->action != ACT_PUSHING_DOOR)) {
        gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
        c->cutscene = 0;
    }
}
...
BAD_RETURN(s32) cutscene_door_move_behind_mario(struct Camera *c) {
    ...
    reset_pan_distance(c);
    determine_pushing_or_pulling_door(&doorRotation, c);
    set_focus_rel_mario(c, 0.f, 125.f, 0.f, 0);
    vec3s_set(sCutsceneVars[0][c->cameraID].angle, 0, (&gPlayerCameraState[c->cameraID])->faceAngle[1] + doorRotation, 0);
    ...
    offset_rotated(c->pos, (&gPlayerCameraState[c->cameraID])->pos, camOffset, sCutsceneVars[0][c->cameraID].angle);
}
...
BAD_RETURN(s32) cutscene_door_mode(struct Camera *c) {
    ...
    // Loop until Mario is no longer using the door
    if ((&gPlayerCameraState[c->cameraID])->action != ACT_ENTERING_STAR_DOOR &&
        (&gPlayerCameraState[c->cameraID])->action != ACT_PULLING_DOOR &&
        (&gPlayerCameraState[c->cameraID])->action != ACT_PUSHING_DOOR) {
        gCutsceneTimer[c->cameraID] = CUTSCENE_STOP;
        c->cutscene = 0;
    }
}
...
/**
 * Play the current cutscene until either gCutsceneTimer[c->cameraID] reaches the max time, or c->cutscene is set to 0
 *
...
void play_cutscene(struct Camera *c) {
    ...
    unusedYawFocToMario = sAreaYaw[c->cameraID];
...
#define CUTSCENE(id, cutscene)                                                                            \
    case id:                                                                                              \
        cutsceneDuration = cutscene[sCutsceneShot[c->cameraID]].duration;                                              \
        cutscene[sCutsceneShot[c->cameraID]].shot(c);                                                                  \
        break;
...
#undef CUTSCENE
    ...
    if ((cutsceneDuration != 0) && !(gCutsceneTimer[c->cameraID] & CUTSCENE_STOP)) {
        ...
        //! Because gCutsceneTimer[c->cameraID] is often set to 0x7FFF (CUTSCENE_LOOP), this conditional can only
    ...
    if ((c->cutscene == 0) && (oldCutscene != 0)) {
        gRecentCutscene[c->cameraID] = oldCutscene;
    }
}

/**
 * Call the event while `start` <= gCutsceneTimer[c->cameraID] <= `end`
...
/**
 * Set gCutsceneObjSpawn[c->cameraID] when gCutsceneTimer[c->cameraID] == `frame`.
 *
 * @see intro_scene.inc.c for details on which objects are spawned.
 */
s32 cutscene_spawn_obj(u32 obj, s16 frame, struct Camera *c) {
    if (frame == gCutsceneTimer[c->cameraID]) {
        gCutsceneObjSpawn[c->cameraID] = obj;
    }
    return 0;
}
 ...
 */
void set_fov_shake(s16 amplitude, s16 decay, s16 shakeSpeed, struct Camera *c) {
    if (amplitude > sFOVState[c->cameraID].shakeAmplitude) {
        sFOVState[c->cameraID].shakeAmplitude = amplitude;
        sFOVState[c->cameraID].decay = decay;
        sFOVState[c->cameraID].shakeSpeed = shakeSpeed;
    }
}
...
void set_fov_shake_from_point(s16 amplitude, s16 decay, s16 shakeSpeed, f32 maxDist, f32 posX, f32 posY, f32 posZ) {
    amplitude = reduce_by_dist_from_camera(amplitude, maxDist, posX, posY, posZ, gMarioStates[gMarioObject->oAnimState].thisPlayerCamera);

    if (amplitude != 0) {
        if (amplitude > sFOVState[gMarioObject->oAnimState].shakeAmplitude) { // literally use the function above you silly nintendo, smh
            sFOVState[gMarioObject->oAnimState].shakeAmplitude = amplitude;
            sFOVState[gMarioObject->oAnimState].decay = decay;
            sFOVState[gMarioObject->oAnimState].shakeSpeed = shakeSpeed;
        }
    }
}
...
void shake_camera_fov(struct GraphNodePerspective *perspective) {
    ...
    for (i = 0; i < gActivePlayers; i++) {
        if (sFOVState[i].shakeAmplitude != 0.f) {
            ...
            camera_approach_f32_symmetric_bool(&sFOVState[i].shakeAmplitude, 0.f, sFOVState[i].decay);
            perspective->fov += sFOVState[i].fovOffset;
        } else {
            sFOVState[i].shakePhase = 0;
        }
    }
}

void set_fov_30(UNUSED struct MarioState *m) {
    sFOVState[m->thisPlayerCamera->cameraID].fov = 30.f;
}

void approach_fov_20(UNUSED struct MarioState *m) {
    camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 20.f, 0.3f);
}

void set_fov_45(UNUSED struct MarioState *m) {
    sFOVState[m->thisPlayerCamera->cameraID].fov = 45.f;
}

void set_fov_29(UNUSED struct MarioState *m) {
    sFOVState[m->thisPlayerCamera->cameraID].fov = 29.f;
}
...
void zoom_fov_30(UNUSED struct MarioState *m) {
    ...
    // Nintendo.
    camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 30.f, (30.f - sFOVState[m->thisPlayerCamera->cameraID].fov) / 60.f);
}
...
void fov_default(struct MarioState *m) {
    sStatusFlags[m->thisPlayerCamera->cameraID] &= ~CAM_FLAG_SLEEPING;

    if ((m->action == ACT_SLEEPING) || (m->action == ACT_START_SLEEPING)) {
        camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 30.f, (30.f - sFOVState[m->thisPlayerCamera->cameraID].fov) / 30.f);
        sStatusFlags[m->thisPlayerCamera->cameraID] |= CAM_FLAG_SLEEPING;
    } else {
        camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 45.f, (45.f - sFOVState[m->thisPlayerCamera->cameraID].fov) / 30.f);
        sFOVState[m->thisPlayerCamera->cameraID].unusedIsSleeping = 0;
    }
...
void approach_fov_30(UNUSED struct MarioState *m) {
    camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 30.f, 1.f);
}

void approach_fov_60(UNUSED struct MarioState *m) {
    camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 60.f, 1.f);
}

void approach_fov_45(struct MarioState *m) {
    f32 targetFoV = sFOVState[m->thisPlayerCamera->cameraID].fov;
    ...
    sFOVState[m->thisPlayerCamera->cameraID].fov = approach_f32(sFOVState[m->thisPlayerCamera->cameraID].fov, targetFoV, 2.f, 2.f);
}

void approach_fov_80(UNUSED struct MarioState *m) {
    camera_approach_f32_symmetric_bool(&sFOVState[m->thisPlayerCamera->cameraID].fov, 80.f, 3.5f);
}
...
Gfx *geo_camera_fov(s32 callContext, struct GraphNode *g, UNUSED void *context) {
    ...
    struct MarioState *marioState = &gMarioStates[0];
    u8 fovFunc = sFOVState[0].fovFunc;
    ...
    perspective->fov = sFOVState[0].fov;
 ...
 * @see geo_camera_fov
 */
void set_fov_function(u8 func, struct Camera *c) {
    sFOVState[c->cameraID].fovFunc = func;
}

/**
 * Start a preset fov shake. Used in cutscenes
 */
void cutscene_set_fov_shake_preset(u8 preset, struct Camera *c) {
    switch (preset) {
        case 1:
            set_fov_shake(0x100, 0x30, 0x8000, c);
            break;
        case 2:
            set_fov_shake(0x400, 0x20, 0x4000, c);
            break;
    }
}
