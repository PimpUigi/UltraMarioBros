...
f32 gPaintingLuigiXPos;
f32 gPaintingLuigiYPos;
f32 gPaintingLuigiZPos;
...
f32 painting_mario_y(struct Painting *painting) {
    float mYOffset = gPaintingMarioYPos - painting->posY;
    float lYOffset = gPaintingLuigiYPos - painting->posY;

    if (mYOffset < 0.0) {
        mYOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (mYOffset > painting->size) {
        mYOffset =
            painting->size; // If Mario is past the right side, return the right side
                                                   
    }

    if (lYOffset < 0.0) {
        lYOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lYOffset > painting->size) {
        lYOffset =
            painting->size; // If Mario is past the right side, return the right side
    }

    return mYOffset * (mYOffset < lYOffset)
           + lYOffset
                 * (mYOffset >= lYOffset);
}
...
f32 painting_mario_z(struct Painting *painting) {
    float mZOffset = gPaintingMarioZPos - painting->posZ;
    float lZOffset = gPaintingLuigiZPos - painting->posZ;

    if (mZOffset < 0.0) {
        mZOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (mZOffset > painting->size) {
        mZOffset =
            painting->size; // If Mario is past the right side, return the right side
    }

    if (lZOffset < 0.0) {
        lZOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lZOffset > painting->size) {
        lZOffset =
            painting->size; // If Mario is past the right side, return the right side
    }

    return mZOffset * (mZOffset < lZOffset)
           + lZOffset
                 * (mZOffset >= lZOffset);
}
...
f32 painting_mario_x(struct Painting *painting) {
    float mXOffset = gPaintingMarioXPos - painting->posX;
    float lXOffset = gPaintingLuigiXPos - painting->posX;

    if (mXOffset < 0.0) {
        mXOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (mXOffset > painting->size) {
        mXOffset =
            painting->size; // If Mario is past the right side, return the right side
    }

    if (lXOffset < 0.0) {
        lXOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lXOffset > painting->size) {
        lXOffset =
            painting->size; // If Mario is past the right side, return the right side
    }

    return mXOffset * (mXOffset < lXOffset)
           + lXOffset
                 * (mXOffset >= lXOffset);
}
...
void painting_update_floors(struct Painting *painting) {
    ...
    if ((gPaintingMarioYPos < painting->posY) || (gPaintingLuigiYPos < painting->posY)) {
