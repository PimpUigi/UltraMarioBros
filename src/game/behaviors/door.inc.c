...
void set_door_camera_event(void) {
    if (segmented_to_virtual(bhvDoor) == o->behavior)
        gPlayerCameraState[gMarioObject->oAnimState].cameraEvent = CAM_EVENT_DOOR;
    else
        gPlayerCameraState[gMarioObject->oAnimState].cameraEvent = CAM_EVENT_DOOR_WARP;
    gPlayerCameraState[gMarioObject->oAnimState].usedObj = o;
}
