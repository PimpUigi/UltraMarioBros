...
void intro_peach_set_pos_and_opacity(struct Object *o, f32 targetOpacity, f32 increment) {
    ...
    vec3f_get_dist_and_angle(gLakituState[0].pos, gLakituState[0].focus, &dist, &focusPitch, &focusYaw);
    vec3f_set_dist_and_angle(gLakituState[0].pos, newPos, o->oIntroPeachDistToCamera, o->oIntroPeachPitchFromFocus + focusPitch,
