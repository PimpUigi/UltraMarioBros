...
void bhv_camera_lakitu_update(void) {
    if (!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM)) {
        ...
        if (o->oBehParams2ndByte != CAMERA_LAKITU_BP_FOLLOW_CAMERA) {
            ...
            f32 val0C = (f32) 0x875C3D / 0x800 - gLakituState[0].curPos[0];
            if (gLakituState[0].curPos[0] < 1700.0f || val0C < 0.0f) {
                cur_obj_hide();
            } else {
                cur_obj_unhide();

                o->oPosX = gLakituState[0].curPos[0];
                o->oPosY = gLakituState[0].curPos[1];
                o->oPosZ = gLakituState[0].curPos[2];
