...
extern void set_camera_shake_from_hit(s16, struct Camera *c);
extern void set_environmental_camera_shake(s16, struct Camera *c);
...
extern s32 cam_select_alt_mode(s32 angle, struct Camera *c);
extern s32 set_cam_angle(s32, struct Camera *c);
extern void set_handheld_shake(u8, struct Camera *c);
extern void shake_camera_handheld(Vec3f, Vec3f, struct Camera *c);
...
extern s32 is_within_100_units_of_mario(f32, f32, f32, struct Camera *c);
extern s32 set_or_approach_f32_asymptotic(f32 *, f32, f32, struct Camera *c);
...
extern void set_or_approach_vec3f_asymptotic(Vec3f dst, Vec3f goal, f32 xMul, f32 yMul, f32 zMul, struct Camera *c); // postdefined
extern s32 camera_approach_s16_symmetric_bool(s16 *current, s16 target, s16 increment);
extern s32 set_or_approach_s16_symmetric(s16 *current, s16 target, s16 increment, struct Camera *c); // postdefined
...
extern void set_camera_pitch_shake(s16 mag, s16 pitchMagInc, s16 pitchInc, struct Camera *c);
extern void set_camera_yaw_shake(s16   mag, s16 yawMagInc, s16 yawInc, struct Camera *c);
extern void set_camera_roll_shake(s16  mag, s16 rollMagInc, s16 rollInc, struct Camera *c);
...
extern void play_camera_buzz_if_cdown(struct Camera *c);
extern void play_camera_buzz_if_cbutton(struct Camera *c);
extern void play_camera_buzz_if_c_sideways(struct Camera *c);
...
extern void play_sound_if_cam_switched_to_lakitu_or_mario(struct Camera *c); // postdefined
...
extern void warp_camera(f32 displacementX, f32 displacementY, f32 displacementZ, struct MarioState *m);
...
extern void set_fixed_cam_axis_sa_lobby(s16 preset, struct Camera *c); // postdefined
...
extern void find_mario_floor_and_ceil(struct PlayerGeometry *, struct Camera *c); // postdefined
extern u8 start_object_cutscene_without_focus(u8, struct Camera *c);
...
extern s32 cutscene_spawn_obj(s32 phase, s16 frame, struct Camera *c);
extern void set_fov_shake(s16 amplitude, s16 decay, s16 shakeSpeed, struct Camera *c);

extern void set_fov_function(u8 func, struct Camera *c);
extern void cutscene_set_fov_shake_preset(u8 preset, struct Camera *c);
...
extern struct ModeTransitionInfo sModeInfo[2]; // D_8033B418
