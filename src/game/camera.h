...
extern struct PlayerCameraState *sLuigiCamState;
...
extern u8 gRecentCutscene[2];

extern Vec3f sCastleEntranceOffset[2];
// TODO: sort all of this extremely messy shit out after the split

void set_camera_shake_from_hit(s16, struct Camera *c);
void set_environmental_camera_shake(s16, struct Camera *c);
void set_camera_shake_from_point(s16, f32, f32, f32);
void move_mario_head_c_up(struct Camera *); // static (ASM)
void transition_next_state(UNUSED struct Camera *c, s16 frames);
void set_camera_mode(struct Camera *, s16, s16);
void update_camera(struct Camera *);
void reset_camera(struct Camera *);
...
void vec3f_to_object_pos(struct Object *, Vec3f); // static (ASM)
s32 move_point_along_spline(Vec3f, struct CutsceneSplinePoint[], s16 *, f32 *);
s32 cam_select_alt_mode(s32 angle, struct Camera *c);
s32 set_cam_angle(s32, struct Camera *c);
void set_handheld_shake(u8, struct Camera *c);
void shake_camera_handheld(Vec3f, Vec3f, struct Camera *c);
s32 find_c_buttons_pressed(u16, u16, u16);
s32 update_camera_hud_status(struct Camera *);
s32 collide_with_walls(Vec3f, f32, f32);
s32 clamp_pitch(Vec3f a, Vec3f b, s16 c, s16 d);
s32 is_within_100_units_of_mario(f32, f32, f32, struct Camera *c);
s32 set_or_approach_f32_asymptotic(f32 *, f32, f32, struct Camera *c);
s32 approach_f32_asymptotic_bool(f32 *current, f32 target, f32 multiplier);
f32 approach_f32_asymptotic(f32 current, f32 target, f32 multiplier); // static (ASM)
s32 approach_s16_asymptotic_bool(s16 *current, s16 target, s16 multiplier);
s32 approach_s16_asymptotic(s16 current, s16 target, s16 multiplier); // static (ASM)
void approach_vec3f_asymptotic(Vec3f current, Vec3f target, f32 xMul, f32 yMul, f32 zMul); // static (ASM)

void set_or_approach_vec3f_asymptotic(Vec3f dst, Vec3f goal, f32 xMul, f32 yMul, f32 zMul, struct Camera *c); // postdefined
s32 camera_approach_s16_symmetric_bool(s16 *current, s16 target, s16 increment);
s32 set_or_approach_s16_symmetric(s16 *current, s16 target, s16 increment, struct Camera *c); // postdefined
s32 camera_approach_f32_symmetric_bool(f32 *current, f32 target, f32 increment);
f32 camera_approach_f32_symmetric(f32 value, f32 target, f32 increment);
void random_vec3s(Vec3s dst, s16 xRange, s16 yRange, s16 zRange); // postdefined
s32 clamp_positions_and_find_yaw(Vec3f, Vec3f, f32, f32, f32, f32);
...
void set_camera_pitch_shake(s16 mag, s16 pitchMagInc, s16 pitchInc, struct Camera *c);
void set_camera_yaw_shake(s16   mag, s16 yawMagInc, s16 yawInc, struct Camera *c);
void set_camera_roll_shake(s16  mag, s16 rollMagInc, s16 rollInc, struct Camera *c);
void set_pitch_shake_from_point(s16 mag, s16 pitchMagInc, s16 pitchInc, f32 maxDist, f32 posX, f32 posY, f32 posZ);
void shake_camera_pitch(); // postdefined
void shake_camera_yaw(); // postdefined
...
void play_sound_button_change_blocked(void); // postdefined
void play_sound_rbutton_changed(void); // postdefined
void play_sound_if_cam_switched_to_lakitu_or_mario(struct Camera *c); // postdefined
s32 radial_camera_input(struct Camera *a, f32 b);
s32 trigger_cutscene_dialog(s32);
void handle_c_button_movement(struct Camera *);
void start_cutscene(struct Camera *a, u8 cutscene); // postdefined
u8 get_cutscene_from_mario_status(); // postdefined
void warp_camera(f32 displacementX, f32 displacementY, f32 displacementZ, struct MarioState *m);
void approach_camera_height(struct Camera *c, f32 goal, f32 inc);
void offset_rotated(Vec3f dst, Vec3f from, Vec3f to, Vec3s rotation);
s16 next_lakitu_state(Vec3f newPos, Vec3f newFoc, Vec3f curPos, Vec3f curFoc, Vec3f oldPos, Vec3f oldFoc, s16 yaw, struct Camera *c); // postdefined
void set_fixed_cam_axis_sa_lobby(s16 preset, struct Camera *c); // postdefined
s16 camera_course_processing(struct Camera *); // postdefined
void resolve_geometry_collisions(Vec3f, Vec3f);
s32 rotate_camera_around_walls(struct Camera *, Vec3f, s16 *, s16);
void find_mario_floor_and_ceil(struct PlayerGeometry *, struct Camera *c); // postdefined
u8 start_object_cutscene_without_focus(u8, struct Camera *c);
s16 cutscene_object_with_dialog(u8 cutsceneTable, struct Object *, s16);
s16 cutscene_object_without_dialog(u8, struct Object *);
s16 cutscene_object(u8 cutscene, struct Object *o);

extern int areaWarped;

void play_cutscene(struct Camera *c);
s32 cutscene_event(CameraEvent event, struct Camera * c, s16 start, s16 end);
s32 cutscene_spawn_obj(u32 phase, s16 frame, struct Camera *c);
...
void obj_rotate_towards_point(struct Object *, Vec3f, s16, s16, s16, s16);
