...
Gfx *geo_switch_area(s32 callContext, struct GraphNode *node) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        if (gMarioObject == NULL) {
            ...
            find_floor(gMarioStates[gCamera->cameraID].marioObj->oPosX, gMarioStates[gCamera->cameraID].marioObj->oPosY, gMarioStates[gCamera->cameraID].marioObj->oPosZ, &sp20);

            if (sp20) {
                if (sp20->object == NULL) {
                    gMarioCurrentRoom[gCamera->cameraID] = sp20->room;
                }
                sp26 = gMarioCurrentRoom[gCamera->cameraID] - 1;
...
void cur_obj_start_cam_event(UNUSED struct Object *obj, s32 cameraEvent) {
    gPlayerCameraState[gCamera->cameraID].cameraEvent = (s16) cameraEvent;
    gSecondCameraFocus[gCamera->cameraID] = o;
}
...
void cur_obj_push_mario_away(f32 radius, struct MarioState *m) {
...
void cur_obj_push_mario_away_from_cylinder(f32 radius, f32 extentY) {
    ...
    for (i = 0; i < activePlayers; i++) {
        ...
        if (marioRelY < extentY) {
            cur_obj_push_mario_away(radius, &gMarioStates[i]);
        }
    }
}
